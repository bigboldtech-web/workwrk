...
flowchart TD
  ...
  Item -.->|"no notification on assign or status"| Notif["Notifications"]
  Item -.->|"invisible to these, they read legacy Task"| Search["Search, analytics, task score"]
```
...
**Board tasks emit no notifications.** Assigning an Item, changing its status, or a due date arriving writes no `Notification` row. The only notification the Item pipeline produces comes from comments (`/api/items/[id]/updates`). There is an SLA escalation cron that does notify, but it walks the legacy `Task` table, so it never sees your board work. The bell in the top right is a **reminders** bell showing your pending reminders, not a notification bell.

**Four surfaces still read the legacy `Task` table, not `Item`.** Tasks you create on a List are `Item` rows. These do not see them:

- Global search and the command palette (`/api/search`) index User, legacy Task, SOP, Doc, Department, Meeting, OKR, Idea, Policy and Announcement, plus four leftovers from removed finance modules (Vendor, PurchaseOrder, GlAccount, BudgetPlan) whose result links point at `/procurement`, `/financials` and `/planning` and therefore 404. Items, Boards, Spaces and Whiteboards are not searchable.
