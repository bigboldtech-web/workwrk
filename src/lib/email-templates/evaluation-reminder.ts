...
export function evaluationReminderTemplate(vars: EvaluationReminderVars): { subject: string; html: string } {
  const memberList = vars.teamMembers.map((m) => `<li style="margin-bottom:4px;color:#a0a0a0;font-size:13px;">${m}</li>`).join("");
  ...
  const html = baseLayout(`
    ...
    <p style="font-size:13px;color:#a0a0a0;">Your direct reports:</p>
