...
// (up to 100) names the chips already on a schedule, eligible or not, so a
// colleague who went inactive shows as a greyed chip and one back from leave
// can be found and added again. An id from another org answers nothing.
//
...
export async function GET(req: Request) {
  ...
  const people = await recipientOptions({ organizationId: c.organizationId, q, ids, limit });
