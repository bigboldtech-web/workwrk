                  ...
                  <div className="cmpl__list">
                    {pending.map((o) => (
                      <Link key={`${o.policyId}:${o.userId}`} href={`/policies/${o.policyId}/compliance`} className="cmpl__over" style={{ textDecoration: "none" }}>
                        ...
                        <span className="text-xs font-medium" style={{ color: o.status === "overdue" ? "var(--os-c-red)" : o.status === "out-of-date" ? "var(--os-c-orange)" : "var(--os-c-orange)" }}>
...
function MoreToggle({ open, total, onClick }: { open: boolean; total: number; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className="mt-1 text-sm text-zinc-500 hover:text-zinc-800 underline underline-offset-2">
