      ...
      <div className="px-6 py-6">
        {err ? (
          ...
          <>
                    ...
                    return (
                      <tr key={r.userId} className="hover:bg-zinc-50/60">
                        <td className="px-3 py-2">
                          ...
                          {r.email ? <div className="text-xs text-zinc-400">{r.email}</div> : null}
                        </td>
                        ...
                        <td className="px-3 py-2 font-mono text-xs text-zinc-400">{r.ipAddress ?? "—"}</td>
                        <td className="max-w-[260px] px-3 py-2 text-xs text-zinc-400" title={r.attestation ?? ""}><span className="line-clamp-1">{r.attestation ?? "—"}</span></td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <p className="mt-3 text-xs text-zinc-400">Each acknowledgement is pinned to the policy version, hashed (sha256 of the exact content shown), and stamped with time, IP, and the attestation statement the person agreed to.</p>
          </>
        )}
      </div>
    </>
