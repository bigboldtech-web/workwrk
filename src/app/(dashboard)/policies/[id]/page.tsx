          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <select
                ...
                className="h-8 rounded-md border border-zinc-200 bg-white px-2 text-base text-zinc-700"
              >
                <option value="DRAFT">Draft</option>
                <option value="PUBLISHED">Published</option>
                <option value="ARCHIVED">Archived</option>
              </select>
              <label className="flex items-center gap-1.5 text-base text-zinc-500" title="Effective date (system field)">
              ...
              <input
                ...
                className="h-8 w-40 rounded-md border border-zinc-200 bg-white px-2 text-base text-zinc-700 outline-none focus:border-zinc-300"
              />
              ...
              {editStatus === "PUBLISHED" ? (
                <label className="flex items-center gap-1.5 text-base text-zinc-600" title="Material change — resets everyone to pending and requires them to re-acknowledge the new version. Leave off for typo/format fixes.">
              ...
              <button type="button" onClick={() => setEditing(false)} className="inline-flex h-8 items-center gap-1.5 rounded-md border border-zinc-200 px-2.5 text-base text-zinc-700 hover:bg-zinc-50">
          ...
          <>
            ...
            {policy.requiresAck ? (
              <div className="mt-4 rounded-xl border border-zinc-200 bg-white p-4">
                {policy.needsReack ? (
                  <div className="mb-3 flex items-center gap-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-base text-amber-800">
                ...
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  {policy.acknowledged ? (
                    <span className="inline-flex h-9 items-center gap-1.5 rounded-md bg-emerald-50 px-3 text-base font-medium text-emerald-700">
                      <CheckCircle2 className="h-4 w-4" /> You&apos;ve acknowledged v{policy.ackVersion ?? policy.version}
                    </span>
                  ) : (
                    <div className="flex flex-col items-end gap-2">
                      <label className="flex max-w-sm cursor-pointer items-start gap-2 text-base text-zinc-700">
            ...
            {showHistory ? (
              <div className="mt-4 rounded-xl border border-zinc-200 bg-white">
                <div className="flex items-center gap-2 border-b border-zinc-100 px-4 py-2.5 text-base font-medium text-zinc-700">
                  ...
                  <ul className="divide-y divide-zinc-100">
                    {versions.map((v) => (
                      <li key={v.id} className="flex items-center gap-3 px-4 py-2.5">
                        <span className="inline-flex h-6 min-w-[2.5rem] items-center justify-center rounded bg-zinc-100 px-1.5 text-sm font-semibold text-zinc-600">v{v.version}</span>
                        <div className="min-w-0 flex-1">
                          <div className="truncate text-base text-zinc-800">{v.title}</div>
                  ...
                  <ul className="divide-y divide-zinc-100">
                    {assignees.map((a) => (
                      <li key={a.id} className="flex items-center gap-3 px-4 py-2.5">
                        <div className="min-w-0 flex-1">
                          <div className="truncate text-base text-zinc-800">{a.name}</div>
                          {a.email ? <div className="truncate text-xs text-zinc-400">{a.email}</div> : null}
                        </div>
                        {a.status === "COMPLETED"
                          ? <span className="inline-flex items-center gap-1 text-sm font-medium text-emerald-600"><CheckCircle2 className="h-3.5 w-3.5" /> Acknowledged</span>
            ...
            {policy.content ? (
              <article
                className="prose prose-zinc mt-6 max-w-none prose-headings:font-semibold prose-h1:text-xl prose-h2:text-lg prose-h3:text-base prose-p:text-base prose-p:leading-relaxed prose-li:text-base"
                dangerouslySetInnerHTML={{ __html: safeHtml(policy.content) }}
              />
            ) : (
              <p className="mt-6 text-xs text-zinc-400">No content yet.{policy.canEdit ? " Click Edit to write the policy." : ""}</p>
            )}
          </>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/30 p-4" onClick={() => setShowAssign(false)}>
          <div className="flex max-h-[80vh] w-full max-w-md flex-col overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-zinc-100 px-4 py-3">
              <div className="text-xs font-semibold text-zinc-900">Assign policy</div>
            ...
            <div className="border-b border-zinc-100 px-4 py-2.5">
              <label className="flex cursor-pointer items-center gap-2 text-base text-zinc-700">
            ...
            {!assignAll ? (
              <>
                <div className="border-b border-zinc-100 px-4 py-2">
                  <input type="text" value={userQuery} onChange={(e) => setUserQuery(e.target.value)} placeholder="Search people…" className="h-8 w-full rounded-md border border-zinc-200 px-2.5 text-base outline-none focus:border-zinc-300" />
                </div>
                <div className="min-h-0 flex-1 overflow-y-auto">
                  {orgUsers === null ? (
                    ...
                    return list.length === 0 ? (
                      <div className="px-4 py-3 text-base text-zinc-400">No people match.</div>
                    ) : (
                      <ul>
                        {list.map((u) => {
                            ...
                            <li key={u.id}>
                              <label className="flex cursor-pointer items-center gap-2.5 px-4 py-2 hover:bg-zinc-50">
                                ...
                                <div className="min-w-0">
                                  <div className="truncate text-base text-zinc-800">{u.firstName} {u.lastName}</div>
            ...
            <div className="flex items-center gap-2 border-t border-zinc-100 px-4 py-2.5">
              <label htmlFor="assign-due" className="text-sm text-zinc-500">Acknowledge by</label>
              <input id="assign-due" type="date" value={assignDueDate} onChange={(e) => setAssignDueDate(e.target.value)} className="h-8 rounded-md border border-zinc-200 px-2 text-base text-zinc-700 outline-none focus:border-zinc-300" />
              {assignDueDate ? <button type="button" onClick={() => setAssignDueDate("")} className="text-sm text-zinc-400 hover:text-zinc-700">Clear</button> : <span className="text-sm text-zinc-300">optional</span>}
            </div>
