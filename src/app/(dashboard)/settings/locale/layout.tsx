...
// Locale & work week is a Workspace-settings page (settings-registry gate). A viewer
...
export default async function GatedSettingsLayout({ children }: { children: React.ReactNode }) {
  if (!(await isOrgAdminViewer())) {
    return <AdminOnly page="Locale & work week" back={{ fallbackHref: "/account/profile", label: SHELL_LABELS.mySettings }} />;
  }
  return <>{children}</>;
}
