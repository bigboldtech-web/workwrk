...
// API & webhooks is a Workspace-settings page (settings-registry gate). A viewer
...
export default async function GatedSettingsLayout({ children }: { children: React.ReactNode }) {
  if (!(await isOrgAdminViewer())) {
    return <AdminOnly page="API & webhooks" back={{ fallbackHref: "/account/profile", label: SHELL_LABELS.mySettings }} />;
  }
  return <>{children}</>;
}
