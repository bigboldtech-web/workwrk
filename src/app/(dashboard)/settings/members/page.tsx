      ...
      <header className="mb-1 flex items-center justify-between">
        ...
        {canEdit ? (
          <button
            ...
            className="inline-flex h-8 items-center gap-1.5 rounded-md bg-[var(--os-brand)] px-3 text-base font-medium text-white hover:bg-[var(--os-brand-hover)]"
          >
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto inline-flex h-8 items-center gap-2 rounded-lg border border-zinc-200 bg-white px-2.5">
          ...
          <input
            ...
            className="w-48 bg-transparent text-base text-zinc-800 outline-none placeholder:text-zinc-400"
          />
        </div>
      </div>
        ...
        <div className="overflow-x-auto rounded-xl border border-zinc-200 bg-white">
          <table className="w-full min-w-[720px] text-base">
            ...
            <tbody>
                  ...
                  <td className="px-3 py-2">
                    <div className="flex items-center gap-2.5">
                      {m.avatar ? (
                        ...
                        <span className="flex h-7 w-7 items-center justify-center rounded-full bg-zinc-200 text-xs font-semibold text-zinc-600">
                          {(m.firstName?.[0] ?? "?").toUpperCase()}
                        </span>
                      )}
                      <div className="min-w-0">
                        <div className="truncate font-medium text-zinc-900">{nameOf(m)}</div>
                        <div className="truncate text-sm text-zinc-500">{m.email ?? m.department?.name ?? ""}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-3 py-2">
                    <select
                      ...
                      className="h-8 rounded-md border border-zinc-200 bg-white px-2 text-base text-zinc-800 disabled:opacity-60"
                    >
                      {ACCESS_LEVELS.map((l) => (
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-3 py-6 text-center text-base text-zinc-400">
                    No people match “{q}”.
                  </td>
                </tr>
              ) : null}
                ...
                {invites.map((inv) => {
                  ...
                  return (
                    <tr key={inv.id} className="border-b border-zinc-100 last:border-0 hover:bg-zinc-50/60">
                      ...
                      <td className="px-3 py-2">
                        {expired ? (
                          <span className="inline-flex rounded-full bg-red-50 px-2 py-0.5 text-xs font-medium text-red-700 dark:bg-red-500/15 dark:text-red-400">
                            Expired
                          </span>
                        ) : (
