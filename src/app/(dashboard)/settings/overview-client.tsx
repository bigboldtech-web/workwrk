...
export function SettingsOverviewClient() {
    ...
    },
    {
      ...
      cards: [
        ...
        },
        {
          href: "/analytics",
          title: "AI usage",
          description: "Ask AI and agent queries this billing period.",
          Icon: Sparkles,
  ...
  return (
    <>
      ...
      {loadError ? (
        <OsEmptyView variant="error" title="Couldn't load settings" hint={`API error: ${loadError}.`} action={{ label: "Try again", onClick: () => { void load(); } }} />
      ) : data === null ? (
        <SkeletonRows />
      ) : (
                ...
                {section.cards.map((card) => (
                  <Link key={card.title + card.href} href={card.href} className="setcard">
                    <div className="setcard__head">
                      <span className="setcard__icon" style={{ background: "var(--os-surface-1)", color: "var(--os-ink-2)" }}>
                        <card.Icon />
                      </span>
