...
// Scoring & reviews is a Workspace-settings page (settings-registry gate). A viewer
...
export default async function GatedSettingsLayout({ children }: { children: React.ReactNode }) {
  if (!(await requireManagerTierViewer())) {
    return <AdminOnly page="Scoring & reviews" back={{ fallbackHref: "/account/profile", label: SHELL_LABELS.mySettings }} />;
  }
  return <>{children}</>;
}
