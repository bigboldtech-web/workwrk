...
const STATUS_HUE: Record<IdeaStatus, string> = {
  SUBMITTED: "var(--os-c-blue)", UNDER_REVIEW: "var(--os-c-orange)",
  APPROVED: "var(--os-c-blue)", IMPLEMENTED: "var(--os-c-green)",
  REJECTED: "var(--os-c-red)", REWARDED: "var(--os-c-yellow)",
};
...
export default function IdeasClient({ migratedListHref }: { migratedListHref: string | null }) {
    ...
    <>
      <OsPageHeader
        ...
        toolbar={{
          ...
          // Once the org's Ideas List exists, this page is a read-only
          // window on rows that now live there, so it stops offering a way
          // to write more into the retired model. The primary moves to the
          // List, where a new idea is a task like every other task.
          primary: migratedListHref ? undefined : { label: "Share an idea", onClick: () => setComposer(true) },
        }}
      />

      {/* Where this page is going, said plainly on the page itself, the same
          way /me/mentions says it. The rows below have already been copied
          onto the Ideas List by scripts/migrate-ideas.ts; they are kept
          readable here for one release rather than deleted, and the composer
          above is gone so nothing new is written into a model nothing reads.
          Without the marker there is no List yet (the migration has not run
          for this workspace), and the page is exactly what it was. */}
      {migratedListHref ? (
        <div className="os-row flex h-9 shrink-0 items-center gap-2 border-b border-line bg-subtle px-6 text-ink-2">
          Ideas have moved to a List.
          <Link href={migratedListHref} className="font-medium text-brand-deep hover:underline">
            Open the Ideas list
          </Link>
        </div>
      ) : null}
