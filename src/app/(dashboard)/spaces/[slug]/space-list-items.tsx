      ...
      <tr key={it.id} className="group hover:bg-zinc-50">
        <td className="px-3 py-2">
          <div className="flex items-center gap-1.5 min-w-0" style={{ paddingLeft: depth * 22 }}>
            ...
            >
              ...
              {kids.length > 0 ? (
                <span className="inline-flex items-center px-1.5 h-4 rounded-full bg-zinc-100 text-xs font-medium text-zinc-500 shrink-0">
                  {kids.length}
                </span>
              ) : null}
            </button>
        ...
        <td className="px-3 py-2 hidden sm:table-cell">
          <Link
            ...
            className="text-sm text-zinc-600 hover:text-zinc-900 truncate inline-block max-w-[200px]"
          >
            {it.board.name}
          </Link>
        </td>
