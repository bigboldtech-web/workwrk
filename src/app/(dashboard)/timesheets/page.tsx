...
function SheetDetailPanel({ detail }: { detail: SheetDetail | "loading" | undefined }) {
  if (!detail || detail === "loading") {
    return <div className="px-4 py-3 text-base text-zinc-400">Loading entries…</div>;
  }
  if (!detail.entries || detail.entries.length === 0) {
    return <div className="px-4 py-3 text-base text-zinc-400">No entries yet. Track time on a card and it lands here.</div>;
  }
        ...
        return (
          <div key={d} className="border-b border-zinc-100 last:border-b-0">
            <div className="flex items-center justify-between px-3 py-1.5 bg-white">
              ...
              <span className="text-xs font-semibold text-zinc-500 tabular-nums">{dayTotal.toFixed(2)}h</span>
            </div>
            <ul>
              {entries.map((e) => {
                const title = e.item?.title ?? e.task?.title ?? e.description ?? "Time entry";
                return (
                  <li key={e.id} className="flex items-center gap-2 px-3 py-1.5 text-base">
                    <span className="h-1.5 w-1.5 rounded-full bg-[#0073EA] shrink-0" />
                    <span className="text-zinc-700 truncate flex-1">{title}</span>
                    {e.source === "TIMER" ? <span className="text-xs text-zinc-400 shrink-0">from timer</span> : null}
