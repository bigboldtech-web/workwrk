    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/30 p-4" onClick={onClose}>
      <div className="w-full max-w-md overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-3 px-4 py-4">
          {tool.description ? <p className="text-base text-zinc-600">{tool.description}</p> : null}
          {tool.category ? <div className="text-sm text-zinc-400">Category · <span className="text-zinc-600">{tool.category}</span></div> : null}

          <div className="pt-1 text-xs font-semibold uppercase tracking-wide text-zinc-400">Credentials</div>
          ...
          {editing ? (
            <div className="space-y-2.5">
              ...
              <div>
                <div className="mb-1 text-xs font-medium uppercase tracking-wide text-zinc-400">Notes</div>
                <textarea value={draft.notes} onChange={(e) => setDraft((d) => ({ ...d, notes: e.target.value }))} rows={2} placeholder="2FA, seat owner, plan…" className="w-full rounded-md border border-zinc-200 px-2.5 py-1.5 text-base outline-none focus:border-zinc-300" />
              </div>
            ...
            <div className="space-y-2">
              ...
              {notes ? (
                <div className="rounded-lg border border-zinc-200 px-3 py-2">
                  <div className="mb-1 flex items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-zinc-400"><StickyNote className="h-3 w-3" /> Notes</div>
                  <p className="whitespace-pre-wrap text-base text-zinc-700">{notes}</p>
                </div>
              ) : null}
            </div>
          ) : (
        ...
        {tool.url ? (
          <div className="border-t border-zinc-100 px-4 py-3">
            <a href={tool.url} target="_blank" rel="noopener noreferrer" className="inline-flex h-9 w-full items-center justify-center gap-1.5 rounded-md bg-zinc-900 text-base font-medium text-white hover:bg-zinc-800">
...
function Field({ label, value, onChange, placeholder, type = "text" }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string; type?: string }) {
  return (
    <div>
      ...
      <input type={type} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="h-9 w-full rounded-md border border-zinc-200 px-2.5 text-base outline-none focus:border-zinc-300" />
    </div>
  );
}
...
function CredRow({ label, value, Icon, secret }: { label: string; value: string; Icon: typeof Lock; secret: boolean }) {
  ...
  return (
    <div className="rounded-lg border border-zinc-200 px-3 py-2">
      <div className="mb-1 flex items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-zinc-400"><Icon className="h-3 w-3" /> {label}</div>
      <div className="flex items-center gap-2">
        <span className="min-w-0 flex-1 truncate font-mono text-base text-zinc-800">{show ? value : "•".repeat(Math.min(value.length || 8, 16))}</span>
        {secret ? (
