      ...
      {menu ? (
        <>
          ...
          <div className="fixed z-[141] w-44 overflow-hidden rounded-lg border border-zinc-200 bg-white py-1 shadow-xl"
            ...
            <button type="button" onClick={() => { const s = menu.s; setMenu(null); window.location.href = `/sops/${s.id}`; }} className="flex w-full items-center gap-2 px-3 py-1.5 text-left text-base text-zinc-700 hover:bg-zinc-50"><FileText className="h-3.5 w-3.5" /> Open</button>
            <button type="button" onClick={() => { const s = menu.s; setMenu(null); void deleteSop(s); }} className="flex w-full items-center gap-2 px-3 py-1.5 text-left text-base text-red-600 hover:bg-red-50"><Trash2 className="h-3.5 w-3.5" /> Delete</button>
          </div>
        </>
      ) : null}
    </>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2">
        <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_PILL[s.status]}`}>
          <Icon className="h-3 w-3" /> {STATUS_LABEL[s.status]}
        </span>
        <span className="text-xs text-zinc-400">v{s.version}</span>
      </div>

      <h3 className="mt-2.5 line-clamp-1 text-base font-semibold text-zinc-900 group-hover:text-zinc-950">{s.title}</h3>
      {s.description && (
        <p className="mt-1 line-clamp-2 text-base leading-relaxed text-zinc-500">{s.description}</p>
      )}
      ...
      {s.tags && s.tags.length > 0 && (
        <div className="mt-2.5 flex flex-wrap gap-1">
          ...
          {s.tags.length > 3 && <span className="text-xs text-zinc-400">+{s.tags.length - 3}</span>}
        </div>
      )}
...
function SopRow({ s, onMenu }: { s: ApiSop; onMenu: (e: React.MouseEvent, s: ApiSop) => void }) {
  ...
  return (
    <Link href={`/sops/${s.id}`} onContextMenu={(e) => onMenu(e, s)} className="group flex items-center gap-3 px-4 py-2.5 transition-colors hover:bg-zinc-50">
      <span className={`inline-flex w-[92px] shrink-0 items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_PILL[s.status]}`}>
        <Icon className="h-3 w-3" /> {STATUS_LABEL[s.status]}
      </span>
      <span className="min-w-0 flex-1">
        <span className="block truncate text-base font-medium text-zinc-900">{s.title}</span>
        {s.description && <span className="block truncate text-sm text-zinc-400">{s.description}</span>}
      </span>
      {s._count?.steps != null && (
        <span className="hidden shrink-0 items-center gap-1 text-xs text-zinc-400 sm:inline-flex"><BookOpen className="h-3 w-3" /> {s._count.steps}</span>
      )}
      {s._count?.assignments != null && (
        <span className="hidden shrink-0 items-center gap-1 text-xs text-zinc-400 sm:inline-flex"><ClipboardCheck className="h-3 w-3" /> {s._count.assignments}</span>
      )}
      <span className="shrink-0 text-xs tabular-nums text-zinc-400">{fmtDate(s.updatedAt)}</span>
