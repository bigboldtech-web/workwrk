      ...
      <div className="px-6 py-6">
        <div className="max-w-3xl">
          <div className="mb-5 flex items-start gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold tracking-[-0.01em] text-zinc-900">Click-capture (Scribe-style)</h2>
          ...
          <div className="mb-5 rounded-xl border border-zinc-200 bg-white p-4">
            <div className="flex items-center justify-between gap-2">
              <div className="text-base font-medium text-zinc-900">Start recording</div>
              {installed && (
            ...
            <div className="mt-3 flex items-center gap-2">
              <input
                ...
                className="h-9 min-w-0 flex-1 rounded-md border border-zinc-200 px-3 text-base text-zinc-900 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-blue-100"
              />
              <button
                type="button"
                onClick={startRecording}
                disabled={status === "recording"}
                className="inline-flex h-9 shrink-0 items-center gap-1.5 rounded-md px-3 text-base font-medium disabled:opacity-60"
            ...
            {status === "recording" && (
              <p className="mt-2 flex items-start gap-1.5 text-base text-emerald-600">
                <CheckCircle2 className="mt-0.5 h-3.5 w-3.5 shrink-0" />
                Recording started. Every click is being captured. Open the extension popup and press Stop &amp; Save when you&apos;re done.
              </p>
            )}
            {status === "missing" && (
              <p className="mt-2 flex items-start gap-1.5 text-base text-amber-600">
            ...
            {STEPS.map((s, i) => {
              ...
              return (
                <li key={i} className="flex items-start gap-3 rounded-xl border border-zinc-200 bg-white p-4">
                  <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-zinc-100 text-sm font-semibold text-zinc-600">
                    {i + 1}
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5 text-base font-medium text-zinc-900">
                      <Icon className="h-4 w-4 text-zinc-400" /> {s.title}
                    </div>
                    <p className="mt-1 text-base leading-relaxed text-zinc-500">{s.body}</p>
                  </div>
                </li>
              );
            })}
          ...
          <div className="mt-4 flex items-start gap-2 rounded-lg border border-blue-100 bg-blue-50/50 px-4 py-2.5 text-base text-zinc-600">
          ...
          <div className="mt-5 flex items-center gap-2">
            <Link
              href="/sops"
              className="inline-flex h-9 items-center gap-1.5 rounded-md px-3 text-base font-medium"
              style={{ background: "var(--os-brand)", color: "#fff" }}
            >
              View recorded SOPs <ArrowRight className="h-3.5 w-3.5" />
            </Link>
            <Link href="/sops/new" className="inline-flex h-9 items-center gap-1.5 rounded-md border border-zinc-200 px-3 text-base text-zinc-700 hover:bg-zinc-50">
