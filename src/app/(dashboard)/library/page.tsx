        ...
        <div className="flex items-center gap-4">
          ...
          <div className="ml-auto relative w-64">
            ...
            <input
              ...
              className="w-full h-8 pl-8 pr-2 rounded-md border border-zinc-200 bg-white text-base focus:outline-none focus:border-zinc-400"
            />
          </div>
        </div>
      </div>
...
function TabPill({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-base font-medium transition-colors ${
        active
...
function NotesTab({ query, spaces }: { query: string; spaces: SpaceChip[] }) {
  ...
  return (
    <div>
      ...
      {rows === null ? (
        <div className="text-zinc-400 text-base">Loading notes…</div>
      ) : filtered.length === 0 ? (
        <EmptyTab
            ...
            <div key={d.id} className="relative group/note">
              ...
              >
                <div className="flex items-center gap-2 mb-2 pr-7">
                  ...
                  {d.entityType ? (
                    <span className="text-micro uppercase tracking-wide text-zinc-500 px-1.5 py-0.5 rounded bg-zinc-100">
                      {d.entityType.toLowerCase()}
                    </span>
                  ) : (
                    <span className="text-micro uppercase tracking-wide text-zinc-400">Standalone</span>
                  )}
                </div>
                <div className="text-base font-medium text-zinc-900 line-clamp-1">{d.title || "Untitled"}</div>
                {d.excerpt ? (
                  <div className="mt-1 text-sm text-zinc-500 line-clamp-2">{d.excerpt}</div>
                ) : null}
...
function WhiteboardsTab({ query, spaces }: { query: string; spaces: SpaceChip[] }) {
  ...
  return (
    <div>
      ...
      {rows === null ? (
        <div className="text-zinc-400 text-base">Loading canvases…</div>
      ) : filtered.length === 0 ? (
        <EmptyTab
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          {filtered.map((b) => (
            ...
            >
              ...
              <div className="p-3">
                <div className="text-base font-medium text-zinc-900 line-clamp-1">{b.name}</div>
                {b.description ? (
                  <div className="mt-1 text-sm text-zinc-500 line-clamp-1">{b.description}</div>
                ) : null}
                <div className="mt-2 flex items-center gap-1 text-xs text-zinc-400">
                  <Clock className="h-3 w-3" />
...
function FilesTab({ query, spaces }: { query: string; spaces: SpaceChip[] }) {
  ...
  return (
    ...
    >
      ...
      {rows === null ? (
        <div className="text-zinc-400 text-base">Loading files…</div>
      ) : filtered.length === 0 ? (
        <EmptyTab
                  ...
                  <div className="flex items-start gap-1.5">
                    <div className="flex-1 min-w-0">
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="block text-base font-medium text-zinc-900 truncate hover:text-zinc-700"
                        title={f.name}
                      >
                        {f.name}
                      </a>
...
function TablesTab({ query, spaces }: { query: string; spaces: SpaceChip[] }) {
  ...
  return (
    <div>
      ...
      {rows === null ? (
        <div className="text-zinc-400 text-base">Loading tables…</div>
      ) : filtered.length === 0 ? (
        <EmptyTab
            ...
            <div key={t.id} className="relative group/table">
              ...
              >
                <div className="flex items-center gap-2 mb-2 pr-7">
                  ...
                  <span className="text-micro uppercase tracking-wide text-zinc-400">
                    {t.columns.length} column{t.columns.length === 1 ? "" : "s"}
                  </span>
                </div>
                <div className="text-base font-medium text-zinc-900 line-clamp-1">{t.name}</div>
                {t.description ? (
                  <div className="mt-1 text-sm text-zinc-500 line-clamp-2">{t.description}</div>
                ) : null}
                <div className="mt-2 flex items-center gap-1 text-xs text-zinc-400">
                  <Clock className="h-3 w-3" />
                  {relTime(t.updatedAt)}
...
function EmptyTab({
  ...
  return (
    <div className="rounded-xl border border-dashed border-zinc-200 bg-white p-12 text-center">
      ...
      <div className="text-base text-zinc-500 max-w-sm mx-auto">{subtitle}</div>
    </div>
  );
}
...
function FileSpaceChip({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="text-xs px-2 py-1 rounded-full border transition inline-flex items-center gap-1.5"
      style={{
