      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex h-8 shrink-0 items-center gap-1 border-b border-zinc-200 !px-4">
          ...
          <div className="relative">
            <button
              type="button"
              className={`inline-flex h-[26px] items-center gap-1 rounded-md !px-2 text-sm font-medium ${
            ...
            {searchOpen ? (
              <div className="flex h-6 items-center gap-1 rounded-md border border-zinc-200 bg-white !px-1.5">
                ...
                <input
                  ...
                  className="h-full w-36 bg-transparent text-sm text-zinc-800 outline-none placeholder:text-zinc-400"
                  autoFocus
                />
                <button
                  type="button"
...
function ViewTabButton({ view, active, onClick }: { view: ViewDef; active: boolean; onClick: () => void }) {
  return (
    <button
      type="button"
      className={`relative inline-flex h-full items-center gap-1.5 !px-1.5 text-sm font-medium ${
...
function ViewCreatePanel({
  ...
  return (
    <>
      ...
      <div className="absolute left-0 top-8 z-[80] w-[360px] overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-2xl">
        ...
        <div className="relative">
          <input
            ...
            className="h-8 w-full rounded-md border border-[color-mix(in_srgb,var(--os-brand-rail)_35%,#e4e4e7)] bg-white !pl-3 !pr-8 text-sm outline-none focus:border-[var(--os-brand-rail)]"
            autoFocus
          />
          <button
            type="button"
      ...
      <div className="flex items-center gap-4 border-t border-zinc-100 !px-2.5 py-1.5 text-xs text-zinc-600">
...
function ViewCatalogItem({ view, onClick }: { view: ViewDef; onClick: () => void }) {
  return (
    <button
      type="button"
      className="flex h-8 items-center gap-2 rounded-md !px-2 text-left text-sm text-zinc-800 hover:bg-zinc-100"
      onClick={onClick}
    >
...
function ListMode({
  ...
  return (
    <div className="min-w-[860px] !px-4 py-3">
      ...
      {loading ? (
        <div className="mb-3 text-sm font-medium text-zinc-400">Loading tasks…</div>
      ) : null}
      {tasks.length > 0 ? (
      ...
      <div
        className="grid h-8 items-center border-b border-zinc-200 text-sm font-medium text-zinc-500"
            ...
            return (
              <section key={group.label} className="group/listgroup relative">
                <div className="mb-2 flex h-7 items-center gap-1.5">
                  <button
                    type="button"
                    className="inline-flex h-7 items-center gap-2 rounded-md text-left text-base font-medium text-zinc-700"
                ...
                {!collapsed ? (
                  <>
                    <div
                      className="grid border-b border-zinc-100 pb-2 text-sm font-medium text-zinc-400"
                      style={{ gridTemplateColumns: tableTemplate }}
                    >
                      <span>Name</span>
                      {tableColumns.map((column) => (
...
function CreateTaskRow({
  ...
  return (
    <div
      className="relative grid min-h-7 items-center border-b border-zinc-100 bg-white text-sm"
      style={{ gridTemplateColumns: tableTemplate }}
    >
      <span className="flex min-w-0 items-center gap-1.5 pr-2">
        ...
        <div className="relative flex shrink-0 items-center gap-0.5">
          ...
          <button
            type="button"
            className="ml-1 inline-flex h-6 items-center rounded-md !px-2 text-xs font-medium text-zinc-500 hover:bg-zinc-100 hover:text-zinc-800"
            onClick={onCancel}
          >
            Cancel
          </button>
          <button
            type="button"
            className="inline-flex h-6 items-center rounded-md bg-[var(--os-brand-rail)] !px-2 text-xs font-medium text-white disabled:opacity-60"
            onClick={() => {
              void onSave();
            }}
            disabled={saving}
...
function TaskRow({
  ...
  return (
    <div
      className="group grid min-h-7 items-center border-b border-zinc-100 text-sm text-zinc-700 hover:bg-zinc-50"
      style={{ gridTemplateColumns: tableTemplate }}
    >
      <span className={`flex min-w-0 items-center gap-1.5 ${task.parentId ? "pl-7" : ""}`}>
        ...
        {editing ? (
          <input
            ...
            className="min-w-0 flex-1 rounded-sm bg-white text-sm font-medium text-zinc-900 outline-none ring-1 ring-[var(--os-brand-rail)]"
            autoFocus
          />
        ) : (
          <span 
...
function TaskFieldCell({
  ...
  const buttonClass = "inline-flex h-6 max-w-full items-center gap-1.5 truncate rounded-md !px-1.5 text-left text-sm text-zinc-600 hover:bg-zinc-100 hover:text-zinc-900";
  ...
  if (column.key === "dateCreated") {
    return <span className="truncate pr-3 text-sm text-zinc-600">{task.dateCreated}</span>;
  }
  ...
  switch (column.key) {
    ...
    default:
      return (
        <span className="truncate pr-3 text-sm text-zinc-600">
...
function AddTaskRow({ tableTemplate, onClick }: { tableTemplate: string; onClick: () => void }) {
  return (
    <button
      type="button"
      className="grid min-h-7 w-full items-center border-b border-zinc-50 text-left text-sm text-zinc-400 hover:bg-zinc-50 hover:text-zinc-700"
      style={{ gridTemplateColumns: tableTemplate }}
      onClick={onClick}
    >
...
function ComposerButton({
  ...
  return (
    <button
      ...
      title={label}
      aria-label={label}
      className={`inline-flex h-[22px] items-center justify-center gap-1 rounded-md border border-zinc-200 !px-1.5 text-xs shadow-sm ${
      ...
      <DropdownPanel className="left-0 top-7 w-[170px] overflow-hidden !p-1.5">
        ...
        {STATUS_COLUMNS.map((status) => (
          <button
            key={status.key}
            type="button"
            className="flex h-7 w-full items-center gap-2 rounded-md !px-2 text-left text-sm text-zinc-700 hover:bg-zinc-50"
    ...
    <>
      ...
      <DropdownPanel className="left-0 top-7 w-[320px] overflow-hidden">
        ...
        <div className="!p-2.5">
          ...
          <div className="max-h-44 overflow-y-auto">
            {filteredOptions.map((option) => (
              <button
                key={option.id}
                type="button"
                className="mb-1 flex h-8 w-full items-center gap-2 rounded-md !px-2 text-left text-sm text-zinc-800 hover:bg-zinc-100"
                onClick={() => onSelect(option)}
              >
                <span className="flex h-6 w-6 items-center justify-center rounded-full bg-[var(--os-brand-rail)] text-xs font-semibold text-white">
                  {option.initials}
                </span>
                <span className="min-w-0">
                  <span className="block truncate">{option.name}</span>
                  {option.email ? <span className="block truncate text-xs text-zinc-400">{option.email}</span> : null}
                </span>
              </button>
            ))}
            {filteredOptions.length === 0 ? (
              <div className="rounded-md bg-zinc-50 !px-2 py-2 text-sm text-zinc-500">
          ...
          <button
            type="button"
            className="flex h-8 w-full cursor-not-allowed items-center gap-2 rounded-md !px-2 text-left text-sm text-zinc-400"
            disabled
          >
          ...
          <button
            type="button"
            className="flex h-7 w-full cursor-not-allowed items-center gap-2 rounded-md !px-2 text-left text-sm font-medium text-zinc-400"
            disabled
          >
    ...
    <>
      ...
      <DropdownPanel className="right-0 top-7 w-[500px] max-w-[calc(100vw-420px)] overflow-hidden">
        <div className="grid grid-cols-2 border-b border-zinc-100 !p-1.5">
          <button type="button" className="flex h-7 items-center gap-2 rounded-md bg-zinc-100 !px-2.5 text-sm text-zinc-500">
            <CalendarDays className="h-3.5 w-3.5" />
            Start date
          </button>
          <button type="button" className="ml-1.5 flex h-7 items-center gap-2 rounded-md border border-[var(--os-brand-rail)] bg-white !px-2.5 text-sm text-zinc-700">
        ...
        <div className="grid grid-cols-[1fr_1.1fr]">
          <div className="border-r border-zinc-100 py-1.5">
            {quick.map((option) => (
              <button
                key={option.label}
                type="button"
                className="flex h-7 w-full items-center justify-between !px-3 text-left text-sm text-zinc-800 hover:bg-zinc-50"
            ...
            <button type="button" className="mt-1.5 flex h-8 w-full items-center justify-between border-t border-zinc-100 !px-3 text-sm text-zinc-800">
              Set Recurring
          ...
          <div className="!p-3">
            <div className="mb-3 flex items-center justify-between">
              <span className="text-base font-medium text-zinc-900">{monthLabel}</span>
              <span className="inline-flex items-center gap-3 text-sm text-zinc-600">
                Today
        ...
        <div className="flex items-center gap-2.5 border-t border-zinc-100 bg-zinc-50 !px-3 py-2">
          ...
          <div>
            <p className="text-sm font-semibold text-zinc-900">Set up your work schedule</p>
            <p className="text-xs text-zinc-500">Set working days/hours and holidays for your workspace.</p>
          </div>
  ...
  return (
    <>
      ...
      <DropdownPanel className="right-0 top-7 w-[200px] overflow-hidden">
        <div className="!p-2">
          ...
          {options.map((option) => (
            <button
              key={option.label}
              type="button"
              className="flex h-7 w-full items-center gap-2.5 rounded-md !px-2 text-left text-sm text-zinc-800 hover:bg-zinc-50"
              onClick={() => onSelect(option.label)}
            >
        ...
        <button
          type="button"
          className="flex h-8 w-full items-center gap-2.5 border-t border-zinc-100 !px-4 text-left text-sm text-zinc-800 hover:bg-zinc-50"
          onClick={() => onSelect("")}
        >
  ...
  return (
    <>
      ...
      <input
        ...
        className="h-9 w-full border-b border-zinc-100 !px-2.5 text-sm outline-none"
      ...
      <div className="!p-2.5">
        ...
        <button
          type="button"
          className="flex h-7 w-full items-center gap-2 rounded-md bg-zinc-100 !px-2 text-left text-sm text-zinc-600"
          onClick={onCreate}
        >
          Create
          {query.trim() ? (
            <span className="rounded-full bg-blue-100 !px-2 py-0.5 text-sm font-medium text-blue-700">{query.trim()}</span>
          ) : null}
  ...
  return (
    <div className="min-w-[860px] !px-4 py-3">
      <div className="mb-3 flex items-center justify-between">
        <button
          type="button"
          className="inline-flex h-6 items-center gap-1.5 rounded-full border border-[color-mix(in_srgb,var(--os-brand-rail)_18%,transparent)] bg-[color-mix(in_srgb,var(--os-brand-rail)_9%,white)] !px-2 text-xs font-medium text-[var(--os-brand-ink)]"
        >
        ...
        <button
          type="button"
          className="inline-flex h-6 items-center gap-1.5 rounded-md border border-zinc-200 !px-2 text-xs text-zinc-600 hover:bg-zinc-50"
          onClick={onCustomize}
        >
      ...
      <div className="flex gap-1.5">
          ...
          return (
            ...
            >
              <div className="mb-1.5 flex items-center justify-between">
                <span className="inline-flex items-center gap-1.5 rounded-md !px-1.5 py-0.5 text-xs font-semibold text-white" style={{ backgroundColor: status.color }}>
              ...
              <div className="space-y-1.5">
                ...
                <button
                  type="button"
                  className="flex h-7 w-full items-center gap-2 rounded-md !px-2 text-left text-sm text-zinc-600 hover:bg-white/70"
        ...
        <button
          type="button"
          className="flex h-7 shrink-0 items-center gap-2 rounded-md !px-2.5 text-sm text-zinc-500 hover:bg-zinc-100"
          onClick={onCustomize}
        >
          <Plus className="h-3.5 w-3.5" />
          Add group
...
function BoardTaskCard({
  ...
  return (
    <div
      ...
      tabIndex={0}
      draggable
      className="group/card w-full cursor-grab rounded-md border border-zinc-200 bg-white !p-2 text-left text-sm shadow-sm transition  hover:border-zinc-300 hover:shadow-md active:cursor-grabbing"
      onClick={(event) => {
...
function BoardDraftCard({
  ...
  return (
    <div className="rounded-md border border-[color-mix(in_srgb,var(--os-brand-rail)_28%,#e4e4e7)] bg-white !p-2 text-sm shadow-sm">
      <input
        value={draftTask.name}
        onChange={(event) => onDraftChange({ name: event.target.value })}
        onKeyDown={(event) => {
        ...
        className="h-6 w-full bg-transparent text-sm text-zinc-900 outline-none placeholder:text-zinc-400"
        autoFocus
      />
      <div className="mt-2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <button
            type="button"
            className="h-6 rounded-md !px-2 text-xs font-medium text-zinc-500 hover:bg-zinc-100"
            onClick={onCancel}
          >
            Cancel
          </button>
          <button
            type="button"
            className="h-6 rounded-md bg-[var(--os-brand-rail)] !px-2 text-xs font-medium text-white disabled:opacity-60"
            disabled={saving}
            onClick={() => {
              void onSave();
            }}
...
function CalendarMode({
  ...
  return (
    <div className="min-w-[860px] !p-4">
      <div className="mb-3 inline-flex h-6 items-center gap-1.5 rounded-full border border-orange-100 bg-orange-50 !px-2 text-xs font-medium text-orange-700">
            ...
            <div className="space-y-1.5">
              {group.items.map((task) => (
                <button
                  key={task.id}
                  type="button"
                  className="w-full rounded-md border border-zinc-200 bg-white !p-2 text-left text-sm shadow-sm hover:border-zinc-300 hover:shadow-md"
                  onClick={() => onOpenTask(task)}
                >
                  <p className="truncate font-medium text-zinc-900">{task.name}</p>
                  <p className="mt-1 truncate text-xs text-zinc-500">{task.assignee || "Unassigned"}</p>
                </button>
              ))}
              {group.items.length === 0 ? <p className="py-2 text-xs text-zinc-400">No tasks</p> : null}
            </div>
          </section>
        ))}
      </div>
...
function GanttMode({
  ...
  return (
    <div className="min-w-[920px] !p-4">
      <div className="mb-3 inline-flex h-6 items-center gap-1.5 rounded-full border border-red-100 bg-red-50 !px-2 text-xs font-medium text-red-700">
        <activeView.Icon className="h-3.5 w-3.5" />
        {activeView.label}
      </div>
      <div className="overflow-hidden rounded-lg border border-zinc-200 bg-white">
        <div className="grid grid-cols-[260px_repeat(6,minmax(100px,1fr))] border-b border-zinc-100 bg-zinc-50 text-xs font-medium text-zinc-500">
        ...
        {lanes.map((task, index) => {
          ...
          return (
            <button
              key={task.id}
              type="button"
              className="grid min-h-9 w-full grid-cols-[260px_repeat(6,minmax(100px,1fr))] items-center border-b border-zinc-100 text-left text-sm hover:bg-zinc-50"
              onClick={() => onOpenTask(task)}
            >
              ...
              <span
                className="h-5 rounded-full !px-2 text-xs font-medium leading-5 text-white"
                style={{
...
function DocMode({ tasks, activeView }: { tasks: TaskItem[]; activeView: ViewDef }) {
  ...
  return (
    <div className="min-w-[860px] !p-4">
      <div className="mb-3 inline-flex h-6 items-center gap-1.5 rounded-full border border-blue-100 bg-blue-50 !px-2 text-xs font-medium text-blue-700">
        <activeView.Icon className="h-3.5 w-3.5" />
        {activeView.label}
      </div>
      <div className="grid grid-cols-[minmax(0,1fr)_260px] gap-3">
        <textarea
          ...
          className="min-h-[360px] resize-none rounded-xl border border-zinc-200 bg-white !p-4 text-base leading-6 text-zinc-800 outline-none focus:border-[var(--os-brand-rail)]"
        />
        <aside className="rounded-xl border border-zinc-200 bg-white !p-3">
          <p className="mb-2 text-sm font-semibold text-zinc-900">Linked tasks</p>
          <div className="space-y-1">
            {tasks.slice(0, 8).map((task) => (
              <div key={task.id} className="flex items-center gap-2 rounded-md !px-2 py-1.5 text-sm text-zinc-700">
                <CircleDashed className="h-3.5 w-3.5 text-zinc-400" />
                <span className="truncate">{task.name}</span>
              </div>
            ))}
            {tasks.length === 0 ? <p className="text-sm text-zinc-400">No linked tasks yet.</p> : null}
          </div>
        </aside>
      </div>
    </div>
    ...
    <div className="min-w-[860px] !p-4">
      ...
      >
        <div className="border-b border-zinc-100 !px-4 py-3">
          <h2 className="text-base font-semibold text-zinc-900">Task intake form</h2>
          <p className="mt-1 text-sm text-zinc-500">Create list tasks without leaving the current view.</p>
        </div>
        <div className="grid gap-3 !p-4 text-sm">
          <label className="grid gap-1.5">
            ...
            <input
              ...
              className="h-9 rounded-lg border border-zinc-200 !px-3 text-base outline-none focus:border-[var(--os-brand-rail)]"
              autoFocus
            />
          </label>
          <div className="grid grid-cols-2 gap-2">
            <div className="relative">
              <button
                type="button"
                className="flex h-9 w-full items-center justify-between rounded-lg border border-zinc-200 !px-3 text-left text-sm text-zinc-700 hover:bg-zinc-50"
                onClick={() => setOpenMenu(openMenu === "assignee" ? null : "assignee")}
              >
        ...
        <div className="flex items-center justify-end gap-2 border-t border-zinc-100 !px-4 py-3">
          <button
            type="button"
            className="h-8 rounded-md !px-3 text-sm text-zinc-500 hover:bg-zinc-100"
            onClick={() => {
              setTitle("");
              setAssignee("");
              setDueDate("");
          ...
          <button
            ...
            className="h-8 rounded-md bg-[var(--os-brand-rail)] !px-3 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-50"
          >
  ...
  return (
    <div className="min-w-[860px] !p-4">
      <div className="mx-auto mt-10 flex max-w-[460px] flex-col items-center rounded-xl border border-zinc-200 bg-white !p-8 text-center">
        ...
        <h2 className="mb-1.5 text-base font-semibold text-zinc-900">{activeView.label} view</h2>
...
function TaskDetailModal({
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/45 !p-8" onClick={onClose}>
      ...
      >
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="flex h-10 shrink-0 items-center justify-between border-b border-zinc-200 !px-3 text-sm text-zinc-500">
          ...
          <div className="min-h-0 flex-1 overflow-auto !px-10 py-7">
            <div className="mb-4 inline-flex h-6 items-center gap-1.5 rounded-md border border-zinc-200 bg-zinc-50 !px-2 text-xs text-zinc-700">
              <CircleDashed className="h-3.5 w-3.5" />
              <button
                type="button"
            ...
            <h2 className="mb-4 text-xl font-semibold leading-tight text-zinc-950">{task.name}</h2>
            <div className="mb-5 rounded-lg bg-zinc-50 !px-3 py-2 text-sm text-zinc-600">
            ...
            <div className="grid max-w-[760px] grid-cols-2 gap-x-12 gap-y-3 text-sm">
              <TaskDetailField Icon={CheckCircle2} label="Status">
                <div className="relative flex items-center gap-1.5">
                  <button
                    type="button"
                    className="inline-flex h-6 items-center gap-1 rounded-md !px-2 text-xs font-semibold text-white"
                      ...
                      <div className="absolute left-0 top-full z-20 mt-1 w-40 overflow-hidden rounded-lg border border-zinc-200 bg-white py-1 shadow-lg">
                        {STATUS_COLUMNS.map((status) => (
                          <button
                            key={status.key}
                            type="button"
                            className="flex h-8 w-full items-center gap-2 !px-2 text-left text-sm text-zinc-700 hover:bg-zinc-50"
                            onClick={() => {
                              setStatusOpen(false);
            ...
            <textarea
              ...
              className="mb-12 min-h-[72px] w-full max-w-[760px] resize-y rounded-lg border border-transparent bg-white !p-2 text-base leading-5 text-zinc-700 outline-none placeholder:text-zinc-400 hover:border-zinc-100 focus:border-[var(--os-brand-rail)]"
            />

            <div className="grid max-w-[360px] gap-3 text-sm text-zinc-700">
              {[
...
function ActivityPanel({ task }: { task: TaskItem }) {
  ...
  return (
    <aside className="flex w-[360px] shrink-0 flex-col border-l border-zinc-200 bg-zinc-50/40">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto !p-3 text-xs text-zinc-500">
        <div className="mb-3 flex">
          <span>You created this task</span>
          <span className="ml-auto">{task.dateCreated}</span>
        </div>
        {loading ? <div className="text-xs text-zinc-400">Loading activity…</div> : null}
        {error ? <div className="mb-2 rounded-md bg-red-50 !px-2 py-1 text-xs text-red-600">{error}</div> : null}
        <div className="space-y-2">
          {comments.map((comment) => (
            <div key={comment.id} className="rounded-lg border border-zinc-200 bg-white !p-2 text-sm text-zinc-700 shadow-sm">
      ...
      <div className="border-t border-zinc-200 !p-3">
        <div className="rounded-lg border border-zinc-200 bg-white !p-2 shadow-sm">
          <input
            ...
            className="mb-2 h-8 w-full bg-transparent text-sm outline-none placeholder:text-zinc-400"
          />
          <div className="flex items-center gap-1 text-zinc-400">
            <Plus className="h-4 w-4" />
            <span className="rounded-md bg-zinc-100 !px-2 py-1 text-xs text-zinc-600">Comment</span>
            <Bot className="h-4 w-4 text-[#0073EA]" />
            <Paperclip className="h-4 w-4" />
            <button
              type="button"
              className="ml-auto rounded-md bg-zinc-100 !px-2 py-1 text-xs text-zinc-500 hover:bg-zinc-200 disabled:opacity-50"
...
function RelatedItemsPanel({
  ...
  return (
    <aside className="flex w-[360px] shrink-0 flex-col border-l border-zinc-200 bg-zinc-50/40">
      ...
      <div className="border-b border-zinc-100 !p-2">
        <div className="grid grid-cols-2 rounded-md bg-zinc-100 !p-0.5 text-xs text-zinc-500">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto !p-3">
        {items.length === 0 && !adding ? (
          <div className="flex min-h-[280px] items-center justify-center text-center">
            <div className="max-w-[220px]">
              ...
              <p className="text-sm font-semibold text-zinc-700">No related items</p>
              <p className="mt-1 text-xs leading-4 text-zinc-400">Link related Tasks or Docs to organize and quickly access them here.</p>
              <button
                type="button"
                className="mt-3 h-7 rounded-md border border-zinc-200 bg-white !px-2.5 text-xs text-zinc-600 shadow-sm hover:bg-zinc-50"
        ...
        {items.length > 0 ? (
          <div className="space-y-1.5">
            {items.map((item) => (
              <div key={item} className="flex h-8 items-center gap-2 rounded-md border border-zinc-200 bg-white !px-2 text-sm text-zinc-700">
        ...
        {adding ? (
          <div className="rounded-lg border border-zinc-200 bg-white !p-2 shadow-sm">
            ...
            <div className="mt-1 flex justify-end gap-1">
              <button type="button" className="h-7 rounded-md !px-2 text-xs text-zinc-500 hover:bg-zinc-100" onClick={() => setAdding(false)}>
                Cancel
              </button>
              <button type="button" className="h-7 rounded-md bg-[var(--os-brand-rail)] !px-2 text-xs font-medium text-white disabled:opacity-50" disabled={!draft.trim()} onClick={addItem}>
                Relate
              </button>
            </div>
          </div>
...
function TaskFieldsPanel({
  ...
  return (
    <aside className="flex w-[360px] shrink-0 flex-col border-l border-zinc-200 bg-zinc-50/40">
      <div className="flex h-10 items-center justify-between border-b border-zinc-200 !px-3">
        <h3 className="text-base font-semibold text-zinc-900">Fields</h3>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto !p-3 text-sm">
        <label className="block">
          ...
          <textarea
            ...
            className="min-h-[104px] w-full resize-y rounded-lg border border-zinc-200 bg-white !p-2 text-sm leading-5 outline-none placeholder:text-zinc-400 focus:border-[var(--os-brand-rail)]"
          />
        </label>
            ...
            <div className="space-y-2">
              {customEntries.map(([key, value]) => (
                <label key={key} className="block">
                  <span className="mb-1 block truncate text-xs font-medium text-zinc-500">{key}</span>
                  <input
                    defaultValue={value}
                    onBlur={(event) => updateField(key, event.currentTarget.value.trim())}
                    className="h-8 w-full rounded-md border border-zinc-200 bg-white !px-2 text-sm outline-none focus:border-[var(--os-brand-rail)]"
                  />
                </label>
              ))}
            </div>
          ) : (
            <p className="rounded-lg border border-dashed border-zinc-200 bg-white !p-3 text-xs leading-4 text-zinc-500">
...
function ChecklistPanel({
  ...
  return (
    <aside className="flex w-[360px] shrink-0 flex-col border-l border-zinc-200 bg-zinc-50/40">
      <div className="flex h-10 items-center justify-between border-b border-zinc-200 !px-3">
        <h3 className="text-base font-semibold text-zinc-900">Checklist</h3>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto !p-3 text-sm">
        <div className="flex h-8 items-center gap-2 rounded-md border border-zinc-200 bg-white !px-2">
          ...
          <button
            type="button"
            className="rounded bg-zinc-100 !px-1.5 py-0.5 text-xs text-zinc-600 hover:bg-zinc-200 disabled:opacity-40"
            disabled={!draft.trim()}
            onClick={addItem}
          >
            Add
  ...
  return (
    <aside className="flex w-[360px] shrink-0 flex-col border-l border-zinc-200 bg-zinc-50/40">
      <div className="flex h-10 items-center border-b border-zinc-200 !px-3">
        <h3 className="text-base font-semibold text-zinc-900">Add a link to this task</h3>
      </div>
      <div className="!p-3 text-sm">
        <p className="mb-1.5 text-xs font-medium text-zinc-500">Add a link</p>
        <div className="flex h-8 items-center gap-2 rounded-md border border-zinc-200 bg-white !px-2">
        ...
        {links.length > 0 ? (
          <div className="mt-3 space-y-1.5">
            {links.map((link) => (
              <div key={link} className="flex h-8 items-center gap-2 rounded-md border border-zinc-200 bg-white !px-2 text-sm text-zinc-700">
        ...
        <p className="mb-2 mt-5 text-xs font-medium text-zinc-500">Or relate items</p>
        <div className="space-y-1">
...
function LinkPanelItem({ Icon, label }: { Icon: LucideIcon; label: string }) {
  return (
    <button type="button" className="flex h-7 w-full items-center gap-2 rounded-md !px-1 text-left text-sm text-zinc-700 hover:bg-zinc-100">
    ...
    <aside className="flex w-[300px] shrink-0 flex-col border-l border-zinc-200 bg-white">
      ...
      <div className="border-b border-zinc-100 !p-2">
        <input
          ...
          className="h-8 w-full rounded-md border border-[color-mix(in_srgb,var(--os-brand-rail)_35%,#e4e4e7)] bg-white !px-2 text-sm outline-none focus:border-[var(--os-brand-rail)]"
        />
      ...
      <div className="absolute left-0 top-7 z-[70] w-[300px] rounded-xl border border-zinc-200 bg-white !p-3 shadow-2xl">
        ...
        <div className="mt-1 flex items-center gap-2">
          ...
          <div className="relative flex-1">
            <button
              type="button"
              className="flex h-8 w-full items-center gap-2 rounded-md border border-zinc-200 !px-2 text-left text-base text-zinc-800 hover:bg-zinc-50"
              ...
              <div className="absolute left-0 top-9 z-[72] w-full overflow-hidden rounded-lg border border-zinc-200 bg-white py-1 shadow-xl">
                {GROUP_OPTIONS.filter((o) => o.key !== "none").map((option) => (
                  <button
                    key={option.key}
                    type="button"
                    className={`flex h-8 w-full items-center gap-2 !px-2 text-left text-base ${
          ...
          <div className="relative w-[120px]">
            <button
              type="button"
              className="flex h-8 w-full items-center gap-1 rounded-md border border-zinc-200 !px-2 text-left text-base text-zinc-800 hover:bg-zinc-50"
        ...
        <div className="mt-2 flex items-center justify-between border-t border-zinc-100 pt-2">
          <span className="text-base text-zinc-700">Also group by List</span>
  ...
  return (
    <>
      ...
      <div className="absolute left-0 top-7 z-[80] w-[260px] overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-2xl">
        <div className="!px-4 py-3 text-sm font-medium text-zinc-500">Group options</div>
...
function GroupOptionsItem({
  ...
  return (
    <button
      type="button"
      className="flex h-8 w-full items-center gap-2 rounded-md !px-2 text-left text-base text-zinc-800 hover:bg-zinc-50"
      onClick={onClick}
    >
    ...
    <>
      ...
      <div className="absolute right-0 top-7 z-[80] w-[480px] rounded-xl border border-zinc-200 bg-white !p-3 shadow-2xl">
        <div className="mb-2 flex items-center justify-between">
          <span className="inline-flex items-center gap-1 text-base font-semibold text-zinc-800">
            Filters <Info className="h-3.5 w-3.5 text-zinc-400" />
          </span>
          <div className="relative">
            <button
              type="button"
              className="inline-flex h-7 items-center gap-1 rounded-md border border-zinc-200 !px-2 text-sm text-zinc-600 hover:bg-zinc-50"
            ...
            {savedOpen ? (
              <div className="absolute right-0 top-9 z-[82] w-[260px] rounded-lg border border-zinc-200 bg-white !p-2 shadow-xl">
                {saved.length === 0 ? (
                  <p className="!px-1 py-1 text-sm text-zinc-400">No saved filters yet.</p>
                ) : (
                  <ul className="mb-2 space-y-0.5">
                    {saved.map((s) => (
                      <li key={s.name} className="group flex items-center gap-2 rounded-md !px-2 py-1 hover:bg-zinc-50">
                        <button type="button" className="flex-1 truncate text-left text-base text-zinc-700" onClick={() => applySaved(s)}>
                          {s.name}
                        </button>
                ...
                <div className="flex items-center gap-1.5 border-t border-zinc-100 pt-2">
                  <input
                    ...
                    className="h-7 flex-1 rounded-md border border-zinc-200 !px-2 text-sm outline-none"
                  />
                  <button
                    type="button"
                    onClick={saveCurrent}
                    disabled={!saveName.trim() || filters.length === 0}
                    className="h-7 rounded-md bg-zinc-900 !px-2 text-sm font-medium text-white disabled:opacity-40"
                  >
                    Save
                  </button>
                </div>
        ...
        {filters.length === 0 ? (
          <p className="!px-1 py-2 text-sm text-zinc-400">No filters yet. Add one to narrow this view.</p>
        ) : (
          <div className="space-y-2">
        ...
        <div className="mt-2 flex items-center justify-between">
          <button type="button" className="inline-flex items-center gap-1 text-sm font-medium text-[var(--os-brand-ink)] hover:opacity-80" onClick={addFilter}>
            <Plus className="h-3.5 w-3.5" /> Add filter
          </button>
          {filters.length > 0 ? (
            <button type="button" className="text-sm font-medium text-red-500 hover:text-red-600" onClick={() => onFiltersChange([])}>
              Clear all
            </button>
          ) : null}
        </div>
          ...
          <div className="flex flex-wrap gap-1">
            {SORT_OPTIONS.map((option) => (
              <button
                key={option.key}
                type="button"
                className={`inline-flex h-7 items-center gap-1 rounded-md !px-2 text-sm ${
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="w-16 shrink-0 text-sm text-zinc-500">
        {index === 0 ? (
          ...
          <select
            ...
            className="h-7 w-full rounded-md border border-zinc-200 bg-white text-sm text-zinc-700"
          >
      ...
      <select
        ...
        className="h-7 flex-1 rounded-md border border-zinc-200 bg-white !px-1.5 text-sm text-zinc-800"
      >
      ...
      <select
        ...
        className="h-7 w-24 shrink-0 rounded-md border border-zinc-200 bg-white !px-1.5 text-sm text-zinc-800"
      >
        {meta.operators.map((op) => <option key={op} value={op}>{FILTER_OPERATOR_LABELS[op]}</option>)}
      </select>
      {needsValue ? (
        meta.valueKind === "status" ? (
          <select value={rule.value} onChange={(e) => onChange({ value: e.target.value })} className="h-7 flex-1 rounded-md border border-zinc-200 bg-white !px-1.5 text-sm">
          ...
          <input value={rule.value} onChange={(e) => onChange({ value: e.target.value })} placeholder="Value" className="h-7 flex-1 rounded-md border border-zinc-200 bg-white !px-2 text-sm outline-none" />
        )
      ) : (
  ...
  return (
    <>
      ...
      <input
        ...
        className="h-8 w-full border-b border-zinc-100 !px-2 text-sm outline-none"
        placeholder="Search or create tag"
        autoFocus
      />
      <div className="!p-1.5">
        ...
        <button
          type="button"
          className="flex h-7 w-full items-center gap-2 rounded-md !px-2 text-left text-sm text-zinc-700 hover:bg-zinc-50 disabled:text-zinc-400"
          disabled={!query.trim()}
          onClick={onCreate}
        >
...
function CustomFieldPopover({
  ...
  return (
    ...
    >
      <div className="border-b border-zinc-100 !px-2.5 py-2 text-xs font-medium text-zinc-500">
        {label}
      </div>
      <div className="!p-2">
        <input
          ...
          className="h-8 w-full rounded-md border border-zinc-200 !px-2 text-sm outline-none focus:border-[var(--os-brand-rail)]"
          autoFocus
        />
        <div className="mt-2 flex items-center justify-end gap-1.5">
          <button
            type="button"
            className="h-7 rounded-md !px-2 text-xs text-zinc-500 hover:bg-zinc-100"
            onClick={onClear}
          >
            Clear
          </button>
          <button
            type="submit"
            className="h-7 rounded-md bg-[var(--os-brand-rail)] !px-2.5 text-xs font-medium text-white"
          >
            Save
          </button>
        </div>
...
function FieldToggle({ column, checked, onToggle }: { column: ColumnDef; checked: boolean; onToggle: () => void }) {
  return (
    <button
      type="button"
      className="flex h-7 w-full items-center gap-2 rounded-md !px-1.5 text-left text-sm text-zinc-700 hover:bg-zinc-50"
      onClick={onToggle}
      disabled={column.key === "name"}
    >
      <column.Icon className="h-3 w-3 text-zinc-500" />
      <span className="min-w-0 flex-1 truncate">{column.label}</span>
      {column.key === "taskType" ? <span className="rounded bg-fuchsia-100 !px-1 text-xs font-medium text-fuchsia-700">New</span> : null}
...
function CreateFieldRow({
  ...
  return (
    <button
      type="button"
      className="group flex h-7 w-full items-center gap-2 rounded-md !px-1.5 text-left text-sm text-zinc-700 hover:bg-zinc-50"
      onClick={onCreate}
    >
      <field.Icon className="h-3 w-3" style={{ color: field.color }} />
      <span className="min-w-0 flex-1 truncate">{field.label}</span>
      <span className="hidden text-xs text-zinc-500 group-hover:inline">Create</span>
    </button>
  );
}
...
function SettingRow({ label, value }: { label: string; value?: string }) {
  return (
    <div className="flex h-6 items-center justify-between text-sm text-zinc-700">
      <span>{label}</span>
...
function SettingToggle({ label, checked, onToggle }: { label: string; checked: boolean; onToggle?: () => void }) {
  return (
    <button
      type="button"
      className="flex h-6 w-full items-center justify-between rounded-md text-left text-sm text-zinc-700 hover:bg-zinc-50"
      onClick={onToggle}
    >
      <span>{label}</span>
...
function GroupBadge({ label }: { label: string }) {
  ...
  if (status) {
    return (
      <span
        className="inline-flex h-6 items-center rounded-md !px-2 text-sm font-semibold text-white"
  ...
  if (color) {
    return <span className="text-base font-semibold" style={{ color }}>{label}</span>;
  }
  return <span className="text-base font-semibold text-zinc-700">{label}</span>;
}
  ...
  return (
    <div className="fixed bottom-8 left-1/2 z-[100] -translate-x-1/2">
      ...
      <div className="relative z-[1] flex h-12 items-center gap-1.5 rounded-lg border border-zinc-700 bg-zinc-900 !px-2.5 text-white shadow-2xl">
        <div className="flex h-8 items-center rounded-md bg-zinc-800 !px-3 text-base font-medium text-white shadow-inner">
          ...
          {menu === "assignee" ? (
            <BarPopover className="max-h-[280px] overflow-y-auto">
              <BarRow onClick={() => { onAssignee("", "Unassigned"); close(); }}>
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-zinc-200 text-micro text-zinc-500">?</span>
                Unassigned
              </BarRow>
              {assigneeOptions.map((a) => (
                <BarRow key={a.id} onClick={() => { onAssignee(a.id, a.name); close(); }}>
                  <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[var(--os-brand-rail)] text-micro font-medium text-white">{a.initials}</span>
            ...
            <BarPopover>
              ...
              <div className="mt-1 border-t border-zinc-100 !px-2 pb-1 pt-2">
                <input
                  ...
                  className="h-8 w-full rounded-md border border-zinc-200 !px-2 text-sm text-zinc-700 outline-none focus:border-[var(--os-brand-rail)]"
...
function BarRow({ children, onClick }: { children: ReactNode; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex h-8 w-full items-center gap-2 rounded-md !px-2 text-left text-sm text-zinc-700 hover:bg-zinc-100"
    >
      {children}
    </button>
  );
...
function ActionPill({ Icon, label, onClick, active }: { Icon: LucideIcon; label: string; onClick?: () => void; active?: boolean }) {
  return (
    <button
      type="button"
      className={`flex h-8 items-center gap-1.5 rounded-md !px-2 text-sm font-medium hover:bg-zinc-800 hover:text-white ${active ? "bg-zinc-800 text-white" : "text-zinc-300"}`}
      onClick={onClick}
    >
      <Icon className="h-3.5 w-3.5" />
      {label}
