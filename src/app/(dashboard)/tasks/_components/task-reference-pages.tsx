        ...
        <section className="flex min-h-0 flex-col rounded-xl border border-zinc-200 bg-white !p-4">
          ...
          <div className="flex flex-1 items-center justify-center text-center">
            <div className="w-full max-w-[360px]">
              ...
              <p className="mx-auto mb-4 max-w-[260px] text-sm leading-5 text-zinc-600">
...
export function PersonalListReferencePage() {
  return (
    <TaskSurface
      ...
      titlePrefix="My Wrk"
      headerRight={
        <div className="flex items-center gap-3 text-base text-zinc-600">
    ...
    <div className="flex h-full flex-col bg-white">
      <header className="flex h-10 shrink-0 items-center justify-between border-b border-zinc-200 !px-4">
        <div
          role="heading"
          aria-level={1}
          className="flex min-w-0 items-center gap-1.5 whitespace-nowrap text-base font-semibold leading-5 text-zinc-900"
        >
...
function CalendarConnect({ label }: { label: string }) {
  return (
    <div className="mb-2 flex h-9 items-center justify-between rounded-lg border border-zinc-200 bg-white !px-2.5 text-left shadow-sm">
      ...
      <button type="button" className="rounded-md bg-zinc-100 !px-2 py-1 text-sm text-zinc-600">
        Connect
      </button>
    </div>
  );
  ...
  return (
    <section className="relative flex min-h-0 flex-col rounded-xl border border-zinc-200 bg-white !p-4">
      <div className="flex items-center justify-between mb-2">
        <h2 className="text-base font-semibold text-zinc-900">My Work</h2>
        <IconButton Icon={Settings} label="Settings" />
      </div>

      <div className="flex items-center gap-4 border-b border-zinc-100 mb-1">
        {(["todo", "done", "delegated"] as const).map((key) => (
          <button key={key} type="button" onClick={() => setTab(key)} className={`relative pb-2 text-base ${tab === key ? "text-zinc-900 font-medium" : "text-zinc-500 hover:text-zinc-700"}`}>
      ...
      <div className="flex-1 overflow-y-auto -mx-1 px-1">
        {loading ? (
          <div className="flex items-center justify-center gap-2 py-10 text-base text-zinc-400"><Loader2 className="w-4 h-4 animate-spin" /> Loading…</div>
        ) : tab === "delegated" ? (
          delegatedState === "loading" ? (
            <div className="flex items-center justify-center gap-2 py-10 text-base text-zinc-400"><Loader2 className="w-4 h-4 animate-spin" /> Loading…</div>
          ) : delegated.length === 0 ? (
            <div className="py-10 text-center text-base text-zinc-400">You haven’t delegated any tasks.</div>
          ) : (
          ...
          visible.length === 0 ? (
            <div className="py-10 text-center text-base text-zinc-400">Nothing completed yet.</div>
          ) : (
            ...
            return (
              <div key={g} className="py-0.5">
                <button type="button" onClick={() => setCollapsed((c) => ({ ...c, [g]: !c[g] }))} className="flex items-center gap-1.5 w-full text-left py-1.5">
                  ...
                  <span className={`text-base font-medium ${g === "Overdue" && items.length > 0 ? "text-red-500" : "text-zinc-700"}`}>{g}</span>
                ...
                {open && (
                  <div className="pl-5">
                    ...
                    {items.length === 0 && g === "Today" && composerGroup !== g && (
                      <p className="py-1.5 text-sm text-zinc-400">Tasks and reminders assigned to you will show here.</p>
                    )}
                    {composerGroup === g ? (
                      <div className="flex items-center gap-2 py-1.5">
                        <span className="w-4 h-4 rounded-full border border-dashed border-zinc-300 shrink-0" />
                        <input autoFocus value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") void addTask(g); if (e.key === "Escape") { setComposerGroup(null); setDraft(""); } }} onBlur={() => void addTask(g)} placeholder="Task name" className="flex-1 text-base bg-transparent outline-none placeholder:text-zinc-400" />
                      </div>
                    ) : (g === "Today" || g === "Next" || g === "Unscheduled") ? (
                      <button type="button" onClick={() => { setComposerGroup(g); setDraft(""); }} className="flex items-center gap-1.5 py-1.5 text-sm text-zinc-400 hover:text-zinc-600">
...
function TaskRow({ task, today, busy, onToggle, showAssignee }: { task: WorkTask; today: Date; busy: boolean; onToggle: () => void; showAssignee?: boolean }) {
  ...
  return (
    <div className="group flex items-center gap-2.5 py-1.5 border-b border-zinc-50 last:border-0">
      ...
      <span className={`flex-1 text-base truncate ${task.done ? "line-through text-zinc-400" : "text-zinc-800"}`}>{task.title}</span>
      {showAssignee && task.assigneeName && (
        <span className="text-xs text-zinc-500 bg-zinc-100 rounded-full px-2 py-0.5 shrink-0 truncate max-w-[120px]">{task.assigneeName}</span>
      )}
      {task.priority && <Flag className="w-3.5 h-3.5 shrink-0" style={{ color: PRIORITY_COLOR[task.priority] }} />}
      {dueLabel && <span className={`text-sm shrink-0 ${overdue ? "text-red-500" : "text-zinc-400"}`}>{dueLabel}</span>}
    </div>
  );
}
