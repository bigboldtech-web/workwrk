  ...
  return (
    <article className="rounded-lg border border-zinc-200 bg-white">
      <header className="flex items-center gap-3 px-4 py-3 border-b border-zinc-200">
        <span className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-zinc-100 text-xs font-medium">
          {initials}
        </span>
        <div className="flex-1 min-w-0">
          <div className="text-xs font-medium truncate">{t.manager.firstName} {t.manager.lastName}</div>
        ...
        {t.manager.via === "dotted" ? (
          <span className="inline-flex items-center gap-1 text-micro uppercase tracking-wide px-1.5 py-0.5 rounded bg-zinc-100 text-zinc-600">
...
function OwnReviewBadge({ own }: { own: SubTeam["manager"]["ownReview"] }) {
  ...
  if (own.status === "DRAFT") {
    return <span className="text-micro uppercase tracking-wide px-1.5 py-0.5 rounded bg-zinc-100 text-zinc-600">Own: Draft</span>;
  }
  if (own.status === "SUBMITTED") {
    return <span className="text-micro uppercase tracking-wide px-1.5 py-0.5 rounded bg-amber-500/15 text-amber-700">Own: Submitted</span>;
  }
...
function DirectIcRow({ ic }: { ic: DirectIcSummary }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-md border border-zinc-200 bg-white px-3 py-2">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs truncate">{ic.firstName} {ic.lastName}</div>
        <div className="text-xs text-zinc-500 truncate">{ic.email}</div>
      </div>
      {ic.via === "dotted" ? (
        <span className="text-micro uppercase tracking-wide px-1.5 py-0.5 rounded bg-zinc-100 text-zinc-600">Dotted</span>
      ) : null}
...
function ReviewChip({ review }: { review: DirectIcSummary["weeklyReview"] }) {
  if (!review.status) {
    return <span className="inline-flex items-center gap-1 text-micro uppercase tracking-wide px-1.5 py-0.5 rounded bg-red-500/15 text-red-700"><AlertCircle className="w-3 h-3" />No review</span>;
  }
  if (review.status === "DRAFT") {
    return <span className="inline-flex items-center gap-1 text-micro uppercase tracking-wide px-1.5 py-0.5 rounded bg-zinc-100 text-zinc-600">Draft</span>;
  }
  if (review.status === "SUBMITTED") {
    return <span className="inline-flex items-center gap-1 text-micro uppercase tracking-wide px-1.5 py-0.5 rounded bg-amber-500/15 text-amber-700"><Clock className="w-3 h-3" />Submitted</span>;
  }
  const approved = review.managerStatus === "APPROVED";
  return <span className={`inline-flex items-center gap-1 text-micro uppercase tracking-wide px-1.5 py-0.5 rounded ${
    approved ? "bg-emerald-500/15 text-emerald-700" : "bg-red-500/15 text-red-700"
