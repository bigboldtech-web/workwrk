    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="workwrk-os os-portal-panel max-w-[560px]">
        ...
        <div className="flex flex-col gap-4 py-1">
          ...
          {/* Content */}
          <label className="flex flex-col gap-1.5">
            <span className="text-sm font-medium text-[var(--os-ink-2)]">Content</span>
            <textarea
              value={content}
              onChange={(e) => setContent(e.target.value)}
              placeholder="Write the announcement…"
              rows={4}
              className="rounded-lg border border-[var(--os-line)] bg-[var(--os-surface-1)] px-3 py-2 text-base leading-relaxed text-[var(--os-ink)] placeholder:text-[var(--os-ink-4)] outline-none resize-y focus:border-[var(--os-brand)]"
            />
          </label>
          ...
          <div className="flex flex-col gap-1.5">
            <span className="text-sm font-medium text-[var(--os-ink-2)]">Who sees this</span>
              ...
              {AUDIENCE_OPTS.map(({ value, label }) => {
                ...
                return (
                  <button
                    ...
                    className={`h-8 px-3 rounded-lg text-base border transition-colors ${
                      active
                    ...
                    {audOptions.map((o) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-2 h-8 px-2 rounded-md text-base text-left border ${
          ...
          {/* Type */}
          <div className="flex flex-col gap-1.5">
            <span className="text-sm font-medium text-[var(--os-ink-2)]">Type</span>
            <div className="flex flex-wrap gap-1.5">
              {TYPE_OPTS.map(({ value, label, Icon }) => {
                ...
                return (
                  <button
                    ...
                    className={`inline-flex items-center gap-1.5 h-8 px-2.5 rounded-lg text-base border transition-colors ${
                      active
                        ? "border-[var(--os-brand)] bg-[var(--os-brand-soft)] text-[var(--os-brand-deep)] font-medium"
                        : "border-[var(--os-line)] text-[var(--os-ink-2)] hover:bg-[var(--os-surface-1)]"
                    }`}
          ...
          {/* Priority */}
          <div className="flex flex-col gap-1.5">
            <span className="text-sm font-medium text-[var(--os-ink-2)]">Priority</span>
            <div className="flex flex-wrap gap-1.5">
          ...
          <div className="flex flex-col gap-2.5 rounded-lg border border-[var(--os-line)] p-3">
            <label className="flex items-center justify-between gap-3">
              <span className="flex flex-col">
                <span className="text-base text-[var(--os-ink)]">Pin to top</span>
                <span className="text-xs text-[var(--os-ink-3)]">Keeps this above the priority sections.</span>
              </span>
              <Switch checked={pinned} onChange={setPinned} aria-label="Pin to top" />
            </label>
            <label className="flex items-center justify-between gap-3">
              <span className="flex flex-col">
                <span className="text-base text-[var(--os-ink)]">Require acknowledgment</span>
                <span className="text-xs text-[var(--os-ink-3)]">Each person must confirm they&apos;ve read it.</span>
              </span>
          ...
          <div className="flex flex-col gap-3">
            <label className="flex items-center justify-between gap-3">
              <span className="flex flex-col">
                <span className="text-base text-[var(--os-ink)]">Schedule for later</span>
                <span className="text-xs text-[var(--os-ink-3)]">Off publishes immediately.</span>
              </span>
              <Switch checked={scheduleOn} onChange={setScheduleOn} aria-label="Schedule for later" />
            </label>
            {scheduleOn ? (
              <label className="flex flex-col gap-1.5">
                <span className="text-sm font-medium text-[var(--os-ink-2)]">Publish at</span>
                <input
                  type="datetime-local"
                  value={publishedAt}
            ...
            <label className="flex flex-col gap-1.5">
              <span className="text-sm font-medium text-[var(--os-ink-2)]">Expires on</span>
              <input
                type="date"
                value={expiresAt}
                ...
                className="h-10 rounded-lg border border-[var(--os-line)] bg-[var(--os-surface-1)] px-3 text-base text-[var(--os-ink)] outline-none focus:border-[var(--os-brand)]"
              />
              <span className="text-xs text-[var(--os-ink-3)]">Required — the post drops off the feed after this date.</span>
            </label>
          </div>
