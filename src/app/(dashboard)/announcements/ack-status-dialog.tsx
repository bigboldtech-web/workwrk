  ...
  return (
    <Dialog open onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent className="workwrk-os os-portal-panel max-w-[520px]">
        ...
        {error ? (
          <div className="py-8 text-center text-base text-[var(--os-c-red)]">{error}</div>
        ) : data === null ? (
          <div className="flex items-center justify-center gap-2 py-10 text-base text-[var(--os-ink-3)]">
          ...
          <div className="flex flex-col gap-4 py-1">
            ...
            <div className="flex flex-col gap-2 rounded-lg border border-[var(--os-line)] p-3">
              <div className="flex items-center justify-between">
                <span className="inline-flex items-center gap-1.5 text-base text-[var(--os-ink-2)]">
                  <Users className="w-3.5 h-3.5" /> Organization-wide
                </span>
                <span className="text-base font-semibold text-[var(--os-ink)]">
            ...
            <section className="flex flex-col gap-1.5">
              <h4 className="inline-flex items-center gap-1.5 text-sm font-semibold text-[var(--os-ink-2)]">
              ...
              {acked.length === 0 ? (
                <p className="text-base text-[var(--os-ink-3)] px-0.5">No one has acknowledged yet.</p>
              ) : (
                  ...
                  {acked.map((u) => (
                    <li key={u.id} className="flex items-center gap-2.5 rounded-lg px-1.5 py-1">
                      ...
                      <span className="flex-1 min-w-0">
                        <span className="block text-base text-[var(--os-ink)] truncate">{fullName(u)}</span>
                        <span className="block text-xs text-[var(--os-ink-3)] truncate">{u.email}</span>
                      </span>
                      <span className="inline-flex items-center gap-1 text-xs text-[var(--os-ink-3)] shrink-0">
            ...
            <section className="flex flex-col gap-1.5">
              ...
              {pending.length === 0 ? (
                <p className="text-base text-[var(--os-ink-3)] px-0.5">Everyone has acknowledged. 🎉</p>
              ) : (
                <ul className="flex flex-col gap-1 max-h-[180px] overflow-y-auto">
                  {pending.map((u) => (
                    <li key={u.id} className="flex items-center gap-2.5 rounded-lg px-1.5 py-1">
...
function Initials({ u }: { u: RosterRow }) {
  return (
    <span
      className="inline-flex items-center justify-center w-7 h-7 rounded-full shrink-0 text-xs font-semibold text-[var(--os-ink-2)] bg-[var(--os-surface-2)]"
      aria-hidden="true"
    >
      {initials(u)}
    </span>
