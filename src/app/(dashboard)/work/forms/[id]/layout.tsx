// /work/forms/[id]: the Work address of a form (the Work door).
//
// A form opened from Work stays in Work. Forms have no Space and no Work
// tree row, so the door is their only Work address; the form's Guest rule
// (formGateAllows) runs in the gate, exactly as FormGate on /forms/[id].
...
export default async function WorkDoorFormLayout({
  children,
  params,
}: {
  children: ReactNode;
  ...
  return (
    <WorkObjectGate kind="form" id={id} at={{ scope: "work" }}>
      {children}
    </WorkObjectGate>
  );
}
