    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v && !removing) onClose(); }}>
      ...
      >
        ...
        <div className="mt-3 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/40 px-3 py-2.5 space-y-1">
          {handoverLoading ? (
            <p className="text-sm text-zinc-400">Checking what {firstName} still holds…</p>
        ...
        {handover && handover.directReports.count > 0 ? (
          <div className="mt-2.5 flex items-start gap-2 rounded-lg bg-amber-50 dark:bg-amber-500/10 px-3 py-2 text-sm leading-relaxed text-amber-700 dark:text-amber-400">
        ...
        {showPicker ? (
          <div className="mt-3">
            ...
            <select
              ...
              disabled={removing}
              className="w-full h-8 px-2 rounded-md border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-base text-zinc-700 dark:text-zinc-200 outline-none"
            >
