  ...
  return (
    <div className="relative">
      <button
        ...
        onClick={() => setOpen((s) => !s)}
        className="flex h-9 w-full items-center justify-between gap-2 rounded-lg border border-zinc-200 bg-white px-3 text-left text-base text-zinc-900 hover:border-zinc-300 focus:border-[#0073EA] focus:outline-none focus:ring-2 focus:ring-[#0073EA]/20 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
      >
            ...
            {searchable && (
              <div className="px-2 pb-1">
                <input
                  ...
                  className="h-8 w-full rounded-md border border-zinc-200 bg-white px-2 text-base text-zinc-900 outline-none focus:border-[#0073EA] dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
                />
              </div>
            )}
            {filtered.length === 0 ? (
              <div className="px-3 py-2 text-base text-zinc-400">No matches</div>
            ) : filtered.map((o) => (
              <button
                key={o.value || "__none"}
                type="button"
                onClick={() => { onChange(o.value); setOpen(false); setQ(""); }}
                className="flex w-full items-center gap-2 px-3 py-1.5 text-left text-base text-zinc-800 hover:bg-zinc-50 dark:text-zinc-200 dark:hover:bg-zinc-800"
              >
                {o.leading}
                <span className="min-w-0 flex-1 truncate">{o.label}</span>
                {o.hint && <span className="text-xs text-zinc-400">{o.hint}</span>}
...
function MemberAvatar({ m }: { m: Member }) {
  return (
    <span
      className="grid h-5 w-5 shrink-0 place-items-center rounded-full text-micro font-semibold text-white"
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="flex flex-col gap-3.5 pt-1">
          ...
          <label className="flex flex-col gap-1">
            ...
            <input
              ...
              className="h-9 w-full rounded-lg border border-zinc-200 bg-white px-3 text-base text-zinc-900 outline-none focus:border-[#0073EA] focus:ring-2 focus:ring-[#0073EA]/20 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100"
            />
          </label>

          {/* Description */}
          <label className="flex flex-col gap-1">
            <span className="text-sm font-medium text-zinc-500 dark:text-zinc-400">Description <span className="font-normal text-zinc-400">(optional)</span></span>
            <textarea
              value={description}
          ...
          <div className="flex flex-col gap-1.5">
            <span className="text-sm font-medium text-zinc-500 dark:text-zinc-400">Color</span>
          ...
          <div className="flex flex-col gap-1">
            <span className="text-sm font-medium text-zinc-500 dark:text-zinc-400">Department head</span>
            <PickerField
          ...
          {/* Parent */}
          <div className="flex flex-col gap-1">
            <span className="text-sm font-medium text-zinc-500 dark:text-zinc-400">Parent department</span>
            <PickerField
        ...
        <DialogFooter>
          <button
            ...
            onClick={onClose}
            disabled={saving}
            className="inline-flex h-8 items-center rounded-lg border border-zinc-200 px-3 text-base font-medium text-zinc-600 hover:bg-zinc-50 disabled:opacity-50 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-800"
          >
            Cancel
          </button>
          <button
