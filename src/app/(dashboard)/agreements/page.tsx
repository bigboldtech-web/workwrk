                  ...
                  {g.items.map((r) => (
                    <div key={r.id}
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1 truncate text-xs font-semibold text-zinc-900">{r.title}</div>
                        <div className="flex shrink-0 items-center gap-1.5">
                          {r.isTemplate
                            ? <span className="rounded bg-blue-50 px-1.5 py-0.5 text-xs font-medium text-blue-700">template</span>
                            : <span className={`rounded px-1.5 py-0.5 text-xs font-medium capitalize ${STATUS_STYLE[r.status] ?? "bg-zinc-100 text-zinc-600"}`}>{r.status.replace(/_/g, " ").toLowerCase()}</span>}
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/30 p-4" onClick={() => setEdit(null)}>
          <div className="w-full max-w-sm rounded-2xl border border-zinc-200 bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <input autoFocus list={edit.mode === "folder" ? "agreement-folders" : undefined} value={edit.value}
              ...
              className="h-9 w-full rounded-md border border-zinc-200 px-3 text-base outline-none focus:border-zinc-300" />
            {edit.mode === "folder" && <datalist id="agreement-folders">{CATEGORY_OPTIONS.map((c) => <option key={c} value={c} />)}</datalist>}
            <div className="mt-4 flex justify-end gap-2">
              <button type="button" onClick={() => setEdit(null)} className="inline-flex h-8 items-center rounded-md border border-zinc-200 px-3 text-base text-zinc-700 hover:bg-zinc-50">Cancel</button>
              <button id="agreement-edit-save" type="button"
                ...
                className="inline-flex h-8 items-center rounded-md bg-[var(--os-brand)] px-3 text-base font-medium text-white hover:bg-[var(--os-brand-hover)]">Save</button>
            </div>
          </div>
        </div>
      ) : null}
      ...
      {folderEdit ? (
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/30 p-4" onClick={() => setFolderEdit(null)}>
          <div className="w-full max-w-sm rounded-2xl border border-zinc-200 bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="mb-3 text-sm text-zinc-500">Applies to {folderEdit.items.length} item{folderEdit.items.length === 1 ? "" : "s"}.</p>
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button type="button" onClick={() => setFolderEdit(null)} className="inline-flex h-8 items-center rounded-md border border-zinc-200 px-3 text-base text-zinc-700 hover:bg-zinc-50">Cancel</button>
      ...
      {showNew ? (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/30 p-4" onClick={() => !busy && setShowNew(false)}>
          <div className="w-full max-w-lg rounded-2xl border border-zinc-200 bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            {step === "choose" ? (
              <>
                <p className="mb-4 text-base text-zinc-500">Choose how you want to start.</p>
                    ...
                    {templates.map((t) => (
                      <li key={t.id}>
                        <button type="button" disabled={busy} onClick={() => createWith({ fromTemplateId: t.id })}
                          ...
                          <span className="min-w-0 flex-1 truncate text-base font-medium text-zinc-800">{t.title}</span>
                          {t.category ? <span className="shrink-0 rounded bg-zinc-100 px-1.5 py-0.5 text-xs text-zinc-500">{t.category}</span> : null}
                        </button>
                      </li>
                    ))}
                  </ul>
...
function MenuItem({ Icon, label, onClick, danger }: { Icon: typeof Pencil; label: string; onClick: () => void; danger?: boolean }) {
  return (
    <button type="button" onClick={onClick} className={`flex w-full items-center gap-2 px-3 py-1.5 text-left text-base hover:bg-zinc-50 ${danger ? "text-red-600" : "text-zinc-700"}`}>
...
function ChooserCard({ Icon, title, sub, onClick, disabled }: { Icon: typeof PenLine; title: string; sub: string; onClick: () => void; disabled?: boolean }) {
  return (
    <button type="button" onClick={onClick} disabled={disabled}
      ...
      <span className="text-xs leading-snug text-zinc-500">{sub}</span>
    </button>
  );
}
