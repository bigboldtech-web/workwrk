      ...
      <OsTitleBar
        ...
        actions={
          <div className="flex items-center gap-2">
            <Link href="/agreements" className={`inline-flex h-8 items-center gap-1.5 rounded-md border px-2.5 text-[14px] hover:bg-zinc-50 ${view === "live" ? "border-zinc-300 bg-zinc-50 text-zinc-900" : "border-zinc-200 text-zinc-700"}`}><FileSignature className="h-3.5 w-3.5" /> Contracts</Link>
            <Link href="/agreements?view=templates" className={`inline-flex h-8 items-center gap-1.5 rounded-md border px-2.5 text-[14px] hover:bg-zinc-50 ${view === "templates" ? "border-zinc-300 bg-zinc-50 text-zinc-900" : "border-zinc-200 text-zinc-700"}`}><Folder className="h-3.5 w-3.5" /> Templates</Link>
            {view === "templates" ? (
              <button type="button" onClick={newTemplate} disabled={busy} className="inline-flex h-8 items-center gap-1.5 rounded-md bg-[var(--os-brand)] px-3 text-[14px] font-medium text-white hover:bg-[var(--os-brand-hover)] disabled:opacity-50">
                {busy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Plus className="h-3.5 w-3.5" />} New template
              </button>
            ) : (
              <button type="button" onClick={openNew} className="inline-flex h-8 items-center gap-1.5 rounded-md bg-[var(--os-brand)] px-3 text-[14px] font-medium text-white hover:bg-[var(--os-brand-hover)]"><Plus className="h-3.5 w-3.5" /> New contract</button>
            )}
          </div>
        }
      />
      ...
      <div className="mx-auto max-w-5xl px-6 py-8">
        {rows === null ? (
          ...
          <div className="rounded-xl border border-dashed border-zinc-200 p-10 text-center">
            ...
            <div className="mt-1 text-[14px] text-zinc-500">{view === "templates" ? "Create a template, or open a contract and choose “Save as template”." : "Write or upload a document, add signers, place fields, and send it."}</div>
            <button type="button" onClick={view === "templates" ? newTemplate : openNew} disabled={busy} className="mt-4 inline-flex h-9 items-center gap-1.5 rounded-md bg-[var(--os-brand)] px-4 text-[14px] font-medium text-white hover:bg-[var(--os-brand-hover)] disabled:opacity-50"><Plus className="h-4 w-4" /> {view === "templates" ? "New template" : "New contract"}</button>
          </div>
        ) : (
                    ...
                    <div key={r.id}
                      ...
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex shrink-0 items-center gap-1.5">
                          {r.isTemplate
                            ? <span className="rounded bg-blue-50 px-1.5 py-0.5 text-[12px] font-medium text-blue-700">template</span>
                            : <span className={`rounded px-1.5 py-0.5 text-[12px] font-medium capitalize ${STATUS_STYLE[r.status] ?? "bg-zinc-100 text-zinc-600"}`}>{r.status.replace(/_/g, " ").toLowerCase()}</span>}
                      ...
                      <div className="mt-3 flex items-center gap-3 text-[13px] text-zinc-400">
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/30 p-4" onClick={() => setEdit(null)}>
          <div className="w-full max-w-sm rounded-2xl border border-zinc-200 bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button type="button" onClick={() => setEdit(null)} className="inline-flex h-8 items-center rounded-md border border-zinc-200 px-3 text-[14px] text-zinc-700 hover:bg-zinc-50">Cancel</button>
              <button id="agreement-edit-save" type="button"
                ...
                className="inline-flex h-8 items-center rounded-md bg-[var(--os-brand)] px-3 text-[14px] font-medium text-white hover:bg-[var(--os-brand-hover)]">Save</button>
            </div>
          </div>
        </div>
      ) : null}
      ...
      {folderEdit ? (
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/30 p-4" onClick={() => setFolderEdit(null)}>
          <div className="w-full max-w-sm rounded-2xl border border-zinc-200 bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="mb-3 text-[13px] text-zinc-500">Applies to {folderEdit.items.length} item{folderEdit.items.length === 1 ? "" : "s"}.</p>
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button type="button" onClick={() => setFolderEdit(null)} className="inline-flex h-8 items-center rounded-md border border-zinc-200 px-3 text-[14px] text-zinc-700 hover:bg-zinc-50">Cancel</button>
      ...
      {showNew ? (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/30 p-4" onClick={() => !busy && setShowNew(false)}>
          <div className="w-full max-w-lg rounded-2xl border border-zinc-200 bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            {step === "choose" ? (
              <>
                <p className="mb-4 text-[14px] text-zinc-500">Choose how you want to start.</p>
              ...
              <>
                <button type="button" onClick={() => setStep("choose")} className="mb-3 text-[13px] text-zinc-500 hover:text-zinc-800">← Back</button>
                {templates === null ? (
                  <div className="flex items-center gap-2 py-6 text-[14px] text-zinc-500"><Loader2 className="h-4 w-4 animate-spin" /> Loading templates…</div>
                ) : templates.length === 0 ? (
                  <div className="rounded-lg border border-dashed border-zinc-200 p-6 text-center text-[14px] text-zinc-500">No templates yet. Open a contract and choose “Save as template”.</div>
                ) : (
                    ...
                    {templates.map((t) => (
                      <li key={t.id}>
                        <button type="button" disabled={busy} onClick={() => createWith({ fromTemplateId: t.id })}
                          ...
                          <span className="min-w-0 flex-1 truncate text-[14px] font-medium text-zinc-800">{t.title}</span>
                          {t.category ? <span className="shrink-0 rounded bg-zinc-100 px-1.5 py-0.5 text-[12px] text-zinc-500">{t.category}</span> : null}
                        </button>
                      </li>
                    ))}
                  </ul>
            ...
            {busy ? <div className="mt-4 flex items-center gap-2 text-[14px] text-zinc-500"><Loader2 className="h-4 w-4 animate-spin" /> Working…</div> : null}
          </div>
        </div>
      ) : null}
    </>
...
function MenuItem({ Icon, label, onClick, danger }: { Icon: typeof Pencil; label: string; onClick: () => void; danger?: boolean }) {
  return (
    <button type="button" onClick={onClick} className={`flex w-full items-center gap-2 px-3 py-1.5 text-left text-[14px] hover:bg-zinc-50 ${danger ? "text-red-600" : "text-zinc-700"}`}>
...
function ChooserCard({ Icon, title, sub, onClick, disabled }: { Icon: typeof PenLine; title: string; sub: string; onClick: () => void; disabled?: boolean }) {
  return (
    <button type="button" onClick={onClick} disabled={disabled}
      ...
      <span className="text-[12px] leading-snug text-zinc-500">{sub}</span>
    </button>
  );
}
