...
// Benefits workspace — Plans + Open Enrollments. Plan creation is
// the entry point; OE windows + per-employee elections come once
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-muted text-sm mt-1">
          Carrier plans, open enrollment windows, and elections.
        </p>
      </div>
      <Tabs defaultValue="plans">
        <TabsList>
          <TabsTrigger value="plans">Plans</TabsTrigger>
          <TabsTrigger value="oe">Open enrollments</TabsTrigger>
        </TabsList>
        <TabsContent value="plans" className="mt-4">
          <PlansTab />
        </TabsContent>
        <TabsContent value="oe" className="mt-4">
          <OpenEnrollmentsTab />
        </TabsContent>
      </Tabs>
    </div>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="p-10 text-center text-sm text-muted">
            No benefit plans yet. Add medical, dental, vision, 401(k), or any other plan you offer.
          </CardContent>
        </Card>
      ) : (
        <Card>
    ...
    try {
      const res = await fetch("/api/benefit-plans", {
        ...
        body: JSON.stringify({
          ...
          employerCost: Number(employerCost) || 0,
          effectiveFrom,
...
function OpenEnrollmentsTab() {
  ...
  return (
    <>
      <div className="flex justify-end mb-3">
        <Button onClick={() => setShowCreate(true)}>
          <Plus size={14} className="mr-1.5" /> New OE window
        </Button>
      </div>
      {loading ? (
        <div className="text-center py-8 text-sm text-muted">Loading…</div>
        ...
        <Card>
          <CardContent className="p-10 text-center text-sm text-muted">
            No open enrollment windows yet. Schedule one to let employees pick benefits.
          </CardContent>
        </Card>
      ) : (
        <Card>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-b border-white/5 hover:bg-surface-2">
                    ...
                    <td className="px-4 py-2.5">
                      <Badge variant="outline" className={`text-[10px] ${OE_STATUS_STYLE[r.status]}`}>{r.status}</Badge>
                    </td>
                  </tr>
                ))}
              </tbody>
