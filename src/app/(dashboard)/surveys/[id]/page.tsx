        ...
        <div className="flex items-start gap-3 flex-wrap">
          ...
          {canSeeResults ? (
            <button
              ...
              className="inline-flex items-center gap-1.5 h-8 px-3 rounded-lg border border-[var(--os-line)] text-base text-[var(--os-ink-2)] hover:bg-[var(--os-surface-1)]"
            >
    ...
    return (
      <div className="flex flex-col gap-4">
        <div className="rounded-xl border border-[var(--os-line)] bg-[var(--os-surface-1)] p-5 text-center">
          ...
          <div className="mt-2 text-base font-semibold text-[var(--os-ink)]">
            {isDraft ? "This survey isn't open yet" : "This survey is closed"}
          </div>
          <div className="mt-1 text-base text-[var(--os-ink-3)]">
            {isDraft
    ...
    <div className="flex flex-col gap-4">
      {survey.anonymous ? (
        <div className="flex items-start gap-2 rounded-lg border border-[var(--os-line)] bg-[var(--os-surface-1)] px-3.5 py-2.5">
          ...
          <div className="text-base text-[var(--os-ink-2)]">
      ...
      {done ? (
        <div className="flex items-center gap-2 rounded-lg border border-[color:var(--os-c-green)]/40 bg-[color:var(--os-c-green)]/10 px-3.5 py-2.5 text-base text-[var(--os-c-green)]">
  ...
  return (
    <div className="rounded-xl border border-[var(--os-line)] bg-white p-4">
      <div className="flex items-baseline gap-2">
        <span className="text-xs font-semibold text-[var(--os-ink-4)] tabular-nums">{index + 1}</span>
        <div className="text-base font-medium text-[var(--os-ink)]">{question.text}</div>
      </div>
      <div className="mt-3">
        {question.type === "rating" ? (
          ...
          <textarea
            ...
            rows={3}
            className="w-full rounded-lg border border-[var(--os-line)] bg-[var(--os-surface-1)] px-3 py-2 text-base leading-relaxed text-[var(--os-ink)] placeholder:text-[var(--os-ink-4)] outline-none resize-y focus:border-[var(--os-brand)]"
          />
        )}
      </div>
    </div>
...
function ScaleRow({
  ...
  return (
    <div className="flex flex-col gap-1.5">
        ...
        {nums.map((n) => {
          ...
          return (
            <button
              ...
              onClick={() => onPick(n)}
              aria-label={`${n}`}
              className={`h-9 min-w-9 px-2 inline-flex items-center justify-center gap-1 rounded-lg border text-base font-medium transition-colors ${
                active
      ...
      <div className="flex justify-between text-xs text-[var(--os-ink-4)] px-0.5">
        <span>{labelLeft}</span>
...
function OptionChip({ active, label, onClick }: { active: boolean; label: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`h-9 px-4 rounded-lg border text-base font-medium transition-colors ${
        active
          ? "border-[var(--os-brand)] bg-[var(--os-brand-soft)] text-[var(--os-brand-deep)]"
          : "border-[var(--os-line)] text-[var(--os-ink-2)] hover:bg-[var(--os-surface-1)]"
      }`}
...
function ChoiceRow({ active, label, kind, onClick }: { active: boolean; label: string; kind: "radio" | "check"; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex items-center gap-2.5 h-10 px-3 rounded-lg border text-base transition-colors text-left ${
        active
...
function ReadonlyAnswers({ survey, values }: { survey: DetailResp["survey"]; values: Record<string, AnswerValue> }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="text-xs font-semibold uppercase tracking-wide text-[var(--os-ink-4)]">Your answers</div>
      {answered.map((q) => {
        const v = values[q.id];
        const display = Array.isArray(v) ? v.join(", ") : String(v);
        return (
          <div key={q.id} className="rounded-lg border border-[var(--os-line)] bg-white p-3">
            <div className="text-base font-medium text-[var(--os-ink)]">{q.text}</div>
            <div className="mt-1 text-base text-[var(--os-ink-2)]">{display}</div>
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="rounded-xl border border-[var(--os-line)] bg-white p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="text-base font-medium text-[var(--os-ink)]">{q.text}</div>
        <span className="shrink-0 text-xs text-[var(--os-ink-4)] tabular-nums">{q.totalAnswered} answered</span>
      </div>
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-end gap-2">
        <span className="text-xl font-semibold text-[var(--os-ink)] leading-none tabular-nums">{q.average ?? "—"}</span>
        <span className="text-sm text-[var(--os-ink-4)] mb-0.5">avg · {q.min}–{q.max}</span>
      </div>
      <div className="flex flex-col gap-1.5">
        {q.distribution.map((d) => {
          ...
          return (
            <div key={d.value} className="flex items-center gap-2">
              <span className="w-6 text-right text-xs text-[var(--os-ink-3)] tabular-nums">{d.value}</span>
              ...
              <span className="w-16 text-xs text-[var(--os-ink-4)] tabular-nums text-right">{d.count} · {pct}%</span>
            </div>
          );
        })}
      </div>
      ...
      {options.map((o) => {
        ...
        return (
          <div key={o.value} className="flex items-center gap-2">
            <span className="w-28 truncate text-sm text-[var(--os-ink-2)]" title={o.value}>{o.value}</span>
            <div className="flex-1 h-4 rounded bg-[var(--os-surface-1)] overflow-hidden">
              <div className="h-full rounded bg-[var(--os-brand)]" style={{ width: `${(o.count / maxCount) * 100}%` }} />
            </div>
            <span className="w-16 text-xs text-[var(--os-ink-4)] tabular-nums text-right">{o.count} · {pct}%</span>
          </div>
        );
      })}
    </div>
...
function TabButton({ active, onClick, Icon, label }: { active: boolean; onClick: () => void; Icon: typeof Activity; label: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex items-center gap-1.5 h-8 px-3 rounded-lg text-base font-medium transition-colors ${
        active
...
function StatTile({ label, value, sub, Icon }: { label: string; value: string; sub: string; Icon: typeof Activity }) {
  return (
    <div className="rounded-xl border border-[var(--os-line)] bg-white px-3.5 py-2.5 min-w-[130px]">
      <div className="flex items-center gap-1.5 text-micro font-semibold uppercase tracking-wide text-[var(--os-ink-4)]">
...
function StatePanel({
  ...
  return (
    <div className={`flex flex-col items-center justify-center text-center gap-2 ${inline ? "py-16" : "h-full py-24"}`}>
      ...
      <div className="text-base font-semibold text-[var(--os-ink)]">{title}</div>
      <div className="text-base text-[var(--os-ink-3)] max-w-[340px]">{subtitle}</div>
      {onRetry ? (
        <button
          type="button"
          onClick={onRetry}
          className="mt-1 h-8 px-3 rounded-lg border border-[var(--os-line)] text-base text-[var(--os-ink-2)] hover:bg-[var(--os-surface-1)]"
        >
          Retry
        </button>
      ) : null}
