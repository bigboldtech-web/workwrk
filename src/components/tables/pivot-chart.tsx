...
// Series hues: the brand palette, no purple (brand rule), through the tokens
// so a chart follows the theme. SVG fill accepts var() like the stroke below.
const PALETTE = [
  "var(--os-brand)", "var(--os-success-solid)", "var(--os-warning-solid)", "var(--os-danger-solid)",
  "var(--os-status-user-2)", "var(--os-status-user-7)", "var(--os-status-user-5)", "var(--os-status-user-4)",
];
...
export function PivotChart({ result, type, height = 340 }: { result: PivotResult; type: ChartType; height?: number }) {
  ...
  if (categories.length === 0 || seriesNames.length === 0) {
    return <div className="h-full flex items-center justify-center text-sm text-ink-3">Nothing to chart yet.</div>;
  }

  const legend = (
    <div className="flex flex-wrap gap-x-4 gap-y-1 justify-center mt-3">
      {seriesNames.map((s, i) => (
        <span key={s} className="inline-flex items-center gap-1.5 text-xs text-ink-2">
  ...
  if (type === "pie") {
    ...
    const paths = slices.map((s, i) => {
      ...
      return <path key={i} d={d} fill={PALETTE[i % PALETTE.length]} stroke="var(--os-surface)" strokeWidth={1.5} />;
    });
    return (
      <div>
    ...
    <div>
      <svg width="100%" height={H} viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMidYMid meet">
        ...
        {gridLines.map((gv, i) => {
          ...
          return (
            <g key={i}>
              <line x1={padL} y1={y} x2={W - padR} y2={y} stroke="var(--os-line-soft)" strokeWidth={1} />
              <text x={padL - 8} y={y + 3} textAnchor="end" fontSize={11} fill="var(--os-ink-3)">{fmt(gv)}</text>
            </g>
          );
        })}
        {/* Category labels */}
        {categories.map((cat, ci) => (
          <text key={ci} x={padL + catW * ci + catW / 2} y={H - padB + 16} textAnchor="middle" fontSize={11} fill="var(--os-ink-2)">
        ...
        {bottom < 0 ? <line x1={padL} y1={zeroY} x2={W - padR} y2={zeroY} stroke="var(--os-line-strong)" strokeWidth={1} /> : null}
      </svg>
