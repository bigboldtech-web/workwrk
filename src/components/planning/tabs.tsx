...
// Planning tab components — extracted for board-route conversion.
// Two boards: plans, variance.
...
function CreatePlanDialog({ years, onClose, onCreated }: { years: FiscalYear[]; onClose: () => void; onCreated: () => void }) {
  const { toast } = useToast();
    ...
    <>
      <div className="flex items-center gap-3 mb-3">
        ...
        <Select value={planId} onValueChange={setPlanId}>
          ...
          <SelectContent>
            {plans.map((p) => (
              <SelectItem key={p.id} value={p.id}>{p.name} · v{p.version} · {p.fiscalYear.label}</SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>
      {loading ? (
        ...
        <Card><CardContent className="p-10 text-center text-sm text-muted">Pick a plan to see variance.</CardContent></Card>
      ) : data.rows.length === 0 ? (
        <Card><CardContent className="p-10 text-center text-sm text-muted">No plan lines on this scenario yet. Add lines to see variance against actuals.</CardContent></Card>
      ) : (
        <Card>
          <CardContent className="p-0">
