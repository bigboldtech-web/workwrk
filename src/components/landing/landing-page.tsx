...
const featureTabs = [
    ...
    visual: (
      <div className="flex flex-col gap-2">
        <div className="mb-1 flex items-center justify-between border-b border-border pb-2">
          <span className="font-[family-name:var(--font-mono)] text-[0.6rem] uppercase tracking-wider text-muted">Performance Scores — March 2026</span>
        ...
        {[
          ...
          <div key={p.name} className="flex items-center gap-3">
            <span className="w-16 truncate text-xs text-muted">{p.name}</span>
        ...
        <div className="mt-1 flex items-center justify-between text-[0.6rem] text-muted">
  ...
  },
  {
    ...
    visual: (
      <div className="flex flex-col gap-3">
        <div className="mb-1 flex items-center justify-between border-b border-border pb-2">
          <span className="font-[family-name:var(--font-mono)] text-[0.6rem] uppercase tracking-wider text-muted">Review — Priya Sharma</span>
  ...
  },
  {
    ...
    visual: (
      <div className="flex flex-col gap-2.5">
        <div className="mb-1 flex items-center justify-between border-b border-border pb-2">
          <span className="font-[family-name:var(--font-mono)] text-[0.6rem] uppercase tracking-wider text-muted">AI Assistant</span>
          <span className="h-1.5 w-1.5 rounded-full bg-[#00D68F]" />
        </div>
        ...
        <div className="mr-auto max-w-[85%] rounded-xl rounded-bl-sm bg-border px-3 py-2 text-xs text-foreground">
  ...
  },
  {
    ...
    visual: (
      <div className="flex flex-col gap-2.5">
        <div className="mb-1 flex items-center justify-between border-b border-border pb-2">
          <span className="font-[family-name:var(--font-mono)] text-[0.6rem] uppercase tracking-wider text-muted">Kudos Feed</span>
        ...
        {[
          ...
          <div key={i} className="rounded-lg border border-border bg-[rgba(255,255,255,0.02)] p-2.5">
            <div className="flex justify-between text-[0.6rem]">
              <span className="text-foreground"><strong>{k.from}</strong> → {k.to}</span>
              <span className="text-muted">{k.time}</span>
            </div>
            <p className="my-1 text-xs text-muted">{k.msg}</p>
...
function HomeFaq({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="border-b border-border last:border-b-0">
      <button className="flex w-full items-center justify-between gap-4 py-5 text-left text-sm font-semibold text-foreground transition-colors hover:text-[#A29BFE]" onClick={() => setOpen(!open)} aria-expanded={open}>
        {q}
        <ChevronDown size={18} className="flex-shrink-0 text-muted transition-transform duration-300" style={{ transform: open ? "rotate(180deg)" : "rotate(0deg)" }} />
      </button>
    ...
    <div className="grid gap-8 lg:grid-cols-2 lg:items-start">
      <div>
        <div className="mb-6 flex flex-wrap gap-2">
          {featureTabs.map((t, i) => (
            <button key={t.id} onClick={() => setActive(i)} className={`flex items-center gap-2 rounded-lg border px-4 py-2.5 text-sm font-medium transition-all ${i === active ? "border-[#6C5CE7] bg-[#6C5CE7]/10 text-foreground" : "border-border bg-transparent text-muted hover:border-muted-2"}`}>
              {t.icon} {t.label}
            </button>
          ))}
        </div>
        <h3 className="mb-3 font-[family-name:var(--font-syne)] text-2xl font-bold text-foreground">{tab.title}</h3>
        <p className="mb-6 text-sm leading-relaxed text-muted">{tab.desc}</p>
        <Link href={`/features#${tab.id === "performance" ? "scores" : tab.id}`} className="inline-flex items-center gap-2 text-sm font-semibold text-[#A29BFE] transition-colors hover:text-foreground">
          Learn more <ArrowRight size={14} />
        </Link>
      </div>
      <div className="rounded-2xl border border-border bg-surface-2 p-5">{tab.visual}</div>
    </div>
  );
}
    ...
    <div className="marketing-page relative min-h-screen bg-background font-[family-name:var(--font-outfit)]">
      ...
      <header className="relative overflow-hidden pb-24 pt-40">
        ...
        <div className="relative z-[1] mx-auto max-w-[1200px] px-6 text-center">
          ...
          <FadeIn delay={0.45}>
            ...
            <p className="mt-5 text-xs text-muted">Free 14-day trial. No credit card required. Setup in 10 minutes.</p>
          </FadeIn>
        </div>
      </header>
      ...
      <section className="border-y border-border py-10">
        <div className="mx-auto max-w-[1200px] px-6">
          <FadeIn>
            <p className="mb-6 text-center text-xs uppercase tracking-[3px] text-muted">Trusted by teams at</p>
          </FadeIn>
          <StaggerContainer className="flex flex-wrap items-center justify-center gap-10 opacity-40">
            {["ScaleOps", "FinEdge", "LogiFleet", "BrightPath", "Apex Realty", "MedCare", "NovaTech", "UrbanGrid"].map((co) => (
              <StaggerItem key={co}>
                <span className="font-[family-name:var(--font-syne)] text-lg font-bold tracking-tight text-muted">{co}</span>
              </StaggerItem>
            ))}
          </StaggerContainer>
        </div>
        ...
        <StaggerContainer className="mx-auto grid max-w-[1200px] grid-cols-2 gap-5 px-6 md:grid-cols-4">
          {[
            ...
            <StaggerItem key={m.label}>
              <div className="flex items-center gap-4 rounded-xl border border-border bg-surface p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
            ...
            {[
              ...
              <StaggerItem key={card.title}>
                <div className="mkt-card transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
                  <h3 className="mb-2 font-[family-name:var(--font-syne)] text-lg font-bold text-foreground">{card.title}</h3>
      ...
      <section className="pb-28">
        <div className="mx-auto max-w-[1200px] px-6">
          ...
          <StaggerContainer className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              <StaggerItem key={m.title}>
                <div className="flex items-center gap-3 rounded-xl border border-border bg-surface p-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:border-[rgba(108,92,231,0.4)]">
          ...
          <FadeIn className="mt-8 text-center" delay={0.3}>
            <Link href="/features" className="inline-flex items-center gap-2 text-sm font-semibold text-[#A29BFE] transition-colors hover:text-foreground">Explore all features <ArrowRight size={16} /></Link>
          </FadeIn>
        </div>
      </section>
          ...
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {testimonials.map((t, i) => (
              <SlideIn key={t.name} from={i % 2 === 0 ? "left" : "right"} delay={i * 0.1}>
                <div className="flex flex-col rounded-2xl border border-border bg-surface p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
                  <Quote size={24} className="mb-4 text-[#6C5CE7]/30" />
                  <p className="mb-6 flex-1 text-sm leading-relaxed text-muted">&ldquo;{t.text}&rdquo;</p>
              ...
              <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
                <div>
                  <FadeIn>
                    ...
                    <p className="mb-8 text-sm leading-relaxed text-muted">Companies using WorkwrK report dramatic improvements in visibility, efficiency, and team culture within the first 90 days.</p>
                  </FadeIn>
                ...
                <FadeIn delay={0.2}>
                  <div className="flex flex-col gap-4">
                    <div className="rounded-xl border border-border bg-surface-2 p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
                      <Quote size={20} className="mb-3 text-[#6C5CE7]/30" />
                      <p className="mb-4 text-sm leading-relaxed text-muted">&ldquo;We went from managing 80 people across 4 spreadsheets to one dashboard. Performance reviews that took 2 weeks now take 2 days.&rdquo;</p>
                      <div className="flex items-center gap-3">
                        <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#6C5CE7]/20 text-xs font-bold text-[#A29BFE]">AM</div>
                        <div>
                          <div className="text-sm font-medium text-foreground">Arjun Mehta</div>
                          <div className="text-xs text-muted">CEO, ScaleOps India</div>
                        </div>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="rounded-xl border border-border bg-surface-2 p-4 text-center transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
                        <div className="text-gradient-purple font-[family-name:var(--font-syne)] text-3xl font-extrabold">500+</div>
                        <div className="text-xs text-muted">Businesses</div>
                      </div>
                      <div className="rounded-xl border border-border bg-surface-2 p-4 text-center transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
                        <div className="text-gradient-purple font-[family-name:var(--font-syne)] text-3xl font-extrabold">15k+</div>
                        <div className="text-xs text-muted">Employees managed</div>
                      </div>
                    </div>
                  </div>
                </FadeIn>
          ...
          <div className="grid gap-12 lg:grid-cols-[1fr_2fr] lg:items-start">
            <FadeIn>
              <div>
                ...
                <Link href="/faq" className="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-[#A29BFE] transition-colors hover:text-foreground">See all FAQs <ArrowRight size={14} /></Link>
              </div>
            </FadeIn>
            <FadeIn delay={0.2}>
              ...
              <div className="mt-4 text-right">
                <Link href="/faq" className="inline-flex items-center gap-2 text-sm font-semibold text-[#A29BFE] transition-colors hover:text-foreground">See all FAQs <ArrowRight size={14} /></Link>
              </div>
            </FadeIn>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-[1200px] px-6">
          <ScaleIn>
            <div className="mkt-highlight text-center">
              ...
              <FadeIn delay={0.3}>
                <p className="mx-auto mb-8 max-w-[460px] text-base text-muted">Join 500+ businesses that replaced spreadsheets, WhatsApp groups, and guesswork with one intelligent platform.</p>
              </FadeIn>
              <FadeIn delay={0.45}>
                <div className="flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
                  <Link href="/register" className="btn-primary px-8 py-3.5 text-base">Start Free Trial <ArrowUpRight size={16} /></Link>
                ...
                <p className="relative mt-4 text-xs text-muted">Free 14-day trial. No credit card. Setup in 10 minutes.</p>
              </FadeIn>
            </div>
          </ScaleIn>
        </div>
