  ...
  if (!product) {
    return (
      <div className="p-8 max-w-xl mx-auto">
        ...
        <h1 className="text-lg font-semibold mb-1">Product not found</h1>
        <p className="text-xs text-muted-2">No product registered for slug &ldquo;{slug}&rdquo;.</p>
      </div>
    );
  }
      ...
      <div className="rounded-2xl border border-dashed border-violet-300/40 dark:border-violet-700/40 bg-gradient-to-br from-violet-50/40 to-transparent dark:from-violet-950/20 p-8">
        <div className="flex items-start gap-4 mb-5">
          ...
          <div className="flex-1 min-w-0">
            <div className="inline-flex items-center gap-2 px-2 py-0.5 rounded-full bg-violet-100 dark:bg-violet-900/40 text-violet-700 dark:text-violet-300 text-micro font-semibold uppercase tracking-wider mb-2">
        ...
        {product.seededAgents && product.seededAgents.length > 0 && (
          <div className="mb-5">
            <p className="text-micro font-semibold uppercase tracking-wider text-muted-2 mb-1.5">Ships with these agents</p>
            <div className="flex flex-wrap gap-1.5">
              {product.seededAgents.map((a) => (
                <span key={a} className="text-xs px-2 py-0.5 rounded-full bg-surface-2 text-foreground/80 font-mono">
                  {a}
                </span>
              ))}
            </div>
        ...
        {product.seededTemplates && product.seededTemplates.length > 0 && (
          <div className="mb-5">
            <p className="text-micro font-semibold uppercase tracking-wider text-muted-2 mb-1.5">Starter templates</p>
            <div className="flex flex-wrap gap-1.5">
              {product.seededTemplates.map((t) => (
                <span key={t} className="text-xs px-2 py-0.5 rounded-full bg-surface-2 text-foreground/80">
                  {t}
                </span>
              ))}
            </div>
