...
export function WeeklyReviewForm({ initialReview, kras }: FormProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* KRA progress */}
      <section>
        <h2 className="text-xs uppercase tracking-wide text-muted mb-2">KRA progress</h2>
        {kras.length === 0 ? (
          <div className="text-sm text-muted px-4 py-3 border border-border rounded-md bg-surface">
          ...
          <ul className="space-y-2">
            {kras.map((k) => {
              ...
              return (
                <li key={k.id} className="rounded-md border border-border bg-surface px-4 py-3">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="text-sm font-medium flex-1 truncate">{k.name}</span>
                    {k.category ? <span className="text-[10px] uppercase tracking-wide text-muted">{k.category}</span> : null}
                    <span className="text-xs text-muted">{entry.progressPct}%</span>
                  </div>
                  <input
                    type="range"
                    min={0}
                  ...
                  <input
                    ...
                    className="w-full mt-2 px-2 py-1 rounded-md border border-border bg-surface text-sm focus:outline-none focus:border-[var(--os-brand)] disabled:opacity-60"
                  />
                </li>
              );
            })}
      ...
      {/* KPI snapshots */}
      <section>
        <h2 className="text-xs uppercase tracking-wide text-muted mb-2">KPI snapshots</h2>
          ...
          <ul className="space-y-1">
            {kras.flatMap((k) =>
              k.kpis.map((kpi) => {
                ...
                return (
                  <li key={kpi.id} className="flex items-center gap-3 px-4 py-2 rounded-md border border-border bg-surface">
                    ...
                    <input
                      ...
                      className="w-24 h-8 px-2 rounded-md border border-border bg-surface text-sm text-right focus:outline-none focus:border-[var(--os-brand)] disabled:opacity-60"
                    />
                  </li>
                );
              }),
        ...
        {review.status === "DRAFT" ? (
          <>
            <button
              ...
              disabled={busy !== null}
              className="inline-flex items-center gap-1.5 h-9 px-3 rounded-md text-sm border border-border hover:bg-surface-2 disabled:opacity-50"
            >
...
function NarrativeField({
  ...
  return (
    <div className="rounded-md border border-border bg-surface px-3 py-3">
      ...
      <div className="text-[11px] text-muted mb-2">{subtitle}</div>
      <textarea
        rows={5}
        value={value}
...
function StatusBanner({ review }: { review: WeeklyReviewDoc }) {
  if (review.status === "DRAFT") {
    return (
      <div className="rounded-md border border-border bg-surface px-4 py-3 flex items-center gap-2 text-sm">
        <Clock className="w-4 h-4 text-muted" />
        <span className="text-muted">Draft — not yet submitted to your manager.</span>
      </div>
    );
  }
  ...
  return (
    <div className={`rounded-md px-4 py-3 flex items-center gap-2 text-sm ${
      ...
      <div className="flex-1">
        ...
        {review.managerNotes ? (
          <div className="text-xs text-muted mt-0.5 whitespace-pre-wrap">{review.managerNotes}</div>
        ) : null}
      </div>
    </div>
  );
