  ...
  return (
    <div className="space-y-5">
      ...
      <section>
        ...
        {kras.length === 0 ? (
          <div className="text-xs text-zinc-500 px-4 py-3 border border-zinc-200 rounded-md bg-white">
              ...
              return (
                <li key={k.id} className="rounded-md border border-zinc-200 bg-white px-4 py-3">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="text-xs font-medium flex-1 truncate">{k.name}</span>
                    {k.category ? <span className="text-micro uppercase tracking-wide text-zinc-500">{k.category}</span> : null}
                  ...
                  <input
                    ...
                    className="w-full mt-2 px-2 py-1 rounded-md border border-zinc-200 bg-white text-xs focus:outline-none focus:border-[var(--os-brand)] disabled:opacity-60"
                  />
                </li>
              );
            })}
                ...
                return (
                  <li key={kpi.id} className="flex items-center gap-3 px-4 py-2 rounded-md border border-zinc-200 bg-white">
                    <div className="flex-1 min-w-0">
                      <div className="text-xs truncate">{kpi.name}</div>
                    ...
                    <input
                      ...
                      className="w-24 h-8 px-2 rounded-md border border-zinc-200 bg-white text-xs text-right focus:outline-none focus:border-[var(--os-brand)] disabled:opacity-60"
                    />
                  </li>
                );
              }),
        ...
        {review.status === "DRAFT" ? (
          <>
            <button
              ...
              disabled={busy !== null}
              className="inline-flex items-center gap-1.5 h-9 px-3 rounded-md text-xs border border-zinc-200 hover:bg-zinc-50 disabled:opacity-50"
            >
  ...
  return (
    <div className="rounded-md border border-zinc-200 bg-white px-3 py-3">
      ...
      <textarea
        ...
        className="w-full text-xs bg-transparent outline-none resize-y disabled:opacity-60"
      />
    </div>
  );
}
...
function StatusBanner({ review }: { review: WeeklyReviewDoc }) {
  if (review.status === "DRAFT") {
    return (
      <div className="rounded-md border border-zinc-200 bg-white px-4 py-3 flex items-center gap-2 text-xs">
  ...
  if (review.status === "SUBMITTED") {
    return (
      <div className="rounded-md border border-amber-500/40 bg-amber-500/10 px-4 py-3 flex items-center gap-2 text-xs">
        <Clock className="w-4 h-4 text-amber-700" />
        <span className="flex-1">Submitted — awaiting manager review.</span>
        {review.managerStatus === "PENDING" ? <span className="text-xs text-amber-700">Pending</span> : null}
      </div>
    );
  }
  // ACKNOWLEDGED
  ...
  return (
    <div className={`rounded-md px-4 py-3 flex items-center gap-2 text-xs ${
