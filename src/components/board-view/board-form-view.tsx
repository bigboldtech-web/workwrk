        ...
        <div className="max-w-md mx-auto text-center">
          ...
          {canEdit ? (
            <div className="flex items-center justify-center gap-2 flex-wrap">
              ...
              {forms === null ? (
                <span className="text-sm text-zinc-400">Loading forms…</span>
              ) : forms.length > 0 ? (
                <select
                  defaultValue=""
                  onChange={(e) => { if (e.target.value) { persistFormId(e.target.value); setPicking(false); } }}
                  className="h-8 rounded-lg border border-zinc-200 bg-white px-2 text-base text-zinc-700 outline-none focus:border-zinc-400"
                >
              ...
              {picking ? (
                <button
                  ...
                  className="h-8 px-3 rounded-lg text-base text-zinc-500 hover:bg-zinc-50"
                >
                  Cancel
                </button>
              ) : null}
            ...
            <p className="text-sm text-zinc-400">Ask a List editor to connect a form.</p>
          )}
        </div>
      </div>
    );
  ...
  return (
    <div className="rounded-lg border border-zinc-200 bg-white overflow-hidden">
      <div className="px-3 py-2 border-b border-zinc-100 flex items-center gap-2">
        ...
        <span className="text-base font-medium text-zinc-800">Live form</span>
