    ...
    <>
      ...
      <aside role="dialog" aria-modal="true" aria-label="Task statuses" className="fixed top-0 right-0 bottom-0 z-50 w-[400px] max-w-full bg-white border-l border-zinc-200 shadow-2xl flex flex-col">
        <div className="px-4 py-3 border-b border-zinc-200 flex items-center gap-2">
          <span className="text-xs font-semibold text-zinc-900">Task statuses</span>
          <span className="text-xs text-zinc-400">{draft.length} of 30</span>
          <button
            type="button"
            onClick={onClose}
            ...
            {draft.map((s, idx) => (
              ...
              >
                ...
                <input
                  ...
                  className="flex-1 min-w-0 h-7 px-1.5 rounded text-base bg-transparent outline-none hover:bg-zinc-50 focus:bg-white focus:ring-1 focus:ring-zinc-300 disabled:opacity-60"
                />
                <select
                  value={s.group}
                  disabled={!canEdit}
                  onChange={(e) => patchRow(idx, { group: e.target.value as StatusGroup })}
                  className="h-7 rounded border border-zinc-200 bg-white px-1 text-xs text-zinc-600 outline-none focus:border-zinc-400 disabled:opacity-60"
          ...
          {canEdit ? (
            <div className="mt-3 flex items-center gap-1.5">
              <input
                ...
                className="flex-1 min-w-0 h-8 px-2 rounded-lg border border-zinc-200 text-base outline-none focus:border-zinc-400"
              />
              <select
                value={newGroup}
                onChange={(e) => setNewGroup(e.target.value as StatusGroup)}
                className="h-8 rounded-lg border border-zinc-200 bg-white px-1 text-xs text-zinc-600 outline-none focus:border-zinc-400"
              >
                {(Object.keys(GROUP_META) as StatusGroup[]).map((g) => (
                  <option key={g} value={g}>{GROUP_META[g].label}</option>
                ))}
        ...
        {canEdit ? (
          <div className="px-4 py-3 border-t border-zinc-200 flex items-center gap-2">
            <button
              ...
              disabled={busy}
              className="inline-flex items-center gap-1.5 h-8 px-3 rounded-lg text-base text-zinc-500 hover:text-zinc-800 hover:bg-zinc-50 disabled:opacity-50"
