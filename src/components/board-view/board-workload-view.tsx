  ...
  if (initialItems.length === 0) {
    return (
      <div className="rounded-lg border border-zinc-200 bg-white px-8 py-14 text-center">
        ...
        <p className="text-base text-zinc-500">No items yet — assign work to see {variant === "team" ? "the team board" : "workload"}.</p>
      </div>
    );
  }
              ...
              {b.rows.map((it) => {
                ...
                return (
                  <button
                    ...
                    className="block w-full text-left rounded-md border border-zinc-200 bg-white px-3 py-2 text-xs hover:shadow-sm transition-shadow"
                  >
      ...
      <div className="flex items-center gap-2">
        {bucket.owner ? (
          <>
            ...
            <span className="text-base font-medium text-zinc-800 truncate">
              {`${bucket.owner.firstName ?? ""} ${bucket.owner.lastName ?? ""}`.trim()}
            </span>
          </>
        ) : (
          <span className="text-base text-zinc-500">Unassigned</span>
        )}
        <span className="text-xs text-zinc-400 tabular-nums">{bucket.rows.length}</span>
      </div>
