  ...
  if (initialItems.length === 0) {
    return (
      <div className="rounded-lg border border-zinc-200 bg-white px-8 py-14 text-center">
        ...
        <p className="text-base text-zinc-500">No items yet — schedule work with start/due dates to see the timeline.</p>
      </div>
    );
  }
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[860px]">
          ...
          <div className="grid border-b border-zinc-200" style={{ gridTemplateColumns: `160px repeat(${WEEK_COUNT}, 1fr)` }}>
            <div className="px-3 py-2 text-xs font-medium text-zinc-400 uppercase tracking-wide">Status</div>
            {weeks.map((w, i) => (
              <div
                key={i}
                className={`px-1.5 py-2 text-xs tabular-nums border-l border-zinc-100 ${
          ...
          {packedLanes.length === 0 ? (
            <div className="px-4 py-10 text-center text-base text-zinc-500">
                ...
                <div className="relative border-l border-zinc-100" style={{ height: 10 + lane.subCount * 26 }}>
                  ...
                  {lane.rows.map((r) => (
                    <button
                      ...
                      className="absolute rounded text-left text-xs font-medium text-white px-1.5 truncate hover:brightness-95"
                      style={{
        ...
        <div className="px-3 py-2 border-t border-zinc-100 flex items-center gap-2 flex-wrap">
          ...
          {unscheduled.slice(0, 8).map((it) => (
            <button
              ...
              className="inline-flex items-center h-6 px-2 rounded-md border border-zinc-200 text-xs text-zinc-600 hover:bg-zinc-50 max-w-[200px] overflow-hidden"
              title={it.title}
            >
          ...
          {unscheduled.length > 8 ? (
            <span className="text-xs text-zinc-400">+{unscheduled.length - 8} more</span>
          ) : null}
        </div>
      ) : null}
      <ItemContextMenuHost
