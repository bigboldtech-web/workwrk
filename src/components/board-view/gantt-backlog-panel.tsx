...
function CountBadge({ count }: { count: number }) {
  return (
    <span className="ml-1 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-zinc-100 px-1 text-xs font-semibold tabular-nums text-zinc-500">
      {count}
    </span>
  );
}
  ...
  return (
    <aside className="w-[260px] shrink-0 border-l border-zinc-200 bg-white flex flex-col">
      <div className="flex h-[44px] items-center justify-between border-b border-zinc-200 pl-3 pr-2">
        <span className="text-sm font-semibold text-zinc-800 dark:text-zinc-100">Tasks</span>
        <button
          type="button"
              ...
              <li key={it.id}>
                ...
                >
                  ...
                  <button
                    ...
                    className="min-w-0 flex-1 truncate text-left text-sm font-medium text-zinc-800 hover:text-[var(--os-brand-ink)]"
                    title={it.title}
                  >
                    {it.title}
                  </button>
                  {tab === "unscheduled" && canEdit ? (
                    <button
                      ...
                      className="h-5 rounded px-1.5 text-xs font-semibold text-zinc-400 opacity-0 transition-opacity group-hover:opacity-100 hover:bg-zinc-100 hover:text-zinc-700"
                    >
                      Today
                    </button>
                  ) : null}
                  {tab === "overdue" ? (
                    <span className="shrink-0 text-xs tabular-nums text-rose-500">{daysLate}d late</span>
                  ) : null}
                </div>
              </li>
            );
