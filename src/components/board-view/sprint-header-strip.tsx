...
export function SprintHeaderStrip({ boardId, canEdit, sprint, items, statuses }: SprintHeaderStripProps) {
  ...
  return (
    <div className="mb-2 flex flex-wrap items-center gap-3 rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-1.5">
      ...
      <span
        className={`inline-flex items-center h-5 rounded-[5px] px-2 text-[10.5px] font-bold uppercase text-white ${
      ...
      <span className="text-[12px] text-zinc-500">Points</span>
      <span className="text-[12.5px] font-semibold text-zinc-900">{done} / {total}</span>
      ...
      <button
        ...
        className="inline-flex items-center gap-1.5 h-7 px-2.5 rounded-md text-[12px] text-zinc-600 border border-zinc-200 hover:bg-zinc-100"
      >
          ...
          <MorePortal anchorRef={anchorRef} panelRef={panelRef} width={260} open={editOpen} placement="below">
            <div className="flex flex-col gap-3 p-3">
              <div className="flex flex-col gap-1">
                <label className="text-[12px] font-medium text-zinc-600">Start date</label>
                <input
                  type="date"
                  value={draftStart}
                  onChange={(e) => setDraftStart(e.target.value)}
                  className="w-full h-8 px-2.5 text-[13px] bg-white border border-zinc-200 rounded-md"
                />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-[12px] font-medium text-zinc-600">End date</label>
                <input
                  type="date"
                  value={draftEnd}
              ...
              <button
                ...
                className="h-8 px-4 text-[12.5px] font-medium rounded-md text-white bg-zinc-900 hover:bg-zinc-800 disabled:opacity-50"
              >
                Save
              </button>
            </div>
