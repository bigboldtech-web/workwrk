  ...
  if (!locationField) {
    return (
      <div className="rounded-lg border border-zinc-200 bg-white px-8 py-14 text-center">
        ...
        <h3 className="text-base font-semibold text-zinc-900 mb-1">No Location field yet</h3>
        ...
        {groups.map(({ place, rows }) => (
          <div key={place} className="rounded-lg border border-zinc-200 bg-white p-3">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="flex-1 min-w-0 truncate text-base font-semibold text-zinc-900" title={place}>
                {place}
              </span>
              <span className="text-xs text-zinc-400 tabular-nums">{rows.length}</span>
              <a
                ...
                return (
                  <li key={it.id}>
                    <button
                      ...
                      className="flex w-full items-center gap-2 rounded px-1.5 py-1 text-left text-base text-zinc-700 hover:bg-zinc-50"
                    >
                      <span
