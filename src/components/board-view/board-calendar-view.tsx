  ...
  return (
    <section>
      ...
      <div className="mb-3 flex items-center gap-2">
        ...
        <h2 className="text-base font-semibold tracking-[-0.01em] text-zinc-900">{monthLabel}</h2>
        <button
          type="button"
          disabled={isCurrentMonth}
          onClick={() => setMonth({ y: now.getFullYear(), m: now.getMonth() })}
          className="inline-flex h-7 items-center rounded-md border border-zinc-200 bg-white px-2.5 text-xs font-medium text-zinc-700 transition-colors hover:bg-zinc-50 disabled:text-zinc-400 disabled:hover:bg-white dark:border-zinc-700 dark:bg-transparent dark:text-zinc-300 dark:hover:bg-white/5 dark:disabled:text-zinc-600"
        >
          Today
        </button>
        ...
        {dateFields.length > 0 ? (
          <label className="inline-flex items-center gap-1.5 text-xs text-zinc-500">
            ...
            <select
              ...
              className="h-7 rounded-md border border-zinc-200 bg-white px-2 text-xs text-zinc-700 transition-colors hover:border-zinc-300 focus:outline-none focus:border-[var(--os-brand)]"
            >
      ...
      <div className="rounded-xl border border-zinc-200 bg-white overflow-hidden">
        <div className="grid grid-cols-7 border-b border-zinc-200 [&>div:nth-child(7n)]:border-r-0">
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
            <div key={d} className="border-r border-zinc-100 px-1.5 py-1.5 text-xs font-medium text-zinc-500">
              {d}
            </div>
          ))}
        </div>
            ...
            return (
              ...
              >
                {cell.day !== null ? (
                  <div className="mb-1.5 flex items-center justify-between">
                    ...
                    <span className="inline-flex items-center gap-1">
                      {dayItems.length > 0 ? (
                        <span className="text-xs font-medium tabular-nums text-zinc-400">{dayItems.length}</span>
                      ) : null}
                ...
                <ul className="space-y-0.5">
                  ...
                  {dayItems.length > 4 ? (
                    <li className="px-1.5 pt-0.5 text-xs font-medium text-zinc-500 hover:text-zinc-700 cursor-default">+ {dayItems.length - 4} more</li>
                  ) : null}
                </ul>
              </div>
            );
