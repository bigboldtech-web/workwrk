...
export function BulkActionBar({
  ...
  const pill = "list-none cursor-pointer inline-flex items-center gap-1.5 h-7 px-3 rounded-full text-sm text-zinc-100 hover:bg-white/15 hover:text-white transition-colors select-none";
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 inline-flex items-center gap-1 rounded-full bg-zinc-900 text-white pl-1.5 pr-2 py-1.5 shadow-2xl shadow-zinc-950/30 ring-1 ring-white/15">
      <span className="inline-flex items-center h-7 px-3 rounded-full bg-[var(--os-brand)] text-sm font-semibold tabular-nums text-white">
        ...
        <div className="absolute left-0 bottom-full mb-2 w-[190px] rounded-xl border border-zinc-200 bg-white shadow-2xl py-1.5 text-zinc-900">
          {statuses.map((o) => (
            <button
              ...
              disabled={busy}
              className="w-full text-left flex items-center gap-2.5 px-3 py-1.5 text-base text-zinc-800 hover:bg-zinc-50 disabled:opacity-50 transition-colors"
            >
      ...
      <details className="relative">
        ...
        <div className="absolute left-0 bottom-full mb-2 w-[190px] rounded-xl border border-zinc-200 bg-white shadow-2xl py-1.5 text-zinc-900">
          ...
          <button type="button" onClick={() => onPriority(null)} disabled={busy} className="w-full text-left mt-1 px-3 py-1.5 text-base text-zinc-500 hover:bg-zinc-50 hover:text-zinc-700 disabled:opacity-50 transition-colors border-t border-zinc-100">Clear priority</button>
        </div>
      </details>
      <span className="w-px h-5 bg-white/15" aria-hidden />
...
function BulkDueDate({ onSet, busy }: { onSet: (iso: string | null) => void; busy: boolean }) {
  ...
  return (
    <details className="relative">
      <summary className="list-none cursor-pointer inline-flex items-center gap-1.5 h-7 px-3 rounded-full text-sm text-zinc-100 hover:bg-white/15 hover:text-white transition-colors select-none">
        Set due date
        <ChevronDown className="w-3 h-3 opacity-60" />
      </summary>
      <div className="absolute left-0 bottom-full mb-2 w-[260px] rounded-xl border border-zinc-200 bg-white shadow-2xl p-3 text-zinc-900">
        <label className="text-xs font-medium text-zinc-500 mb-1.5 block">Due date</label>
        <input
          type="date"
          value={value}
          onChange={(e) => setValue(e.target.value)}
          className="w-full h-8 px-2.5 border border-zinc-200 rounded-md text-base text-zinc-800 focus:outline-none focus:border-[var(--os-brand)]"
        />
        <div className="flex items-center gap-1.5 mt-2.5">
          ...
          <button
            ...
            className="h-7 px-3 rounded-md text-sm text-zinc-600 hover:bg-zinc-100 transition-colors"
          >
            Clear
          </button>
        </div>
    ...
    <details className="relative" onToggle={(e) => setOpen((e.currentTarget as HTMLDetailsElement).open)}>
      ...
      <div className="absolute left-0 bottom-full mb-2 w-[240px] max-h-[280px] overflow-y-auto rounded-xl border border-zinc-200 bg-white shadow-2xl py-1.5 text-zinc-900">
        <button
          ...
          disabled={busy}
          onClick={() => onSet(null)}
          className="w-full text-left flex items-center gap-2.5 mb-1 px-3 py-1.5 text-base text-zinc-500 hover:bg-zinc-50 hover:text-zinc-700 disabled:opacity-50 transition-colors border-b border-zinc-100"
        >
          Unassign
        </button>
        {users === null ? (
          ...
          <div className="px-3 py-4 text-sm text-zinc-400">No users</div>
        ) : (
          users.map((u) => (
            <button
              key={u.id}
              ...
              className="w-full text-left flex items-center gap-2.5 px-3 py-1.5 text-base text-zinc-800 hover:bg-zinc-50 disabled:opacity-50 transition-colors"
            >
              <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-zinc-200 text-xs font-medium text-zinc-600 shrink-0 overflow-hidden">
                {u.avatar ? (
