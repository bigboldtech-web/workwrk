    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        ...
        <div className="rounded-lg border border-zinc-200 bg-white p-4">
          <div className="text-base font-semibold text-zinc-900 mb-3">By person</div>
          {owners.length === 0 ? (
            <p className="text-sm text-zinc-400">No assigned items yet.</p>
          ) : (
            <ul className="space-y-2">
              {owners.slice(0, 8).map(({ owner, open, done }) => (
                <li key={owner.id} className="flex items-center gap-2 text-base">
              ...
              {owners.length > 8 ? (
                <li className="text-xs text-zinc-400">+{owners.length - 8} more</li>
              ) : null}
            </ul>
          )}
        </div>
...
function StatCard({ label, value, Icon, tint }: { label: string; value: number; Icon: React.ComponentType<{ className?: string; style?: React.CSSProperties }>; tint: string }) {
  return (
    <div className="rounded-lg border border-zinc-200 bg-white p-3" style={{ borderLeft: `3px solid ${tint}` }}>
      ...
      <div className="mt-1 text-xl font-semibold tabular-nums text-zinc-900 leading-none">{value}</div>
    </div>
  );
}
...
function BreakdownCard({ title, segs, total, emptyHint }: { title: string; segs: Seg[]; total: number; emptyHint?: string }) {
  return (
    <div className="rounded-lg border border-zinc-200 bg-white p-4">
      <div className="text-base font-semibold text-zinc-900 mb-3">{title}</div>
      {segs.length === 0 || total === 0 ? (
        <p className="text-sm text-zinc-400">{emptyHint ?? "Nothing here yet."}</p>
      ) : (
