      ...
      <div key={row.id}>
        ...
        >
          ...
          <button
            ...
            className="flex-1 min-w-0 text-left text-base text-zinc-800 truncate hover:text-[var(--os-brand)]"
            title={row.title}
          >
            {row.title}
          </button>
          {children.length > 0 ? (
            <span className="text-xs text-zinc-400 tabular-nums shrink-0">{children.length}</span>
          ) : null}
          {opt ? (
            <span
