    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
      {initialItems.map((it) => {
        ...
        return (
          ...
          >
            <div className="text-base font-medium text-zinc-900 break-words">{it.title}</div>
            {desc ? (
              <p className="mt-1 text-xs text-zinc-500 line-clamp-2">{desc}</p>
            ) : null}
            <div className="mt-2 flex items-center gap-1.5 flex-wrap">
              ...
              {tags.length > 2 ? <span className="text-xs text-zinc-500">+{tags.length - 2}</span> : null}
            </div>
