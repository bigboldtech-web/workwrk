        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 text-base font-semibold text-zinc-800"
            >
            ...
            {!fieldsCollapsed ? (
              <div className="flex items-center gap-1.5">
                <div className="inline-flex items-center gap-1.5 h-7 px-2 rounded-md border border-zinc-200">
                  ...
                  <input value={fieldSearch} onChange={(e) => setFieldSearch(e.target.value)} placeholder="Search fields…" className="w-[110px] text-sm bg-transparent outline-none" />
                </div>
                {emptyCount > 0 ? (
                  <button type="button" onClick={() => setHideEmpty((v) => !v)} className="inline-flex items-center gap-1 h-7 px-2 rounded-md text-xs text-zinc-500 hover:bg-zinc-100">
          ...
          {fieldsCollapsed ? null : shown.length === 0 ? (
            <p className="text-base text-zinc-400">{fieldSearch ? "No matching fields." : "All fields empty."}</p>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-10 gap-y-0.5">
              {shown.map((f) => (
        ...
        <div className="grid max-w-[420px] gap-0.5">
          {actionRows.map((r) => (
            <button
              ...
              type="button"
              onClick={r.onClick}
              className="flex h-8 items-center gap-2 rounded-md px-2 text-left text-base text-zinc-600 transition-colors hover:bg-zinc-50 hover:text-zinc-900"
            >
  ...
  return (
    <div className="flex min-h-[38px] items-center gap-3">
      <div className="flex w-[122px] shrink-0 items-center gap-2">
        ...
        <span className="truncate text-base text-zinc-500">{label}</span>
      </div>
  ...
  if (editing) {
    return (
      <input
        ...
        className="h-7 w-[110px] rounded-md border border-[var(--os-brand)] bg-white px-2 text-base outline-none placeholder:text-zinc-400"
      />
    );
  }

  if (!canEdit) {
    return minutes ? <span className="text-base text-zinc-700 tabular-nums">{formatMinutes(minutes)}</span> : <span className="text-base text-zinc-400">Empty</span>;
  }
...
function AlignmentField({ item, canEdit, onPatch }: { item: BoardItemRow; canEdit: boolean; onPatch: (b: DetailPatch) => void }) {
  ...
  const summary = (
    <span className="inline-flex flex-wrap items-center gap-1.5">
      ...
      {!kraId && !kpiId ? <span className="text-base text-zinc-400">Empty</span> : null}
    </span>
  );
        ...
        <>
          ...
          <div className="absolute z-20 mt-1 left-0 w-[300px] max-w-[calc(100vw-2rem)] rounded-xl border border-zinc-200 bg-white shadow-2xl p-2">
            <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-md border border-zinc-200 focus-within:border-[var(--os-brand)] mb-2">
              ...
              <input autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search KPIs or KRAs…" className="flex-1 text-base bg-transparent outline-none placeholder:text-zinc-400" />
            </div>
            <div className="max-h-[240px] overflow-y-auto">
              <div className="px-1 pb-1 text-xs font-medium text-zinc-400 uppercase tracking-wide">KPIs</div>
              {fKpis.length === 0 ? (
                <div className="px-2 py-1.5 text-sm text-zinc-400">{ready ? "No KPIs — pick a KRA below." : "Loading…"}</div>
              ) : (
                fKpis.map((k) => (
                  <button key={k.id} type="button" onClick={() => commit(k.kra?.id ?? null, k.id)} className="w-full flex items-center gap-2 px-2 py-1.5 text-left text-base hover:bg-zinc-50 rounded">
              ...
              {fKras.length === 0 ? (
                <div className="px-2 py-1.5 text-sm text-zinc-400">{ready ? "No KRAs available." : "Loading…"}</div>
              ) : (
                fKras.map((k) => (
                  <button key={k.id} type="button" onClick={() => commit(k.id, null)} className="w-full flex items-center gap-2 px-2 py-1.5 text-left text-base hover:bg-zinc-50 rounded">
            ...
            {(kraId || kpiId) ? (
              <button type="button" onClick={() => commit(null, null)} className="w-full flex items-center gap-2 px-2 py-1.5 mt-1 pt-1.5 text-left text-base text-zinc-500 hover:bg-zinc-50 rounded border-t border-zinc-100">
...
function TitleField({ item, canEdit, onSave }: { item: BoardItemRow; canEdit: boolean; onSave: (title: string) => void }) {
  ...
  if (!canEdit || !editing) {
    return <button type="button" onClick={() => canEdit && setEditing(true)} className="w-full text-left text-xl font-semibold leading-snug tracking-[-0.01em] text-zinc-900">{item.title}</button>;
  }
  return (
    <input
      autoFocus
      ...
      className="w-full text-xl font-semibold leading-snug tracking-[-0.01em] text-zinc-900 bg-transparent outline-none border-b border-[var(--os-brand)]"
    />
  );
}
  ...
  return (
    <div>
      ...
      <textarea
        ...
        className="w-full px-3 py-2 rounded-lg border border-transparent hover:border-zinc-200 bg-transparent text-xs leading-relaxed resize-y transition-colors focus:outline-none focus:border-[var(--os-brand)] focus:bg-white disabled:opacity-60 placeholder:text-zinc-400"
      />
    </div>
  );
}
...
function StatusPicker({ value, statuses, canEdit, onChange }: { value: string | null; statuses: StatusOption[]; canEdit: boolean; onChange: (v: string) => void }) {
  ...
  const pill = current ? (
    <span
      className={`inline-flex items-center h-5 ${showAdvance && canEdit ? "rounded-l-[5px]" : "rounded-[5px]"} px-2 text-micro font-semibold uppercase tracking-wider text-white whitespace-nowrap shrink-0`}
      style={{ background: current.color }}
    >
      {current.label}
    </span>
  ) : (
    <span className="text-base text-zinc-400">Empty</span>
  );
    ...
    <div className="relative inline-block">
      ...
      {open ? (
        <div className="absolute z-10 mt-1 left-0 min-w-[190px] rounded-xl border border-zinc-200 bg-white shadow-2xl py-1.5" onMouseLeave={() => setOpen(false)}>
          {statuses.map((opt) => (
            <button key={opt.value} type="button" onClick={() => { onChange(opt.value); setOpen(false); }} className="flex items-center gap-2 w-full px-2 py-1.5 text-left text-xs hover:bg-zinc-50">
