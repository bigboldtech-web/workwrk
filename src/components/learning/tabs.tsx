...
// Learning tab components — extracted for board-route conversion.
// Three boards: mine (my courses), catalog (browse + enroll), manage (admin).
...
export function MyCoursesTab() {
  ...
  if (loading) {
    return <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">{[1, 2, 3].map((i) => <SkeletonCard key={i} />)}</div>;
  }
  if (rows.length === 0) {
    return (
      <EmptyState
...
export function CatalogTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const ids = new Set((Array.isArray(enrollmentsData) ? enrollmentsData : []).map((e: Enrollment) => e.course.id));
      setEnrolledIds(ids);
    } finally { setLoading(false); }
  }, [search]);
  useEffect(() => { load(); }, [load]);
  ...
  async function enroll(courseId: string) {
    ...
    const data = await res.json();
    if (!res.ok) { toast({ type: "error", title: "Couldn't enroll", description: data?.error }); return; }
  ...
  return (
    <>
      <div className="mb-3">
        <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search courses…" className="max-w-sm" />
      </div>
      {loading ? (
              ...
              <CardContent className="p-4 space-y-3">
                <div>
                  <div className="flex items-start justify-between gap-2">
                    ...
                    {c.mandatory && (
                      <Badge variant="outline" className="text-[10px] text-orange-400 border-orange-400/30 flex-shrink-0">Mandatory</Badge>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-[10px] text-muted mt-2">
                    ...
                    {c.duration && (<span className="flex items-center gap-1"><Clock size={9} /> {c.duration} min</span>)}
                ...
                {enrolledIds.has(c.id) ? (
                  ...
                  <Button size="sm" className="w-full h-7 text-xs" onClick={() => enroll(c.id)}>Enroll</Button>
                )}
              </CardContent>
            </Card>
          ))}
  ...
  return (
    <>
      <div className="flex justify-end mb-3">
        <Button onClick={() => setCreating(true)}><Plus size={14} className="mr-1.5" /> New course</Button>
      </div>
      {loading ? (
                ...
                {courses.map((c) => (
                  <tr key={c.id} className="border-b border-white/5 hover:bg-surface-2">
                    ...
                    <td className="px-4 py-2.5 text-right">
                      <Button size="sm" variant="outline" className="h-7 text-xs text-red-400" onClick={() => deleteCourse(c.id, c.title)}>Delete</Button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {creating && (<CreateCourseDialog onClose={() => setCreating(false)} onCreated={() => { setCreating(false); load(); }} />)}
    </>
  );
}
    ...
    try {
      const res = await fetch("/api/courses", {
        ...
        body: JSON.stringify({
          ...
          duration: duration ? Number(duration) : undefined, mandatory,
        }),
      });
      const data = await res.json();
      ...
      <DialogContent>
        ...
        <div className="space-y-3 pt-1">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <label htmlFor="mandatory">Mandatory — surface in every employee&rsquo;s Inbox until completed</label>
          </div>
        </div>
        <DialogFooter>
