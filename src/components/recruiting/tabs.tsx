...
// Recruiting tab components — extracted from the original
// `/recruiting/page.tsx` so the per-board routes (`/recruiting/jobs`,
// `/candidates`, `/pipeline`, `/interviews`) can each render one tab
// inside the canonical BoardShell. The components themselves are
// unchanged — only the page shell + Tabs wrapper went away.
...
import {
  Briefcase, Users, GitBranch, Plus, Mail, Phone, ExternalLink, ChevronRight, UserPlus,
} from "lucide-react";
...
const STAGE_STYLE: Record<string, string> = {
  ...
  OFFER: "text-[color:var(--accent-strong)] border-violet-500/30",
...
const JOB_STATUS_STYLE: Record<string, string> = {
  ...
  OPEN: "text-[color:var(--accent-strong)] border-violet-500/30",
...
// Avoid an "unused" lint complaint while keeping the const exported-shape stable.
void STAGE_STYLE;
...
export function JobsTab() {
  ...
  return (
    <>
              ...
              <tbody>
                {rows.map((j) => (
                  <tr key={j.id} className="border-b border-white/5 hover:bg-surface-2">
                    ...
                    <td className="px-4 py-2.5">
                      <Badge variant="outline" className={`text-[10px] ${JOB_STATUS_STYLE[j.status]}`}>{j.status}</Badge>
                    </td>
                    <td className="px-4 py-2.5 text-right">
                      {j.status === "DRAFT" && (
                        <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => transition(j.id, "OPEN")}>Publish</Button>
                      )}
                      {j.status === "OPEN" && (
                        <>
                          <Button size="sm" variant="outline" className="h-7 text-xs mr-1" onClick={() => transition(j.id, "ON_HOLD")}>Hold</Button>
                          <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => transition(j.id, "CLOSED")}>Close</Button>
                        </>
                      )}
                      {j.status === "ON_HOLD" && (
                        <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => transition(j.id, "OPEN")}>Resume</Button>
                      )}
                    </td>
                  </tr>
                ))}
      ...
      {showCreate && <CreateJobDialog onClose={() => setShowCreate(false)} onCreated={() => { setShowCreate(false); load(); }} />}
    </>
  );
}
    ...
    try {
      const res = await fetch("/api/recruiting/jobs", {
        ...
        body: JSON.stringify({
          ...
          employmentType, location: location.trim() || undefined,
          openings: Number(openings) || 1,
          salaryMin: salaryMin || undefined, salaryMax: salaryMax || undefined, salaryCurrency,
        }),
      });
      const data = await res.json();
      if (!res.ok) {
      ...
      <DialogContent>
        ...
        <div className="space-y-3 pt-1">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5"><Label>Openings</Label><Input value={openings} onChange={(e) => setOpenings(e.target.value)} inputMode="numeric" /></div>
          </div>
          <div className="space-y-1.5"><Label>Location</Label><Input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="e.g. Remote, NYC" /></div>
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1.5"><Label>Currency</Label><Input value={salaryCurrency} onChange={(e) => setSalaryCurrency(e.target.value.toUpperCase().slice(0, 3))} maxLength={3} /></div>
            <div className="space-y-1.5"><Label>Salary min</Label><Input value={salaryMin} onChange={(e) => setSalaryMin(e.target.value)} inputMode="decimal" /></div>
            <div className="space-y-1.5"><Label>Salary max</Label><Input value={salaryMax} onChange={(e) => setSalaryMax(e.target.value)} inputMode="decimal" /></div>
          </div>
        ...
        <DialogFooter>
          ...
          <Button disabled={!title.trim() || saving} onClick={save}>{saving ? "Creating…" : "Create draft"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
export function CandidatesTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { toast({ type: "error", title: "Couldn't load candidates" }); return; }
      setRows(Array.isArray(data) ? data : []);
    } finally { setLoading(false); }
  }, [toast]);
  useEffect(() => { load(); }, [load]);
  ...
  return (
    <>
      <div className="flex justify-end mb-3">
        <Button onClick={() => setShowCreate(true)}><Plus size={14} className="mr-1.5" /> Add candidate</Button>
      </div>
      {loading ? (
        <div className="space-y-3">{[1, 2, 3].map((i) => <SkeletonCard key={i} />)}</div>
      ) : rows.length === 0 ? (
                ...
                {rows.map((c) => (
                  <tr key={c.id} className="border-b border-white/5 hover:bg-surface-2">
                    <td className="px-4 py-2.5">
                      ...
                      {c.hiredUserId && <div className="text-[10px] text-green-400">Hired</div>}
                    </td>
                    ...
                    <td className="px-4 py-2.5 text-right">
                      {c.resumeUrl && (
                        <a href={c.resumeUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-muted hover:text-fg mr-2 inline-flex items-center gap-1">
                          Resume <ExternalLink size={10} />
                        </a>
                      )}
                      <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => setShowApply(c)}>Apply to job</Button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {showCreate && <CreateCandidateDialog onClose={() => setShowCreate(false)} onCreated={() => { setShowCreate(false); load(); }} />}
      {showApply && <ApplyCandidateDialog candidate={showApply} onClose={() => setShowApply(null)} onCreated={() => { setShowApply(null); load(); }} />}
    </>
  );
}
    ...
    try {
      const res = await fetch("/api/recruiting/candidates", {
        ...
        body: JSON.stringify({
          ...
          phone: phone.trim() || undefined, source: source.trim() || undefined,
          resumeUrl: resumeUrl.trim() || undefined, notes: notes.trim() || undefined,
        }),
      });
      const data = await res.json();
      if (!res.ok) { toast({ type: "error", title: "Couldn't add", description: data?.error }); return; }
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        ...
        <div className="space-y-3 pt-1">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5"><Label>First name</Label><Input value={firstName} onChange={(e) => setFirstName(e.target.value)} autoFocus /></div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5"><Label>Source</Label><Input value={source} onChange={(e) => setSource(e.target.value)} placeholder="e.g. LinkedIn, Referral" /></div>
            <div className="space-y-1.5"><Label>Resume URL</Label><Input value={resumeUrl} onChange={(e) => setResumeUrl(e.target.value)} /></div>
          </div>
...
function ApplyCandidateDialog({ candidate, onClose, onCreated }: { candidate: Candidate; onClose: () => void; onCreated: () => void }) {
  const { toast } = useToast();
  ...
  async function save() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { toast({ type: "error", title: "Couldn't apply", description: data?.error }); return; }
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader><DialogTitle>Apply {candidate.firstName} {candidate.lastName} to a job</DialogTitle></DialogHeader>
        <div className="space-y-3 pt-1">
          <div className="space-y-1.5">
            <Label>Job</Label>
...
export function PipelineTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { toast({ type: "error", title: "Couldn't load pipeline" }); return; }
  ...
  if (loading) {
    return <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">{[1, 2, 3, 4, 5, 6].map((i) => <SkeletonCard key={i} />)}</div>;
  }
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
      {STAGES.map((s) => {
        ...
        return (
          <Card
            key={s}
            className={"min-h-[200px] transition-colors " + (isDropTarget ? "border-[color:var(--accent-strong)] bg-[color:var(--accent-soft)]" : "")}
            onDragOver={(e) => {
...
function ApplicationCard({ app, onMove, isDragging, onDragStart, onDragEnd }: {
  app: Application;
  ...
  return (
    <div
      ...
      onDragEnd={() => onDragEnd?.()}
      className={"rounded-md border border-white/10 bg-card-2/30 p-2 text-xs hover:border-white/20 transition-all cursor-grab active:cursor-grabbing " + (isDragging ? "opacity-40" : "")}
    >
      <div className="font-medium truncate">{app.candidate.firstName} {app.candidate.lastName}</div>
      <div className="text-[10px] text-muted truncate mt-0.5">{app.job.title}</div>
      {app.recruiter && (
        <div className="text-[10px] text-muted truncate mt-0.5">{app.recruiter.firstName} {app.recruiter.lastName}</div>
      )}
      {next && app.stage !== "HIRED" && app.stage !== "REJECTED" && (
        <div className="flex items-center gap-1 mt-2">
          <Button size="sm" variant="outline" className="h-6 text-[10px] px-1.5 flex-1" onClick={() => onMove(app.id, next)}>
            <ChevronRight size={10} className="mr-0.5" /> {STAGE_LABEL[next]}
          </Button>
          <Button size="sm" variant="outline" className="h-6 text-[10px] px-1.5 text-red-400" onClick={() => onMove(app.id, "REJECTED")}>
            Reject
          </Button>
        </div>
      )}
...
const INTERVIEW_TYPE_LABEL: Record<string, string> = {
  SCREEN: "Phone screen", TECHNICAL: "Technical", BEHAVIORAL: "Behavioral",
  ONSITE: "On-site", FINAL: "Final", OTHER: "Other",
};
...
export function InterviewsTab() {
  ...
  async function patch(id: string, body: Record<string, unknown>) {
    ...
    const data = await res.json();
    if (!res.ok) { toast({ type: "error", title: "Couldn't update", description: data?.error }); return; }
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-3">
        ...
        <Button onClick={() => setScheduling(true)}><Plus size={14} className="mr-1.5" /> Schedule</Button>
      </div>
      {loading ? (
        <div className="space-y-3">{[1, 2, 3].map((i) => <SkeletonCard key={i} />)}</div>
      ) : rows.length === 0 ? (
                  ...
                  return (
                    <tr key={iv.id} className="border-b border-white/5 hover:bg-surface-2">
                      <td className="px-4 py-2.5 text-xs">
                        ...
                        <div className="text-[10px] text-muted">
                          {when.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })} · {iv.durationMinutes}m
                        </div>
                      </td>
                      <td className="px-4 py-2.5 text-xs">{INTERVIEW_TYPE_LABEL[iv.type] ?? iv.type}</td>
                      <td className="px-4 py-2.5 text-xs">{iv.application.candidate.firstName} {iv.application.candidate.lastName}</td>
                      <td className="px-4 py-2.5 text-xs text-muted">{iv.application.job.title}</td>
                      <td className="px-4 py-2.5 text-xs">{iv.interviewer.firstName} {iv.interviewer.lastName}</td>
                      <td className="px-4 py-2.5">
                        <Badge variant="outline" className={`text-[10px] ${INTERVIEW_STATUS_STYLE[iv.status]}`}>{iv.status}</Badge>
                        {iv.score !== null && <span className="ml-2 text-[10px] text-muted">{iv.score}/5</span>}
                      </td>
                      <td className="px-4 py-2.5 text-right">
                            ...
                            <>
                              {isPast && (
                                <>
                                  ...
                                  <Button size="sm" variant="outline" className="h-7 text-xs text-amber-400" onClick={() => patch(iv.id, { status: "NO_SHOW" })}>No-show</Button>
                                </>
                              )}
                              <Button
      ...
      {scheduling && <ScheduleInterviewDialog onClose={() => setScheduling(false)} onScheduled={() => { setScheduling(false); load(); }} />}
    </>
  );
}

function ScheduleInterviewDialog({ onClose, onScheduled }: { onClose: () => void; onScheduled: () => void }) {
  const { toast } = useToast();
  ...
  useEffect(() => {
    Promise.all([
      ...
      const live = (Array.isArray(apps) ? apps : []).filter((a: Application) => !["HIRED", "REJECTED", "WITHDRAWN"].includes(a.stage));
  ...
  async function save() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { toast({ type: "error", title: "Couldn't schedule", description: data?.error }); return; }
      ...
      <DialogContent>
        ...
        <div className="space-y-3 pt-1">
            ...
            <Select value={applicationId} onValueChange={setApplicationId}>
              ...
              <SelectContent>
                {applications.length === 0 ? (
                  ...
                  <SelectItem key={a.id} value={a.id}>{a.candidate.firstName} {a.candidate.lastName} · {a.job.title}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="space-y-1.5"><Label>Duration (min)</Label><Input value={durationMinutes} onChange={(e) => setDurationMinutes(e.target.value)} inputMode="numeric" /></div>
          </div>
          <div className="space-y-1.5">
            <Label>Type</Label>
          ...
          <div className="space-y-1.5"><Label>Location (optional)</Label><Input value={location} onChange={(e) => setLocation(e.target.value)} placeholder="Zoom URL or office room" /></div>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose} disabled={saving}>Cancel</Button>
