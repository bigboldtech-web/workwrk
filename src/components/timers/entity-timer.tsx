...
export function EntityTimer({ entityType, entityId, compact }: Props) {
  ...
  if (compact) {
    return (
      <button
        ...
        disabled={loading}
        className={
          "inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-xs font-mono transition-colors " +
          (running
  ...
  return (
    <div className="inline-flex items-center gap-2 rounded-md border border-border bg-surface-2 px-2.5 py-1.5">
      ...
      {running && <span className="text-micro text-emerald-600 uppercase tracking-wider">live</span>}
    </div>
  );
}
