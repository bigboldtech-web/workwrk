  ...
  function renderField(f: PlacedField) {
    ...
    return (
      <div
        ...
        className={`group absolute flex cursor-move items-center justify-center rounded text-[12px] font-medium ${isSel ? "ring-2 ring-blue-400" : ""}`}
      ...
      >
        ...
        <span className="pointer-events-none absolute -left-1.5 -top-2 flex h-4 min-w-4 items-center justify-center rounded-full px-1 text-[10px] font-semibold text-white" style={{ background: color }}>{partyIdx + 1}</span>
        ...
        {isSel && (
          <div onMouseDown={(e) => e.stopPropagation()} onClick={(e) => e.stopPropagation()}
            ...
            <div className="mb-1.5 flex items-center justify-between">
              <span className="text-[12px] font-semibold text-zinc-500">{fieldLabel(f.type)} · {party?.name ?? "Party"}</span>
            ...
            {f.type === "dropdown" && (() => {
              ...
              return (
                <div className="mb-2">
                  <div className="mb-1 text-[13px] font-medium text-zinc-600">Options</div>
                  <div className="space-y-1.5">
                    {opts.map((o, i) => (
                      <div key={i} className="flex items-center gap-1.5">
                        <input value={o} onChange={(e) => setOpts(opts.map((x, j) => (j === i ? e.target.value : x)))} placeholder={`Option ${i + 1}`}
                          className="h-7 flex-1 rounded border border-zinc-200 px-2 text-[13px] outline-none focus:border-zinc-300" />
                  ...
                  <button type="button" onClick={() => setOpts([...opts, `Option ${opts.length + 1}`])} className="mt-1.5 inline-flex items-center gap-1 text-[13px] font-medium text-blue-600 hover:text-blue-700"><Plus className="h-3 w-3" /> Add option</button>
                </div>
              );
            })()}

            <label className="flex cursor-pointer items-center justify-between gap-2 text-[13px] text-zinc-700">
              Required
            ...
            {(f.type === "text" || f.type === "email" || f.type === "dropdown") && (
              <div className="mt-2">
                <div className="mb-1 text-[13px] font-medium text-zinc-600">Label</div>
                <input value={f.label ?? ""} onChange={(e) => patchField(f.id, { label: e.target.value })} placeholder="Label / placeholder"
                  className="h-7 w-full rounded border border-zinc-200 px-2 text-[13px] outline-none focus:border-zinc-300" />
              </div>
            )}

            {f.type === "dropdown" && (
              <div className="mt-2">
                <div className="mb-1 text-[13px] font-medium text-zinc-600">Default value</div>
                <select value={f.defaultValue ?? ""} onChange={(e) => patchField(f.id, { defaultValue: e.target.value })}
                  className="h-8 w-full rounded border border-zinc-200 bg-white px-2 text-[13px] text-zinc-700 outline-none">
            ...
            <button type="button" onClick={() => removeField(f.id)} className="mt-2.5 inline-flex items-center gap-1 text-[13px] text-red-500 hover:text-red-600"><Trash2 className="h-3 w-3" /> Delete field</button>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="flex gap-4">
      ...
      <aside className="w-56 shrink-0">
        <div className="rounded-xl border border-zinc-200 bg-white p-3">
          <div className="mb-2 flex items-center justify-between">
            <span className="text-[13px] font-semibold text-zinc-500">Parties</span>
            <span className="rounded bg-zinc-100 px-1.5 text-[12px] text-zinc-500">{parties.length}</span>
          </div>
          <div className="space-y-1.5">
            {parties.map((p) => {
                  ...
                  {editing ? (
                    <>
                      <input autoFocus value={draftName} onClick={(e) => e.stopPropagation()}
                        ...
                        className="min-w-0 flex-1 rounded border border-blue-300 px-2 py-1 text-[14px] font-medium text-zinc-800 outline-none" />
                      <button type="button" onClick={(e) => { e.stopPropagation(); commit(); }} className="rounded-md bg-blue-600 px-2.5 py-1 text-[13px] font-medium text-white hover:bg-blue-500">Save</button>
                    </>
                  ) : (
                    <>
                      <span className="min-w-0 flex-1 truncate text-[14px] font-medium text-zinc-800">{p.name}</span>
          ...
          <button type="button" onClick={onAddParty} className="mt-2 inline-flex w-full items-center justify-center gap-1.5 rounded-lg border border-dashed border-blue-300 px-2 py-1.5 text-[14px] font-medium text-blue-600 hover:bg-blue-50">
            <Plus className="h-3.5 w-3.5" /> Add new party
          </button>
        </div>
        <p className="mt-2 px-1 text-[12px] leading-relaxed text-zinc-400">Click a party to select it, then drag a field from the right onto the document. Double-click a party to rename it.</p>
      </aside>
      ...
      <aside className="w-52 shrink-0">
        <div className="rounded-xl border border-zinc-200 bg-white p-3">
          <div className="mb-2 flex items-center justify-between">
            <span className="text-[13px] font-semibold text-zinc-500">Fields</span>
            {selected ? <span className="inline-flex items-center gap-1 rounded-full px-1.5 text-[12px] font-medium text-white" style={{ background: partyColor(parties, selected) }}>{parties.findIndex((p) => p.id === selected) + 1}</span> : null}
          </div>
          {!selected ? <div className="mb-2 rounded-md bg-amber-50 px-2 py-1.5 text-[12px] text-amber-700">Add &amp; select a party first.</div> : null}
          <div className="space-y-1.5">
            {FIELD_TOOLS.map((t) => (
              <div key={t.type}
                draggable={!!selected}
                ...
                className={`flex items-center gap-2 rounded-lg border border-zinc-200 px-2.5 py-2 text-[14px] text-zinc-700 ${selected ? "cursor-grab hover:border-blue-300 hover:bg-blue-50 active:cursor-grabbing" : "cursor-not-allowed opacity-50"}`}
                title={selected ? "Drag onto the document" : "Select a party first"}>
                <t.Icon className="h-4 w-4 text-zinc-400" /> {t.label}
              </div>
            ))}
          </div>
          <p className="mt-2 text-[12px] leading-relaxed text-zinc-400">Drag a field onto the page. Click a placed field to set Required or delete it.</p>
        </div>
      </aside>
    </div>
  );
