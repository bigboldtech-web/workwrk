    ...
    <div className="flex flex-col rounded-lg border border-zinc-200 bg-white shadow-sm p-3 gap-2.5">
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          <Link
            href={`/people/${m.id}`}
            className="block text-base font-semibold text-zinc-900 truncate hover:text-[#0073EA]"
          >
            {m.name}
          </Link>
          <div className="text-xs text-zinc-500 truncate">{m.roleTitle ?? m.department ?? "—"}</div>
        </div>
        <div className="flex items-center gap-3 shrink-0 text-center">
          <div>
            <div className="text-base font-semibold tabular-nums text-zinc-900">{m.open}</div>
            <div className="text-xs text-zinc-400">Not done</div>
          </div>
          <div>
            <div className="text-base font-semibold tabular-nums text-zinc-900">{m.done}</div>
      ...
      {m.total > 0 && segs.length > 0 ? (
        ...
        <div className="text-xs text-zinc-400 py-1">No open work</div>
      )}

      {/* Working on now */}
      <div>
        <div className="text-micro font-semibold uppercase tracking-wide text-zinc-400 mb-1">Working on</div>
        {m.inProgress.length === 0 ? (
          <div className="text-xs text-zinc-400 px-1.5">Nothing in progress</div>
        ) : (
          m.inProgress.map((it) => {
            ...
            return (
              <Link key={it.id} href={`/item/${it.id}`} className="flex h-7 items-center gap-2 rounded-md px-1.5 hover:bg-zinc-50">
                <span
                  ...
                  aria-hidden
                />
                <span className="min-w-0 flex-1 truncate text-base text-zinc-700">{it.title}</span>
                <span className="max-w-[90px] truncate text-xs text-zinc-400">{it.board.name}</span>
                {isOverdue ? <span className="shrink-0 text-xs font-medium text-red-600">overdue</span> : null}
              </Link>
            );
          })
        )}
      ...
      {m.recent.length > 0 ? (
        <div className="border-t border-zinc-100 pt-2 space-y-1">
          {m.recent.map((r) => (
            <div key={r.id} className="flex items-baseline gap-2 text-xs text-zinc-400">
...
export function TeamPulse() {
  ...
  if (error) {
    return <div className="py-8 text-center text-sm text-zinc-400">Couldn&apos;t load your team</div>;
  }
  if (members === null) {
    return (
  ...
  if (members.length === 0) {
    return <div className="py-8 text-center text-sm text-zinc-400">No teammates visible to you yet</div>;
  }
