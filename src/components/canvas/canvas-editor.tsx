 ...
 *
 * ONE CANVAS, THREE ADDRESSES. This is the body of /canvas/[id] (the Docs
 * hub) and of the Work addresses /spaces/[slug]/canvas/[id] and
 * /work/canvas/[id], which a canvas opened from Work uses so the person stays
 * in Work (src/lib/nav/object-href.ts). The id is a prop read from the
 * route's own params, never useParams(): while the task drawer is open,
 * useParams() merges the drawer's [id] into the same key. In Work the crumb
 * is the WorkPlacementProvider's, Back lands on the Work crumb, and the
 * ?new=1 strip stays at the address the canvas is mounted at. Autosave, the
 * draft mirror and the unload flush do not change with the address.
 */
...
import { copyObjectLink, sectionHrefNow } from "@/components/layout/os/use-object-href";
...
async function loadCanvasEntities(excludeId?: string): Promise<TaskSummary[]> {
  ...
  if (taskRes?.ok) {
    ...
    for (const it of items) {
      ...
      // Card hrefs are STORED in the scene, so every seed below is the
      // canonical URL; the section a card is opened from maps it at click
      // time (onOpenEntity below).
      tasks.push({
...
export function CanvasEditor({ canvasId }: { canvasId: string }) {
  // The shape useParams() gave, so every params.id read and every effect
  // dependency below is unchanged; the id can no longer be the drawer's.
  const params = useMemo(() => ({ id: canvasId }), [canvasId]);
  const search = useSearchParams();
  const router = useRouter();
  // Work mode is a value, read before any early return.
  ...
  // The share form, not window.location.href: that carried a Space's slug in
  // Work and a stale ?new=1 everywhere. The door from Work, canonical elsewhere.
  const copyLink = useCallback(() => {
    void navigator.clipboard.writeText(copyObjectLink("canvas", canvasId)).then(() => toast("Link copied"), () => toast("Couldn't copy link"));
  }, [toast, canvasId]);
  ...
  // ?new=1: focus the title so the natural next action is naming it, then
  // strip ONLY that flag, at the address the canvas is mounted at. A replace
  // to /canvas/<id> would bounce every new Work canvas into the Docs hub.
  ...
  useEffect(() => {
    ...
    q.delete("new");
    const qs = q.toString();
    router.replace(`${selfPath}${qs ? `?${qs}` : ""}`, { scroll: false });
  }, [board, search, router, selfPath]);
  ...
  // The Work crumb's live title, and the one Back target every state uses.
  useWorkTitle(inWork && board ? (board.name || "Untitled canvas") : null);
  const exitBack = inWork && place ? place.back : { href: spaceBack?.fallbackHref ?? "/canvas", label: spaceBack?.label ?? "Canvases" };
  ...
  if (loadError) {
    return (
      <OsEmptyView variant="error" context="board" title="Couldn't open this canvas" action={{ label: "Retry", onClick: () => { setLoadError(false); setLoading(true); setReloadKey((k) => k + 1); } }}>
        <BackButton fallbackHref={inWork && place ? place.back.href : "/canvas"} label={inWork && place ? place.back.label : "Canvases"} />
      </OsEmptyView>
    );
  }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-app">
      {/* In Work the WorkPlacementProvider declares the crumb (Work > Space > canvas). */}
      {!inWork ? <Breadcrumb items={[...(spaceBack ? [{ label: spaceBack.label, href: spaceBack.fallbackHref }] : [{ label: "Canvases", href: "/canvas" }]), { label: board.name || "Untitled canvas" }]} /> : null}
      <header className="os-chrome flex h-12 shrink-0 items-center gap-2 border-b border-line bg-app px-6">
        ...
        <MorePortal anchorRef={moreRef} panelRef={morePanelRef} width={240} open={moreOpen} placement="below">
          <CanvasRowMenu
            canvas={{ id: board.id, name: board.name, spaceId: board.spaceId ?? null, spaceSlug: place?.spaceSlug ?? null, favorite: !!favorite, canManage, canEdit: !readOnly }}
            context="editor"
        ...
        {useFirstParty ? (
          <>
            ...
            <WhiteboardCanvas
              ...
              onOpenEntity={(href) => router.push(sectionHrefNow(href))}
              readOnly={readOnly}
            />
            </div>
