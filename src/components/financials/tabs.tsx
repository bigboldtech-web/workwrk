...
// Financials tab components — extracted for board-route conversion.
// Five boards: accounts, entries, reports, statements, calendar.
...
export function AccountsTab() {
  ...
  return (
    <>
        ...
        <div className="flex items-center gap-1.5 text-xs">
          {(["ALL", "ASSET", "LIABILITY", "EQUITY", "REVENUE", "EXPENSE"] as const).map((t) => (
            <button key={t} onClick={() => setFilter(t)}
              className={`px-2.5 py-1 rounded border text-xs ${filter === t ? "border-fg/30 text-fg" : "border-line text-muted hover:text-fg"}`}>
              {t === "ALL" ? "All" : TYPE_LABEL[t]}
            </button>
          ))}
        </div>
        <Button onClick={() => setShowCreate(true)}><Plus size={14} className="mr-1.5" /> New account</Button>
      </div>
      {loading ? (
        <div className="text-center py-8 text-sm text-muted">Loading…</div>
      ) : visible.length === 0 ? (
        <Card><CardContent className="p-10 text-center text-sm text-muted">No accounts in this view. Create your chart of accounts to start posting journal entries.</CardContent></Card>
      ) : (
        <Card>
      ...
      {showCreate && (
        <CreateAccountDialog accounts={rows} onClose={() => setShowCreate(false)} onCreated={() => { setShowCreate(false); toast({ type: "success", title: "Account created" }); load(); }} />
      )}
    </>
  );
}

function CreateAccountDialog({ accounts, onClose, onCreated }: { accounts: GlAccount[]; onClose: () => void; onCreated: () => void }) {
  const { toast } = useToast();
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={parentId} onValueChange={setParentId}>
                ...
                <SelectContent>
                  {parentOptions.length === 0 ? (<div className="p-2 text-xs text-muted">No same-type parents yet.</div>) :
                    parentOptions.map((p) => (<SelectItem key={p.id} value={p.id}>{p.code} — {p.name}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>
          </div>
...
export function EntriesTab() {
  ...
  if (rows.length === 0) {
    return <Card><CardContent className="p-10 text-center text-sm text-muted">No journal entries yet. Posted entries from payroll, AP invoices, and expenses land here.</CardContent></Card>;
  }
  return (
    <Card>
      <CardContent className="p-0">
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-white/5 hover:bg-surface-2">
                ...
                <td className="px-4 py-2.5"><Badge variant="outline" className={`text-[10px] ${ENTRY_STATUS_STYLE[r.status]}`}>{r.status}</Badge></td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <>
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        <div className="flex items-center gap-1.5">
          {([
            ["trial-balance", "Trial balance"], ["income-statement", "P&L"], ["balance-sheet", "Balance sheet"],
          ] as const).map(([k, lbl]) => (
            <button key={k} onClick={() => setReport(k)}
              className={`px-2.5 py-1 rounded border text-xs ${report === k ? "border-fg/30 text-fg" : "border-line text-muted hover:text-fg"}`}>
              {lbl}
            </button>
          ))}
        </div>
...
function TrialBalanceView({ data }: { data: { rows: AccountRow[]; totalDebits: number; totalCredits: number; delta: number } }) {
  if (data.rows.length === 0) return <Card><CardContent className="p-10 text-center text-sm text-muted">No posted entries in this range. Post a journal entry to populate the trial balance.</CardContent></Card>;
  return (
    <Card>
      <CardContent className="p-0">
        <table className="w-full text-sm">
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <Card><CardContent className="p-4">
        <div className="text-xs uppercase tracking-wide text-muted mb-2 flex items-center gap-1"><FileText size={12} /> Revenue</div>
        <ReportLines rows={data.revenue} total={data.totalRevenue} />
      </CardContent></Card>
      <Card><CardContent className="p-4">
        <div className="text-xs uppercase tracking-wide text-muted mb-2 flex items-center gap-1"><FileText size={12} /> Expense</div>
        <ReportLines rows={data.expense} total={data.totalExpense} />
      </CardContent></Card>
      <Card className="md:col-span-2"><CardContent className="p-4 flex items-center justify-between">
        <div className="text-sm font-semibold">Net income</div>
        <div className={`font-mono text-lg ${data.netIncome >= 0 ? "text-green-400" : "text-red-400"}`}>{fmtMoney(data.netIncome)}</div>
      </CardContent></Card>
    </div>
  );
}
...
function ReportLines({ rows, total }: { rows: AccountRow[]; total: number }) {
  if (rows.length === 0) return <div className="text-xs text-muted py-1">No accounts.</div>;
  return (
    <>
...
type StatementResponse =
  | { period: PeriodRef; kind: "pnl"; revenue: StatementAccountTotals[]; expense: StatementAccountTotals[]; totals: { revenue: number; expense: number; netIncome: number } }
  | { period: PeriodRef; kind: "bs"; assets: StatementAccountTotals[]; liabilities: StatementAccountTotals[]; equity: StatementAccountTotals[]; totals: { assets: number; liabilities: number; equity: number; plug: number } }
  | { period: PeriodRef; kind: "cf"; operating: StatementAccountTotals[]; investing: StatementAccountTotals[]; financing: StatementAccountTotals[]; totals: { operating: number; investing: number; financing: number; netChange: number }; note?: string };
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-1.5">
          {([["pnl", "Income Statement (P&L)"], ["bs", "Balance Sheet"], ["cf", "Cash Flow"]] as const).map(([k, lbl]) => (
            <button key={k} onClick={() => setKind(k)}
              className={`px-2.5 py-1 rounded border text-xs ${kind === k ? "border-fg/30 text-fg" : "border-line text-muted hover:text-fg"}`}>
              {lbl}
            </button>
          ))}
        </div>
        <div className="flex items-center gap-2 text-xs">
          ...
          <select className="h-8 text-xs rounded-md border border-line bg-card-2/40 px-2" value={periodId} onChange={(e) => setPeriodId(e.target.value)} disabled={calendarLoading}>
            {calendarLoading && <option value="">Loading…</option>}
            {!calendarLoading && years.length === 0 && (<option value="">No fiscal periods yet</option>)}
            {years.map((y) => (
              <optgroup key={y.id} label={y.label}>
                {y.periods.map((p) => (<option key={p.id} value={p.id}>{p.label} {p.status === "CLOSED" ? "· closed" : ""}</option>))}
              </optgroup>
            ))}
          </select>
        </div>
      ...
      {!periodId && !calendarLoading ? (
        <Card><CardContent className="p-8 text-center text-sm text-muted">Set up a fiscal year in the Calendar tab first, then come back to generate statements.</CardContent></Card>
    ...
    <div className="space-y-3">
      <Card><CardContent className="p-4">
        ...
        <div className="flex items-baseline gap-4 mt-1">
          ...
          <div className="ml-auto text-xs text-muted">Revenue {fmtMoney(data.totals.revenue)} · Expense {fmtMoney(data.totals.expense)}</div>
        </div>
      </CardContent></Card>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card><CardContent className="p-4">
    ...
    <div className="space-y-3">
      <Card><CardContent className="p-4">
        ...
        <div className="grid grid-cols-3 gap-3 mt-2 text-sm">
          <div><div className="text-[10px] uppercase text-muted">Total Assets</div><div className="font-mono text-base">{fmtMoney(data.totals.assets)}</div></div>
          <div><div className="text-[10px] uppercase text-muted">Total Liabilities</div><div className="font-mono text-base">{fmtMoney(data.totals.liabilities)}</div></div>
          <div><div className="text-[10px] uppercase text-muted">Total Equity</div><div className="font-mono text-base">{fmtMoney(data.totals.equity)}</div></div>
        </div>
        {plugFlagged && (
          <div className="mt-3 text-[11px] text-amber-400 border border-amber-400/30 rounded px-2 py-1">
            A = L + E delta: {fmtMoney(data.totals.plug)} — either an out-of-balance ledger, a multi-period roll, or unposted P&L close.
          </div>
        )}
      </CardContent></Card>
...
function CashFlowView({ data }: { data: Extract<StatementResponse, { kind: "cf" }> }) {
  const empty = data.operating.length === 0 && data.investing.length === 0 && data.financing.length === 0;
  return (
    <div className="space-y-3">
      <Card><CardContent className="p-4">
        ...
        <div className="grid grid-cols-4 gap-3 mt-2 text-sm">
          <div><div className="text-[10px] uppercase text-muted">Operating</div><div className="font-mono">{fmtMoney(data.totals.operating)}</div></div>
          <div><div className="text-[10px] uppercase text-muted">Investing</div><div className="font-mono">{fmtMoney(data.totals.investing)}</div></div>
          <div><div className="text-[10px] uppercase text-muted">Financing</div><div className="font-mono">{fmtMoney(data.totals.financing)}</div></div>
          <div><div className="text-[10px] uppercase text-muted">Net Change</div><div className="font-mono">{fmtMoney(data.totals.netChange)}</div></div>
        </div>
        {data.note && (<div className="mt-3 text-[11px] text-muted border border-line rounded px-2 py-1">{data.note}</div>)}
      </CardContent></Card>
      {!empty && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card><CardContent className="p-4">
...
export function CalendarTab() {
  ...
  const togglePeriod = useCallback(async (id: string, currentStatus: "OPEN" | "CLOSED") => {
    ...
    try {
      ...
      load();
    } finally { setBusyPeriodId(null); }
  }, [busyPeriodId, toast, load]);

  return (
    <>
      <div className="flex justify-end mb-3">
        <Button onClick={() => setShowCreate(true)}><Plus size={14} className="mr-1.5" /> New fiscal year</Button>
      </div>
      {loading ? (
        <div className="text-center py-8 text-sm text-muted">Loading…</div>
      ) : years.length === 0 ? (
        <Card><CardContent className="p-10 text-center text-sm text-muted">No fiscal years yet. Create one to enable journal-entry posting.</CardContent></Card>
      ) : (
        <div className="space-y-3">
          {years.map((y) => (
            <Card key={y.id}>
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                  ...
                  <Badge variant="outline" className={`text-[10px] ${y.status === "OPEN" ? "text-green-400 border-green-400/30" : "text-muted border-white/20"}`}>{y.status}</Badge>
                </div>
                <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-1.5">
                  {y.periods.map((p) => {
                    const busy = busyPeriodId === p.id;
                    return (
                      <button key={p.id} type="button" onClick={() => togglePeriod(p.id, p.status)} disabled={busy}
                        ...
                        className={`group text-xs px-2 py-1.5 rounded border text-center transition-colors ${
                          p.status === "OPEN" ? "border-line text-fg hover:border-[color:var(--accent-strong)]"
      ...
      {showCreate && (
        <CreateFiscalYearDialog onClose={() => setShowCreate(false)} onCreated={() => { setShowCreate(false); toast({ type: "success", title: "Fiscal year created" }); load(); }} />
      )}
    </>
  );
}
