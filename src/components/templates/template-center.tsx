    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center p-4">
      ...
      <div role="dialog" aria-modal="true" aria-label="Template Center" className="relative w-full max-w-[1040px] h-[82vh] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-zinc-200/60">
        ...
        <div className="flex items-center gap-3 px-5 py-3 border-b border-zinc-100">
          {selected ? (
            <button type="button" onClick={() => setSelected(null)} className="inline-flex items-center gap-1.5 text-base text-zinc-600 hover:text-zinc-900">
          ...
          {!selected ? (
            <div className="ml-2 flex-1 max-w-[420px] inline-flex items-center gap-2 h-9 px-3 rounded-lg border border-zinc-200">
              ...
              <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search templates…" className="flex-1 text-base bg-transparent outline-none placeholder:text-zinc-400" />
            </div>
        ...
        {selected ? (
          ...
          <div className="flex-1 min-h-0 flex">
            ...
            <main className="flex-1 min-w-0 overflow-y-auto p-5 space-y-6">
              {rows === null ? (
                <div className="flex items-center justify-center h-40 text-xs text-zinc-400"><Loader2 className="w-4 h-4 animate-spin mr-2" /> Loading…</div>
              ) : filtered.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-48 text-center text-zinc-400">
                  <FileStack className="w-8 h-8 mb-2" />
                  <div className="text-xs">No templates yet{scope === "featured" ? "" : " in this view"}.</div>
                  <div className="text-xs mt-1">Save a task, list, or space as a template to see it here.</div>
                </div>
              ) : (
                byCategory.map(([cat, items]) => (
                  <section key={cat}>
                    <h3 className="text-base font-semibold text-zinc-900 mb-2">{cat}</h3>
                      ...
                      {items.map((t) => (
                        <button key={t.id} type="button" onClick={() => openDetail(t.id)} className="text-left rounded-xl border border-zinc-200 bg-white hover:shadow-sm hover:border-zinc-300 transition-all p-3">
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-base font-medium text-zinc-900 truncate">{t.name}</span>
                          </div>
                          <div className="mt-1.5 flex items-center gap-1.5">
                            <span className="text-micro uppercase tracking-wide text-zinc-400">{KIND_LABEL[t.kind]}</span>
                            {t.complexity ? <span className={`text-xs px-1.5 py-0.5 rounded ${COMPLEXITY_TONE[t.complexity]}`}>{COMPLEXITY_LABEL[t.complexity]}</span> : null}
                          </div>
                        </button>
                      ))}
                    </div>
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold text-zinc-900">{tpl.name}</h2>
          {tpl.complexity ? <span className={`inline-block mt-1.5 text-xs px-2 py-0.5 rounded ${COMPLEXITY_TONE[tpl.complexity]}`}>Complexity: {COMPLEXITY_LABEL[tpl.complexity]}</span> : null}
        </div>
        <button type="button" onClick={onUse} disabled={busy || (needSpace && !chosenSpace)} className={`h-9 px-4 rounded-lg text-base inline-flex items-center gap-2 disabled:opacity-50 ${taupeButton}`}>
          {busy ? <Loader2 className="w-4 h-4 animate-spin" /> : null} Use Template
        </button>
      </div>

      {tpl.description ? <p className="mt-3 text-base text-zinc-600 leading-relaxed max-w-[680px]">{tpl.description}</p> : null}

      {needSpace ? (
        <div className="mt-4 max-w-[360px]">
          <label className="text-sm font-medium text-zinc-600">Create in Space</label>
          <select value={chosenSpace} onChange={(e) => onChooseSpace(e.target.value)} className="mt-1 w-full h-9 px-2 rounded-lg border border-zinc-200 text-base bg-white">
      ...
      <div className="mt-6 space-y-3 max-w-[680px]">
        <h3 className="text-base font-semibold text-zinc-900">Template includes</h3>
        {statuses.length > 0 ? (
        ...
        {fields.length > 0 ? (
          <IncludeRow title={`${fields.length} custom field${fields.length === 1 ? "" : "s"}`}>
            <span className="flex flex-wrap gap-1">{fields.slice(0, 12).map((f, i) => <span key={i} className="text-xs px-1.5 py-0.5 rounded bg-zinc-100 text-zinc-600">{f.label ?? "Field"}</span>)}</span>
          </IncludeRow>
        ) : null}
        {views.length > 0 ? (
          <IncludeRow title={`${views.length} view${views.length === 1 ? "" : "s"}`}>
            <span className="flex flex-wrap gap-1">{views.map((v, i) => <span key={i} className="text-xs px-1.5 py-0.5 rounded bg-zinc-100 text-zinc-600">{v.name ?? v.type}</span>)}</span>
          </IncludeRow>
        ) : null}
        {statuses.length === 0 && fields.length === 0 && views.length === 0 ? (
          <p className="text-base text-zinc-400">A clean {KIND_LABEL[tpl.kind].toLowerCase()} with default settings.</p>
        ) : null}
      </div>
    </div>
  );
...
function IncludeRow({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-lg border border-zinc-200 px-3 py-2.5 flex items-center justify-between gap-3">
      <span className="text-base font-medium text-zinc-700">{title}</span>
...
function NavRow({ Icon, label, active, onClick }: { Icon: typeof Compass; label: string; active: boolean; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className={`w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-base ${active ? "bg-zinc-100 text-zinc-900 font-medium" : "text-zinc-600 hover:bg-zinc-50"}`}>
...
function FilterGroup({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <div className="text-micro uppercase tracking-wide text-zinc-400 font-semibold px-1 mb-1.5">{title}</div>
...
function CheckRow({ label, checked, onToggle }: { label: string; checked: boolean; onToggle: () => void }) {
  return (
    <button type="button" onClick={onToggle} className="w-full flex items-center gap-2 px-1 py-1 text-base text-zinc-700 hover:bg-zinc-50 rounded">
