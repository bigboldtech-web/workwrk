...
// Procurement tab components — extracted for board-route conversion.
// Three boards: pos, invoices, vendors. Each tab has its own internal
// scope filter (mine/approve/all) — that stays in-tab.
...
import { Plus, Building2, FileText, Inbox, Receipt } from "lucide-react";
  ...
  return (
    <>
      <div className="flex justify-end mb-3">
        <Button onClick={() => setShowCreate(true)}><Plus size={14} className="mr-1.5" /> Add vendor</Button>
      </div>
      {loading ? (
        <div className="space-y-3">{[1, 2, 3].map((i) => <SkeletonCard key={i} />)}</div>
      ) : rows.length === 0 ? (
        <EmptyState icon={Building2} title="No vendors yet" description="Add a vendor to track POs, invoices, and payment terms in one place." actionLabel="Add vendor" onAction={() => setShowCreate(true)} />
      ) : (
        <Card>
                ...
                {rows.map((v) => (
                  <tr key={v.id} className="border-b border-white/5 hover:bg-surface-2">
                    <td className="px-4 py-2.5">
                      <div className="flex items-center gap-2"><Building2 size={12} className="text-muted" /><span className="font-medium">{v.name}</span></div>
      ...
      {showCreate && (<CreateVendorDialog onClose={() => setShowCreate(false)} onCreated={() => { setShowCreate(false); load(); }} />)}
    </>
  );
}
    ...
    <>
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          <a href="/api/export/purchase-orders" className="inline-flex items-center gap-1.5 h-9 px-3 text-sm rounded-md border border-line text-fg hover:bg-card-2/40">Export CSV</a>
          <Button onClick={() => setShowCreate(true)}><Plus size={14} className="mr-1.5" /> New PO</Button>
        </div>
      </div>
      {loading ? (
        <div className="space-y-3">{[1, 2, 3].map((i) => <SkeletonCard key={i} />)}</div>
        ...
        tab === "approve" ? (
          <EmptyState icon={Inbox} title="Nothing waiting on you" description="POs needing your approval will appear here. Inbox zero — go ship something." />
        ) : (
          <EmptyState icon={FileText} title="No purchase orders yet" description="Create a PO to request goods or services from one of your vendors — it'll route through your approval chain." actionLabel="New PO" onAction={() => setShowCreate(true)} />
        )
      ) : (
        <Card>
          <CardContent className="p-0">
                ...
                <tr className="text-left text-xs text-muted border-b border-white/5">
                  {tab === "approve" && (
                    <th className="px-3 py-2.5 font-normal w-8">
                      <input type="checkbox" checked={allSelected} onChange={toggleAll} aria-label="Select all submitted" disabled={submittedRows.length === 0} />
                    </th>
                  )}
                  ...
                  <tr key={p.id} className="border-b border-white/5 hover:bg-surface-2">
                    {tab === "approve" && (
                      <td className="px-3 py-2.5">
                        {p.status === "SUBMITTED" ? (
                          <input type="checkbox" checked={selectedIds.has(p.id)} onChange={() => toggleId(p.id)} aria-label={`Select PO ${p.number}`} />
                        ) : null}
                      </td>
                    )}
                    ...
                    <td className="px-4 py-2.5"><Badge variant="outline" className={`text-[10px] ${PO_STATUS_STYLE[p.status]}`}>{p.status}</Badge></td>
                    <td className="px-4 py-2.5 text-right"><POActions po={p} onAction={action} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {showCreate && (<CreatePoDialog onClose={() => setShowCreate(false)} onCreated={() => { setShowCreate(false); load(tab); }} />)}
      {tab === "approve" && (
        <BulkApproveBar entityType="purchase-order" selectedIds={selectedArray} onClear={() => setSelectedIds(new Set())} onDone={() => load(tab)} />
      )}
    </>
  );
}
...
function POActions({ po, onAction }: { po: PO; onAction: (id: string, action: string, opts?: { note?: string }) => void }) {
  ...
  if (po.status === "SUBMITTED") buttons.push({ label: "Approve", action: "approve" }, { label: "Reject", action: "reject", danger: true });
  ...
  return (
    <div className="flex items-center justify-end gap-1">
      {buttons.map((b) => (
        <Button key={b.action} size="sm" variant="outline" className={`h-7 text-xs ${b.danger ? "text-red-400" : ""}`}
          onClick={() => {
            if (b.action === "reject") {
              const note = prompt("Reason for rejection?");
              if (note === null) return;
              onAction(po.id, b.action, { note });
            } else { onAction(po.id, b.action); }
          }}>
          {b.label}
        </Button>
      ))}
...
function CreatePoDialog({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  useEffect(() => {
    fetch("/api/vendors").then((r) => r.ok ? r.json() : []).then((data) => setVendors(Array.isArray(data) ? data : [])).catch(() => setVendors([]));
  }, []);
  ...
  async function save(submit: boolean) {
    ...
    try {
      const res = await fetch("/api/purchase-orders", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          vendorId, description: description.trim(),
          amount: Number(amount), currency,
          expectedDeliveryDate: expectedDeliveryDate || undefined,
          notes: notes.trim() || undefined, submit,
        }),
      });
      const data = await res.json();
      if (!res.ok) { toast({ type: "error", title: "Couldn't save", description: data?.error }); return; }
        ...
        <div className="space-y-3 pt-1">
          <div className="space-y-1.5">
            ...
            <Select value={vendorId} onValueChange={setVendorId}>
              ...
              <SelectContent>
                {vendors.length === 0 ? (<div className="p-2 text-xs text-muted">Add a vendor first.</div>) :
                  vendors.filter((v) => !v.archived).map((v) => (<SelectItem key={v.id} value={v.id}>{v.name}</SelectItem>))}
              </SelectContent>
            </Select>
          </div>
  ...
  return (
    <>
      <div className="flex justify-end mb-3 gap-2">
        <a href="/api/export/invoices" className="inline-flex items-center gap-1.5 h-9 px-3 text-sm rounded-md border border-line text-fg hover:bg-card-2/40">Export CSV</a>
        <Button onClick={() => setShowCreate(true)}><Plus size={14} className="mr-1.5" /> Record invoice</Button>
      </div>
      {loading ? (
        <div className="space-y-3">{[1, 2, 3].map((i) => <SkeletonCard key={i} />)}</div>
      ) : rows.length === 0 ? (
        <EmptyState icon={Receipt} title="No invoices yet" description="Record an invoice against a PO to track what's owed and what's been paid — overdue invoices route to your AP queue automatically." actionLabel="Record invoice" onAction={() => setShowCreate(true)} />
      ) : (
        <Card>
          <CardContent className="p-0">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs text-muted border-b border-white/5">
                  <th className="px-3 py-2.5 font-normal w-8">
                    <input type="checkbox" checked={allSelected} onChange={toggleAll} aria-label="Select all pending" disabled={pendingRows.length === 0} />
                  </th>
                ...
                {rows.map((inv) => {
                  ...
                  return (
                    <tr key={inv.id} className="border-b border-white/5 hover:bg-surface-2">
                      <td className="px-3 py-2.5">
                        {inv.status === "PENDING" ? (
                          <input type="checkbox" checked={selectedIds.has(inv.id)} onChange={() => toggleId(inv.id)} aria-label={`Select invoice ${inv.invoiceNumber}`} />
                        ) : null}
                      </td>
                      <td className="px-4 py-2.5"><div className="font-mono text-xs">{inv.invoiceNumber}</div></td>
                      ...
                      <td className={`px-4 py-2.5 text-xs ${isOverdue ? "text-red-400" : "text-muted"}`}>
                        {new Date(inv.dueDate).toLocaleDateString()}{isOverdue && " · overdue"}
                      </td>
                      <td className="px-4 py-2.5 text-right font-mono text-xs">{fmt(inv.amount, inv.currency)}</td>
                      <td className="px-4 py-2.5"><Badge variant="outline" className={`text-[10px] ${INVOICE_STATUS_STYLE[inv.status]}`}>{inv.status}</Badge></td>
                      <td className="px-4 py-2.5 text-right">
                        <div className="flex items-center justify-end gap-1">
                          {inv.status === "PENDING" && (
                            <>
                              <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => action(inv.id, "approve")}>Approve</Button>
                              <Button size="sm" variant="outline" className="h-7 text-xs text-red-400"
                                onClick={() => {
                                  const note = prompt("Reason for rejection?");
                                  if (note === null) return;
      ...
      {showCreate && (<CreateInvoiceDialog onClose={() => setShowCreate(false)} onCreated={() => { setShowCreate(false); load(); }} />)}
      <BulkApproveBar entityType="invoice" selectedIds={selectedArray} onClear={() => setSelectedIds(new Set())} onDone={load} />
    </>
  );
}
...
function CreateInvoiceDialog({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/purchase-orders?scope=all&limit=200").then((r) => r.ok ? r.json() : [])
      .then((data) => {
  ...
  async function save() {
    ...
    try {
      const res = await fetch("/api/invoices", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          vendorId, purchaseOrderId: purchaseOrderId || undefined,
          invoiceNumber: invoiceNumber.trim(),
          amount: Number(amount), currency, issueDate, dueDate,
        }),
      });
      const data = await res.json();
      if (!res.ok) { toast({ type: "error", title: "Couldn't save", description: data?.error }); return; }
            ...
            <div className="space-y-1.5">
              ...
              <Select value={purchaseOrderId} onValueChange={setPurchaseOrderId}>
                ...
                <SelectContent>
                  ...
                  {pos.map((p) => (<SelectItem key={p.id} value={p.id}>{p.number} · {fmt(p.amount, p.currency)}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>
          )}
          <div className="space-y-1.5"><Label>Invoice #</Label><Input value={invoiceNumber} onChange={(e) => setInvoiceNumber(e.target.value)} /></div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5"><Label>Amount</Label><Input value={amount} onChange={(e) => setAmount(e.target.value)} inputMode="decimal" /></div>
            <div className="space-y-1.5"><Label>Currency</Label><Input value={currency} onChange={(e) => setCurrency(e.target.value.toUpperCase().slice(0, 3))} maxLength={3} /></div>
          </div>
