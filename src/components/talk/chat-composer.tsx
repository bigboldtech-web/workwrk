...
import {
  AtSign, Bold, Code, Italic, Link2, List, ListOrdered, Mic,
...
export function ChatComposer({ members, meId, placeholder, autoFocus, onSend, onError, onStartCall }: {
   ...
   *  (thread panels omit it; threads don't start calls). */
  ...
  // The emoji popover is a real overlay, so it goes on the layer stack:
  // Esc closes the topmost overlay, and a popover that is not registered
  // is one a keyboard user cannot get out of.
  useLayer(emojiOpen, { kind: "popover", close: () => setEmojiOpen(false) });
...
${cur}` : body));
  ...
  return (
    <div
      className={`relative rounded-xl border bg-raised px-3 py-2 focus-within:border-line-strong ${dragOver ? "border-[var(--os-brand)] bg-[var(--os-brand)]/5" : "border-line"}`}
    ...
    >
      ...
      {mentionMatches.length > 0 && (
        <div className="absolute bottom-full left-2 mb-1 z-20 w-64 rounded-lg border border-line bg-raised shadow-lg py-1">
          {mentionMatches.map((p, i) => (
            <button
              key={p.id}
              type="button"
              onMouseDown={(e) => { e.preventDefault(); pickMention(p); }}
              className={`w-full flex items-center gap-2 px-2.5 h-8 text-left text-sm ${i === mentionIndex ? "bg-hover text-ink-strong" : "text-ink hover:bg-subtle"}`}
            >
      ...
      {files.length > 0 && (
        <div className="flex flex-wrap gap-1.5 pb-2">
          {files.map((f, i) => (
            <span key={`${f.name}-${i}`} className="inline-flex items-center gap-1.5 h-7 pl-2 pr-1.5 rounded-lg bg-hover text-xs text-ink">
              <Paperclip className="w-3 h-3 text-ink-3" />
              <span className="max-w-[180px] truncate">{f.name}</span>
              <button type="button" onClick={() => setFiles((prev) => prev.filter((_, x) => x !== i))} aria-label={`Remove ${f.name}`} className="text-ink-3 hover:text-ink">
                <X className="w-3 h-3" />
              </button>
            </span>
          ))}
      ...
      <div className="flex flex-col">
        ...
        <textarea
          ...
          className="w-full resize-none bg-transparent outline-none text-base text-ink placeholder:text-ink-3 leading-6 max-h-40"
        />
        ...
        <div className="mt-1 flex items-center gap-0.5">
          ...
          <button
            ...
            className={`inline-flex h-7 w-7 items-center justify-center rounded-md ${fmtOpen ? "bg-hover text-ink underline underline-offset-2" : "text-ink-3 hover:bg-hover hover:text-ink"}`}
          >
          ...
          <div className="relative">
            ...
            {emojiOpen && (
              <>
                ...
                <div className="absolute bottom-8 left-0 z-20 grid grid-cols-6 gap-0.5 rounded-lg border border-line bg-raised p-1.5 shadow-lg">
                  {COMPOSER_EMOJI.map((e) => (
                    <button
                      key={e}
                      type="button"
                      onClick={() => { setEmojiOpen(false); insertAtCaret(e); }}
                      className="flex h-8 w-8 items-center justify-center rounded-md text-lg hover:bg-hover"
                    >
                      {e}
                    </button>
                  ))}
          ...
          {onStartCall && (
            <>
              <span className="mx-1 h-4 w-px bg-active" />
              <button
                type="button"
          ...
          <button
            ...
            className={`inline-flex h-7 w-8 items-center justify-center rounded-md text-white  ${
              input.trim() || files.length > 0 ? "bg-[var(--os-brand)] hover:bg-[var(--os-brand-hover)]" : "bg-active"
            }`}
          >
