  ...
  return (
    <div className="flex items-center gap-1 px-1 shrink-0 overflow-x-auto no-scrollbar">
      {pins.map((p) => (
        <span
          key={`${p.kind}:${p.id}`}
          className="group inline-flex items-center gap-1 h-6 pl-1 pr-0.5 rounded-md border border-zinc-200 bg-white hover:bg-zinc-50 text-sm text-zinc-700 shrink-0"
        >
