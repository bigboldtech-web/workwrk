...
export function RemindersBell() {
  ...
  const Group = ({ label, rows, tone = "amber" }: { label: string; rows: Reminder[]; tone?: "amber" | "brand" }) =>
    rows.length === 0 ? null : (
      <div className="py-1">
        <div className="px-3 pt-1 pb-0.5 text-micro font-semibold uppercase tracking-wide text-zinc-400">{label}</div>
        <ul>
          {rows.map((r) => {
            ...
            const inner = (
              <div className="group flex items-center gap-2 px-3 py-1.5 hover:bg-zinc-50">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-base text-zinc-800 truncate">{r.title}</div>
                  <div className="text-xs text-zinc-400">{fmtTime(r.remindAt)}</div>
                </div>
  ...
  return (
    <>
      ...
      >
        ...
        {badge > 0 ? (
          <span className="absolute -top-0.5 -right-0.5 min-w-[14px] h-[14px] px-0.5 rounded-full bg-[#FB5A6F] text-white text-micro font-semibold leading-[14px] text-center">
            {badge > 9 ? "9+" : badge}
          </span>
        ) : null}
      </button>
        ...
        ? createPortal(
            ...
            >
              <div className="flex items-center gap-2 px-3 h-10 border-b border-zinc-100">
                ...
                <span className="text-base font-semibold text-zinc-900 flex-1">Reminders</span>
                <button
                  type="button"
                  onClick={() => { setOpen(false); window.dispatchEvent(new CustomEvent("workwrk:tool", { detail: "reminder" })); }}
                  className="inline-flex items-center gap-1 h-7 px-2 rounded-md text-sm text-zinc-600 hover:bg-zinc-100"
                >
              ...
              <div className="max-h-[60vh] overflow-y-auto">
                {nothing ? (
                  <div className="px-3 py-8 text-center">
                    ...
                    <p className="text-base text-zinc-500">No reminders</p>
                    <p className="text-xs text-zinc-400 mt-0.5">Schedule one on a task or add a personal reminder.</p>
                  </div>
                ) : (
                  <>
                    ...
                    {history.length > 0 ? (
                      <div className="py-1 border-t border-zinc-100">
                        <div className="px-3 pt-1.5 pb-0.5 text-micro font-semibold uppercase tracking-wide text-zinc-400">Recently done</div>
                        <ul>
                          {history.map((r) => (
                            <li key={r.id} className="flex items-center gap-2 px-3 py-1.5">
                              <Check className="w-3.5 h-3.5 flex-shrink-0 text-zinc-300" />
                              <div className="flex-1 min-w-0">
                                <div className="text-base text-zinc-500 truncate line-through">{r.title}</div>
                                <div className="text-xs text-zinc-400">{fmtTime(r.updatedAt || r.firedAt || r.remindAt)}</div>
                              </div>
                            </li>
                          ))}
                        </ul>
