    ...
    <aside className="w-[256px] shrink-0 border-e border-zinc-200 dark:border-[#2A2F38] overflow-y-auto px-3 py-3 space-y-4">
      ...
      <section>
        ...
        <div className="rounded-lg border border-dashed border-zinc-200 dark:border-[#2A2F38] px-3 py-4 text-center">
          ...
          <div className="mt-1.5 text-xs text-zinc-400 dark:text-zinc-500 leading-snug">Prioritize a task to see it appear here</div>
        </div>
        <button type="button" className="mt-1.5 w-full h-7 rounded-md text-sm text-zinc-500 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-white/10 inline-flex items-center justify-center gap-1.5">
...
function MeetWith({ onCreated, autoFocus }: { onCreated: () => void; autoFocus?: boolean }) {
  ...
  return (
    <section>
      <h3 className="px-1 mb-1.5 text-xs font-semibold uppercase tracking-wide text-zinc-400 dark:text-zinc-500">Meet with</h3>
      <div className="flex items-center gap-2 h-8 rounded-lg border border-zinc-200 dark:border-[#2A2F38] px-2.5">
        <Search className="w-3.5 h-3.5 text-zinc-400 shrink-0" />
        <input ref={inputRef} value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search for people…" className="flex-1 min-w-0 bg-transparent outline-none text-base text-zinc-800 dark:text-zinc-100 placeholder:text-zinc-400" />
      </div>
