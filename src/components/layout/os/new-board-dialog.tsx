...
const POPULAR_TILES: ViewTile[] = [
  ...
  { key: "board",       label: "Board", sublabel: "Kanban",       Icon: LayoutGrid,      viewType: "KANBAN",     hue: "#0073EA" },
  { key: "form",        label: "Form", sublabel: "Survey",        Icon: ClipboardList,   viewType: "FORM",       hue: "#7F5347" },
  ...
  { key: "mindmap",     label: "Mind Map",                        Icon: Network,         viewType: null,         hue: "#037F4C" },
...
const INTEGRATION_TILES: ViewTile[] = [
  ...
  { key: "embed-figma",     label: "Figma",           Icon: Brush,         viewType: null, hue: "#71717A" },
];
    ...
    <Dialog open={open} onOpenChange={handle}>
      <DialogContent className="max-w-[560px] p-0">
        <div className="px-5 pt-5 pb-2">
          <DialogTitle className="text-[15px] font-semibold">New Board</DialogTitle>
          <DialogDescription className="text-[13px] text-zinc-500 mt-1">
            Name your Board and pick how it should open.
          </DialogDescription>
        </div>

        <div className="px-5 pb-3">
          <label className="text-[13.5px] font-medium block mb-1.5">Name</label>
          <input
            type="text"
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="e.g. Onboarding tasks, Sprint board, Customer feedback"
            className="w-full h-8 px-3 rounded-md border border-zinc-200 bg-white text-[14px] focus:outline-none focus:border-[var(--os-brand)]"
            autoFocus
          />
        </div>
        ...
        <div className="px-5 pb-2">
          <div className="relative">
            <input
              ...
              className="w-full h-8 px-3 pr-9 rounded-md border border-zinc-200 bg-white text-[14px] focus:outline-none focus:border-[var(--os-brand)]"
            />
            <button
              type="button"
        ...
        <div className="px-3 pb-3 max-h-[55vh] overflow-y-auto">
          {tiles.popular.length > 0 ? (
            <>
              <div className="text-[12px] font-semibold uppercase tracking-wide text-zinc-400 px-3 pt-2 pb-1">Popular</div>
              <TileGrid tiles={tiles.popular} selectedKey={selectedTile} onPick={setSelectedTile} />
            </>
          ) : null}
          {tiles.integrations.length > 0 ? (
            <>
              <div className="text-[12px] font-semibold uppercase tracking-wide text-zinc-400 px-3 pt-3 pb-1">Embed an integration</div>
          ...
          {tiles.popular.length + tiles.integrations.length === 0 ? (
            <div className="px-3 py-6 text-[14px] text-zinc-500 text-center">
        ...
        <div className="px-5 py-3 border-t border-zinc-200 flex items-center justify-between gap-3">
          <label className="flex-1 cursor-pointer" onClick={() => setIsPrivate(!isPrivate)}>
            <div className="text-[13.5px] font-medium text-zinc-900">Make Private</div>
            <div className="text-[13px] text-zinc-500 leading-snug">
              Tighter than the Space — only board members + the Space owner can read.
            </div>
          </label>
          <Switch checked={isPrivate} onChange={setIsPrivate} aria-label="Make board private" />
        </div>

        {error ? <div className="px-5 py-2 text-[13px] text-red-500 bg-red-500/10">{error}</div> : null}

        <div className="px-5 py-3 flex items-center justify-end gap-2 border-t border-zinc-200">
          <button type="button" onClick={() => handle(false)} className="text-[13.5px] text-zinc-500 hover:text-zinc-900 px-3 h-8 rounded-md" disabled={submitting}>
            Cancel
          </button>
          <button
            type="button"
            ...
            className="px-4 h-8 rounded-md text-[13.5px] font-medium text-white bg-[var(--os-brand)] hover:bg-[var(--os-brand-hover)] disabled:opacity-50"
          >
      ...
      {tiles.map((t) => {
        ...
        return (
          ...
          >
            ...
            <span className="text-[14px] flex items-baseline gap-1.5">
              {t.label}
              {t.sublabel ? <span className="text-[13px] text-zinc-500">{t.sublabel}</span> : null}
              {disabled ? <span className="text-[11px] uppercase tracking-wide text-zinc-500">Soon</span> : null}
            </span>
          </button>
        );
      })}
