          ...
          return (
            <li key={n.key}>
              ...
              >
                ...
                {n.badge && n.badge > 0 ? (
                  <span className="text-[12px] text-zinc-400 tabular-nums">{n.badge}</span>
                ) : null}
              </Link>
            </li>
          );
  ...
  return (
    <>
      ...
      <button
        ...
        className="mt-0.5 flex w-full items-center gap-2 h-7 px-2 rounded-md text-[13.5px] text-zinc-400 hover:bg-zinc-50 hover:text-zinc-600"
      >
...
function PageRow({ doc, depth, hasChildren, open, active, onToggle, onOpen, onAdd, onMenu }: {
  ...
  return (
    <li
      className={`group/page flex items-center gap-1 h-7 pr-1 rounded-md text-[14px] cursor-pointer ${
...
function DocLink({ doc, active, onOpen, onMenu }: {
  ...
  return (
    <li
      className={`group/doc flex items-center gap-2 h-7 px-2 rounded-md text-[14px] cursor-pointer ${
        active ? "bg-zinc-100 text-zinc-900" : "text-zinc-700 hover:bg-zinc-50"
      }`}
      onClick={onOpen}
      onContextMenu={onMenu}
