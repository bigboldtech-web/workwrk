                ...
                {days.map((d) => {
                  ...
                  return (
                    <div key={d.toISOString()} className="flex-1 py-2 text-center">
                      <div className="text-xs uppercase tracking-wide text-zinc-400 dark:text-zinc-500">{DOW[d.getDay()]}</div>
                      <div className={`text-base font-semibold mt-0.5 inline-flex items-center justify-center ${today ? "h-7 w-7 rounded-full bg-[#2F8BF0] text-white" : "text-zinc-800 dark:text-zinc-100"}`}>{d.getDate()}</div>
                    </div>
                  );
                })}
              </div>
                        ...
                        {events.map((e) => {
                          ...
                          return (
                            <button
                              ...
                              title={e.title}
                            >
                              <div className="text-xs font-medium truncate" style={{ color }}>{e.title}</div>
                              <div className="text-xs truncate" style={{ color: `${color}cc` }}>{s.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}</div>
                            </button>
                          );
                        })}
                      </div>
    ...
    <div className="fixed inset-0 z-[95] flex items-center justify-center bg-black/20" onClick={onClose}>
      <div className="w-[420px] max-w-[94vw] rounded-2xl bg-white dark:bg-[#181C22] border border-zinc-200 dark:border-[#2A2F38] shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-3 pb-3 space-y-2.5">
          <input value={title} onChange={(e) => setTitle(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") void create(); }} autoFocus placeholder={`${placeholder}, @ for people, @@ for tasks`} className="w-full h-11 px-3 rounded-lg border text-base text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 outline-none" style={{ borderColor: accent }} />

          {/* date + time range */}
          <div className="flex items-center gap-2 text-base text-zinc-700 dark:text-zinc-200">
            ...
            {!allDay ? (
              <>
                ...
                <span className="text-sm text-zinc-400">{durLabel}</span>
              </>
            ) : null}
          </div>
          <label className="flex items-center gap-2 text-base text-zinc-600 dark:text-zinc-300 cursor-pointer">
          ...
          <div className="pt-1 border-t border-zinc-100 dark:border-[#23272F] space-y-1">
            ...
            {showDesc ? (
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} placeholder="Add a description…" className="w-full rounded-lg border border-zinc-200 dark:border-[#2A2F38] bg-transparent px-2.5 py-2 text-base text-zinc-800 dark:text-zinc-100 placeholder:text-zinc-400 outline-none resize-none" />
            ) : null}
          </div>
...
function Row({ icon: Icon, label, muted }: { icon: typeof Video; label: string; muted?: boolean }) {
  return (
    <div className={`flex items-center gap-2.5 px-1.5 py-1.5 rounded-md text-base ${muted ? "text-zinc-400 dark:text-zinc-500" : "text-zinc-700 dark:text-zinc-200"}`}>
