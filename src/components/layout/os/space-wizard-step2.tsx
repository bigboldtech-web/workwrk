  ...
  return (
    <>
      <div className="px-6 pt-6 pb-3">
        <DialogTitle className="text-base font-semibold">Define your workflow</DialogTitle>
          ...
          {PRESETS.map((p) => {
            ...
            return (
              ...
              >
                ...
                <div className="text-xs text-muted mt-0.5 leading-snug">{p.blurb}</div>
              </button>
            );
          })}
        </div>
      ...
      <div className="px-6 py-4 mt-2 border-t border-border flex items-center justify-between">
        <button
          type="button"
          onClick={onBack}
          className="text-base text-muted hover:text-foreground px-3 py-2"
        >
          Back
        </button>
        <button
  ...
  return (
    <>
      ...
      <div className="px-6 pb-2 max-h-[60vh] overflow-y-auto">
        ...
        >
          <span className="h-7 w-7 rounded-full bg-surface-2 border border-border flex items-center justify-center text-xs font-semibold text-muted">
            You
          </span>
          <span className="flex-1 text-base">Creator (default)</span>
          ...
          <div className="rounded-lg border border-border divide-y divide-border">
            {filtered.map((u) => {
              ...
              return (
                ...
                >
                  <span className="h-7 w-7 rounded-full bg-surface-2 border border-border flex items-center justify-center text-micro font-semibold uppercase text-muted shrink-0">
                    {(u.firstName?.[0] ?? u.email[0] ?? "?").toUpperCase()}
                  </span>
                  <span className="flex-1 min-w-0">
                    <span className="block text-base font-medium truncate">{userDisplayName(u)}</span>
          ...
          {VIEW_CATALOG.filter((v) => v.shipped).map((v) => {
            ...
            return (
              <div key={v.key} className="flex items-center gap-3 px-3 py-2.5">
                ...
                {isDefault ? <span className="text-xs font-medium" style={{ color: accent }}>Default</span> : null}
                <Toggle
                  value={enabled}
            ...
            <div key={group.id}>
              <div className="flex items-center justify-between mb-2">
                <div>
                  ...
                  <div className="text-xs text-muted">{group.blurb}</div>
                </div>
                <button
                  type="button"
              ...
              <div className="rounded-lg border border-border divide-y divide-border">
                {rows.map((s) => (
                  <div key={s.key} className="flex items-center gap-2 px-3 py-2">
                    ...
                    <input
                      ...
                      className="flex-1 bg-transparent text-base font-medium uppercase tracking-wide focus:outline-none"
                    />
                    <button
                      type="button"
                ...
                {rows.length === 0 ? (
                  <div className="px-3 py-3 text-xs text-muted-2">No statuses in this group yet.</div>
                ) : null}
              </div>
            </div>
          );
    ...
    <>
      ...
      <div className="px-6 pb-2 max-h-[60vh] overflow-y-auto space-y-4">
        <label className="flex items-center justify-between gap-3 rounded-lg border border-border px-3 py-2.5">
          <div>
            <div className="text-base font-semibold">{turnOffAll ? "Turn off all Modules" : "Turn on starter modules"}</div>
            <div className="text-xs text-muted">Bulk toggle — you can fine-tune below</div>
          </div>
          <Toggle
            value={turnOffAll}
  ...
  return (
    <div className="px-6 py-4 mt-2 border-t border-border flex justify-end">
      <button
        type="button"
        onClick={onDone}
        className="px-4 py-2 rounded-lg text-base font-medium text-white shadow-sm transition hover:opacity-90"
        style={{ backgroundColor: accent }}
      >
        {doneLabel}
      </button>
