// The route-transition loader body (spec-shell 2.2, design-system 5.15).
// While the next page suspends: the skeleton page header (title row, views
// row, toolbar, the same drawing a page shows while it fetches its object)
// and eight skeleton rows at the data-row height, with one 13px value line
// under the first row. The rail's logo dots pulse (after 200ms, through
// RoutePendingSignal); rail, sidebar and bar stay interactive. No overlay,
// no progress line, no in-content four dots.
//
// (dashboard)/loading.tsx renders it, and so does the Work placement
// provider while it moves an object from the door to its Space-scoped
// address, so that hop reads as one continuous load rather than skeleton,
// blank, skeleton.
