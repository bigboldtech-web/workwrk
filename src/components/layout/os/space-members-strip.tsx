    ...
    <>
      <div className="flex items-center justify-between gap-3">
        ...
        >
          <span className="flex -space-x-1.5">
            {shown.map((m) => (
              <span
                key={m.id}
                className="h-6 w-6 rounded-full border-2 border-white inline-flex items-center justify-center text-xs font-semibold text-white shadow-sm"
            ...
            {overflow > 0 ? (
              <span className="h-6 w-6 rounded-full border-2 border-white bg-zinc-100 text-zinc-600 inline-flex items-center justify-center text-xs font-semibold shadow-sm">
                +{overflow}
              </span>
            ) : null}
            {shown.length === 0 ? (
              <span className="h-6 w-6 rounded-full border-2 border-white bg-zinc-100 text-zinc-400 inline-flex items-center justify-center text-xs font-semibold shadow-sm">
                ?
              </span>
            ) : null}
          </span>
        ...
        <button
          type="button"
          onClick={() => setOpen(true)}
          className="h-7 px-2.5 rounded-md text-sm font-medium border border-zinc-200 hover:bg-zinc-50 text-zinc-700"
        >
          Manage
        </button>
      </div>
