...
// SpaceFilesCard — files at a Space's ROOT (moved "into the space" without a
// folder, or dropped onto the Space row / this card). Shares the drive: every
// file also appears in Library → Files with a Space chip. Listens for
// `workwrk:files-changed` so sidebar drops and moves reflect instantly.
...
export function SpaceFilesCard({ spaceId, spaceName, canEdit }: { spaceId: string; spaceName: string; canEdit: boolean }) {
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch(`/api/files?spaceId=${encodeURIComponent(spaceId)}&spaceRoot=1`, { cache: "no-store" });
      if (!res.ok) return;
      const d = await res.json();
      setFiles(Array.isArray(d) ? d : d.data ?? []);
    } catch { /* stays loading; nothing destructive */ }
  }, [spaceId]);
  ...
  async function remove(f: FileRow) {
    if (!(await confirm({
      title: `Delete "${f.name}"?`,
      description: "Removes the file from this Space and from Library → Files.",
  ...
  return (
    <div className="relative">
      <FileDropZone spaceId={spaceId} disabled={!canEdit} label={spaceName} />
      ...
      {files === null ? (
        <div className="flex items-center gap-2 py-3 text-[13px] text-zinc-400"><Loader2 className="h-4 w-4 animate-spin" /> Loading…</div>
      ) : files.length === 0 ? (
        <p className="px-2 py-3 text-xs text-zinc-500">
          No files at this Space&apos;s root yet.{canEdit ? " Drop files here or on the Space in the sidebar." : ""}
        </p>
      ) : (
          ...
          {files.map((f) => {
            ...
            return (
              <li key={f.id} className="group flex items-center gap-3 py-2">
                ...
                {canEdit ? (
                  <>
                    <button type="button" onClick={() => setMovingFile(f)} aria-label={`Move ${f.name}`} title="Move to a folder, another space or the library" className="rounded-md p-1.5 text-zinc-300 opacity-0 hover:bg-zinc-100 hover:text-zinc-700 group-hover:opacity-100">
