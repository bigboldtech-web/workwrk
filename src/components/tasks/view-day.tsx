  ...
  return (
    <div className="rounded-xl border border-border bg-surface overflow-hidden">
          ...
          {allDay.map((t) => {
            ...
            return (
              ...
              >
                ...
                {badgeText && <span className={`text-micro px-1 rounded ${badgeCls}`}>{badgeText}</span>}
                {t.assignee && <span className="text-xs text-muted">{t.assignee.firstName}</span>}
              </button>
            );
          })}
        </div>
      ...
      <div className="relative flex">
        {/* Hour gutter */}
        <div className="w-14 shrink-0 border-r border-border text-xs text-muted">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-1.5 text-xs font-medium">
                  ...
                  {badgeText && <span className={`ml-auto text-micro px-1 rounded ${badgeCls}`}>{badgeText}</span>}
                </div>
                {height > SLOT_PX * 1.2 && t.assignee && (
                  <div className="text-xs text-muted mt-0.5">{t.assignee.firstName} {t.assignee.lastName}</div>
                )}
              </button>
            );
          })}
