...
export function MonthView({
  ...
  return (
    <div className="rounded-xl border border-border bg-surface overflow-hidden">
      <div className="grid grid-cols-7 text-xs text-muted border-b border-border bg-surface-2/40">
      ...
      <div className="grid grid-cols-7 auto-rows-fr">
        {days.map(({ date, inMonth }) => {
          ...
          return (
            ...
            >
              <div className={`text-xs font-medium mb-1 ${isToday ? "text-[#d4ff2e]" : ""}`}>
                {date.getDate()}
              </div>
              <div className="space-y-0.5">
                {list.slice(0, 3).map((t) => {
                  ...
                  return (
                    <span
                      ...
                      className={`block truncate text-xs rounded px-1 py-0.5 cursor-pointer ${cls}`}
                    >
                      {t.title}
                    </span>
                  );
                ...
                {list.length > 3 && (
                  <span className="block text-xs text-muted">+{list.length - 3} more</span>
                )}
              </div>
            </button>
          );
