 ...
 *
 * ONE BUILDER, TWO ADDRESSES. This is the body of /forms/[id] (the Tables
 * hub) and of the Work door /work/forms/[id], which a form opened from Work
 * uses so the person stays in Work (src/lib/nav/object-href.ts). The id is a
 * prop read from the route's own params: it used to be pathname.split("/")[2],
 * which under the task drawer is the TASK's id. Every query-only navigation
 * (?tab, ?preview, the ?new strip) goes to the address the builder is
 * mounted at (`selfPath`), never to usePathname(). In Work the crumb is the
 * WorkPlacementProvider's. Autosave and the leave guard do not change.
 */
...
import { canonicalHref, sectionHref } from "@/lib/nav/object-href";
...
export function FormBuilder({ formId }: { formId: string }) {
  ...
  // Work mode is a value, read once here before any early return.
  const place = useWorkPlacement();
  const inWork = place?.kind === "form" && place.id === formId;
  const selfPath = inWork && place ? place.self : canonicalHref("form", formId);
  ...
  // The Work crumb's live title (the door names nothing until the form loads).
  useWorkTitle(inWork ? (draft?.name?.trim() || null) : null);
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(t);
  }, [draft, searchParams, router, selfPath]);
  ...
  useEffect(() => {
    ...
    const onClick = (e: MouseEvent) => {
      ...
      // section the builder is in, whichever capture listener saw it first.
  ...
  async function deleteAllResponses() {
    ...
    setResponseTotal(0);
    router.replace(`${selfPath}?tab=responses&r=${Date.now()}`, { scroll: false });
  }
  ...
  if (loadError) {
    return (
      <div className="os-chrome flex min-h-0 flex-1 flex-col bg-app">
        {!inWork ? <Breadcrumb items={[{ label: "Forms", href: "/forms" }]} /> : null}
  ...
  return (
    <div className="os-chrome flex min-h-0 flex-1 flex-col bg-app">
      {/* In Work the WorkPlacementProvider declares the crumb (Work > form). */}
      {!inWork ? <Breadcrumb items={[{ label: "Forms", href: "/forms" }, { label: displayName }]} /> : null}

      {/* ── Title row 40 ── */}
      <header className="flex h-10 shrink-0 items-center gap-2 px-4">
        {/* Back is the List or table the form goes to (mapped into Work in
            Work), else the section's own fallback. */}
        <BackButton
          fallbackHref={primaryDest?.href ? (inWork ? sectionHref(primaryDest.href, "home") : primaryDest.href) : inWork && place ? place.back.href : "/forms"}
          label={(primaryDest?.href && primaryDest.name) || (inWork && place ? place.back.label : "Forms")}
        />
        <EntityTile size="md" name={displayName} fallback="form" {...NEUTRAL_TILE} />
        <input
          ref={titleRef}
          value={draft.name}
          ...
          readOnly={locked}
          maxLength={200}
          className="h-8 min-w-[120px] max-w-[420px] flex-1 truncate rounded-md border border-transparent bg-transparent px-1.5 text-title font-semibold text-ink hover:border-line focus:border-brand focus:outline-none read-only:hover:border-transparent"
        />
