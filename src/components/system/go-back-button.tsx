...
export function GoBackButton({ className, fallbackHref = "/today" }: { className?: string; fallbackHref?: string }) {
  ...
  return (
    <button
      ...
      className={
        className ??
        "inline-flex items-center h-9 px-4 rounded-full border border-zinc-200 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300 text-base hover:bg-zinc-50 dark:hover:bg-zinc-900"
      }
    >
      Go back
    </button>
