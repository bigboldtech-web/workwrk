...
// Payroll tab components — extracted from the original
// `/payroll/page.tsx` for the board-route conversion. Two boards:
// runs, groups.
...
const FREQUENCY_LABEL: Record<PayGroup["frequency"], string> = {
  WEEKLY: "Weekly", BIWEEKLY: "Biweekly", SEMIMONTHLY: "Semi-monthly", MONTHLY: "Monthly",
};
  ...
  return (
    <>
      ...
      {loading ? (
        ...
        <Card><CardContent className="p-10 text-center text-sm text-muted">No pay groups yet. Create one to start running payroll.</CardContent></Card>
      ) : (
        <Card>
  ...
  async function save() {
    ...
    try {
      const res = await fetch("/api/pay-groups", {
        ...
        body: JSON.stringify({
          name: name.trim(), country: country.trim().toUpperCase(), currency: currency.trim().toUpperCase(),
          frequency, anchorDate, payOffsetDays: Number(payOffsetDays) || 3,
        }),
      });
      const data = await res.json();
    ...
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        ...
        <div className="space-y-3 pt-1">
          <div className="space-y-1.5"><Label>Name</Label><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="US Salaried" autoFocus /></div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5"><Label>Anchor date</Label><Input type="date" value={anchorDate} onChange={(e) => setAnchorDate(e.target.value)} /></div>
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1.5"><Label>Country</Label><Input value={country} onChange={(e) => setCountry(e.target.value.toUpperCase().slice(0, 2))} maxLength={2} /></div>
            ...
            <div className="space-y-1.5"><Label>Pay offset (days)</Label><Input value={payOffsetDays} onChange={(e) => setPayOffsetDays(e.target.value)} inputMode="numeric" /></div>
          </div>
        </div>
        <DialogFooter>
...
export function PayRunsTab() {
  ...
  if (rows.length === 0) {
    return <Card><CardContent className="p-10 text-center text-sm text-muted">No pay runs yet. Create a pay group, then schedule the first run.</CardContent></Card>;
  }
  return (
    <Card>
      <CardContent className="p-0">
